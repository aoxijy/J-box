...
**Goal:** 补上 P3 服务控制当前指向的**不存在的文件**——两个 procd init 脚本(`/etc/init.d/jbox`、`/etc/init.d/jbox-panel`),以及面板挂掉时的 LuCI 兜底页;并修正恢复路径会拆掉面板访问通道的缺陷。
...
- 服务名与路径必须与 P3 `createPaths` 完全一致:core=`/etc/init.d/jbox`、panel=`/etc/init.d/jbox-panel`;二进制 `/opt/j-box/bin/sing-box`;配置 `/opt/j-box/etc/config.json`;数据 `/opt/j-box/data`
...
- 面板端口 2026;`JBOX_ROOT=/opt/j-box`
- 文件落位:`openwrt/initd/jbox`、`openwrt/initd/jbox-panel`、`openwrt/luci/`(P6 的安装脚本负责铺到系统路径)
...
- P3 `dns-takeover.mjs` 的备份文件路径:`${paths.dataDir}/dnsmasq-backup.txt`;J-Box 写入的 dnsmasq 上游值固定为 `127.0.0.1#7853`。
- P3 `firewall.mjs` 的两条具名规则:`firewall.jbox_panel`(面板 LAN 放行,端口 2026)、`firewall.jbox_v6block`(IPv6 泄漏拦截)。
- 面板进程需要的 env:`PORT`、`HOST`、`JBOX_ROOT`、`ZASHBOARD_DB_PATH`(默认落在 panel 目录下,init 脚本显式指到 `/opt/j-box/data/` 以便所有状态集中)。
- 安装布局(设计文档 §3):`/opt/j-box/{node,panel,bin,etc,data}`;Node 运行时在 `/opt/j-box/node/bin/node`,面板入口 `/opt/j-box/panel/server/index.mjs`。

---

### Task 1: `/etc/init.d/jbox`(sing-box 内核服务 + 停止时安全清理)

**Files:**
- Create: `openwrt/initd/jbox`

**Interfaces:**
- Produces:procd init 脚本。`start` 前校验二进制与配置存在(缺失则报错退出非 0,便于 P3 的 deploy 感知);`stop` 在停进程后执行**安全清理**。

**停止时安全清理的设计(重要,含一个刻意的取舍):**
- 移除 `firewall.jbox_v6block` 并 reload —— 内核已停,再拦 IPv6 只会让用户断 v6 网
- **保留** `firewall.jbox_panel` —— 那是用户访问面板的通道,恢复路径绝不能自断退路(仅卸载时移除,归 P6)
...
- [ ] **Step 1: 写脚本 `openwrt/initd/jbox`**

```sh
#!/bin/sh /etc/rc.common
# J-Box sing-box core service
...
JBOX_ROOT=/opt/j-box
BIN="$JBOX_ROOT/bin/sing-box"
CONF="$JBOX_ROOT/etc/config.json"
DATA="$JBOX_ROOT/data"
JBOX_DNS_UPSTREAM='127.0.0.1#7853'

start_service() {
	if [ ! -x "$BIN" ]; then
		echo "jbox: sing-box binary not found at $BIN" >&2
		return 1
	fi
	if [ ! -f "$CONF" ]; then
		echo "jbox: config not found at $CONF" >&2
		return 1
	fi

	procd_open_instance jbox
...
jbox_cleanup() {
	uci -q del_list dhcp.@dnsmasq[0].server="$JBOX_DNS_UPSTREAM"
	uci -q commit dhcp
	/etc/init.d/dnsmasq restart >/dev/null 2>&1

	uci -q delete firewall.jbox_v6block
...
```bash
sh -n openwrt/initd/jbox && echo SYNTAX-OK
```
...
```bash
git add openwrt/initd/jbox-panel
...
const core = readScript('jbox')
const panel = readScript('jbox-panel')
const paths = createPaths()

test('init 脚本文件名与 createPaths 的 initd 路径一致', () => {
  assert.equal(paths.initd.core, '/etc/init.d/jbox')
  assert.equal(paths.initd.panel, '/etc/init.d/jbox-panel')
})

test('内核脚本引用的二进制与配置路径与 createPaths 一致', () => {
  assert.ok(core.includes('/opt/j-box/bin/sing-box'), '二进制路径漂移')
  assert.ok(core.includes('/opt/j-box/etc/config.json'), '配置路径漂移')
  assert.equal(paths.singbox, '/opt/j-box/bin/sing-box')
  assert.equal(paths.configPath, '/opt/j-box/etc/config.json')
})

test('两个脚本都启用 procd(status/enable/disable 依赖它)', () => {
  for (const [name, body] of [['jbox', core], ['jbox-panel', panel]]) {
...
test('内核停止清理:移除 v6 拦截但保留面板放行规则', () => {
  assert.match(core, /uci -q delete firewall\.jbox_v6block/)
  assert.ok(
    !/uci -q delete firewall\.jbox_panel/.test(core),
    '停止时不得移除面板放行规则,否则用户会失去访问恢复界面的通道',
  )
})

test('面板脚本以 2026 端口与 JBOX_ROOT 启动', () => {
  assert.match(panel, /PORT=2026/)
  assert.match(panel, /JBOX_ROOT=/)
})
...
**Interfaces:**
- 问题:`rollbackToDirect` 调 `removeJBoxRules`,后者同时删掉 `firewall.jbox_panel`(面板 LAN 放行)。恢复路径把用户访问恢复界面的通道也拆了——若 LAN→路由器 input 策略不是 ACCEPT,用户将彻底进不去面板。
- 修复:把移除拆成两个函数,语义分明:
  - `removeProxyRules(ctx)` —— 只移除 `jbox_v6block`(代理相关),供 `rollbackToDirect` 使用
  - `removeJBoxRules(ctx)` —— 移除全部两条(含面板放行),**仅供卸载**(P6)使用
...
test('rollbackToDirect 不移除面板 LAN 放行规则(否则自断恢复通道)', async () => {
  ...
  assert.ok(joined.includes('delete firewall.jbox_v6block'), '应移除 v6 拦截')
  assert.ok(
    !joined.includes('delete firewall.jbox_panel'),
    '不得移除面板放行规则',
  )
})
```

在 `firewall.test.mjs` 追加 `removeProxyRules` 只删一条、`removeJBoxRules` 删两条的断言。
...
### Task 5: LuCI 兜底页(luci-app-jbox)

**Files:**
- Create: `openwrt/luci/htdocs/luci-static/resources/view/jbox/status.js`
- Create: `openwrt/luci/root/usr/share/luci/menu.d/luci-app-jbox.json`
- Create: `openwrt/luci/root/usr/share/rpcd/acl.d/luci-app-jbox.json`

**Interfaces:**
- Produces:LuCI「服务 → J-Box」页面,**面板挂掉时的救场入口**。功能限定为遥控器:
  - 两个服务各自的:运行状态、启动/停止、开机自启开关
  - 「紧急停止并恢复直连」按钮(= 停止内核,其清理逻辑在 init 脚本里)
  - 「打开 J-Box 面板」链接(`http://<当前主机>:2026`)
...
return view.extend({
	load: function () {
		return Promise.all([ serviceState('jbox'), serviceState('jbox-panel') ]);
	},
	...
	render: function (data) {
		...
		return E('div', {}, [
			...
			E('p', { 'class': 'cbi-section-descr' },
				_('Fallback controls. Full management lives in the J-Box panel.')),

			serviceRow.call(this, _('sing-box core'), 'jbox', core),
			serviceRow.call(this, _('J-Box panel'), 'jbox-panel', panel),
			...
			E('div', { 'class': 'cbi-section' }, [
				...
				E('button', { 'class': 'cbi-button cbi-button-negative',
					'click': ui.createHandlerFn(this, function () { return act('jbox', 'stop'); }) },
...
- [ ] **Step 2: 写菜单 `menu.d/luci-app-jbox.json`**
...
```json
{
	"admin/services/jbox": {
		"title": "J-Box",
		"order": 30,
		"action": { "type": "view", "path": "jbox/status" },
		"depends": { "acl": [ "luci-app-jbox" ] }
	}
}
```

- [ ] **Step 3: 写 ACL `acl.d/luci-app-jbox.json`**

```json
{
	"luci-app-jbox": {
		"description": "Grant access to J-Box service controls",
		"read": {
...
```bash
node -e "JSON.parse(require('fs').readFileSync('openwrt/luci/root/usr/share/luci/menu.d/luci-app-jbox.json','utf8')); JSON.parse(require('fs').readFileSync('openwrt/luci/root/usr/share/rpcd/acl.d/luci-app-jbox.json','utf8')); console.log('JSON-OK')"
node -e "const s=require('fs').readFileSync('openwrt/luci/htdocs/luci-static/resources/view/jbox/status.js','utf8'); new Function(s); console.log('JS-SYNTAX-OK')"
```
...
git commit -m "feat(luci): J-Box 兜底页(两服务启停/自启、紧急停止、面板跳转)+ 菜单与 ACL"
```
...
**Files:**
- Modify: `panel/server/store/jbox-store.mjs`(新增 `wizardDone` 状态)
...
- 修复:后端 store 增加 `jbox/wizard-done`(布尔),`GET /api/jbox/profile` 的响应或独立端点暴露;向导是否显示以**后端状态**为准,localStorage 仅作为本地缓存。全新安装时后端为 false → 必定显示引导。
...
- procd init 脚本 jbox / jbox-panel → Task 1/2。✅
...
- `removeProxyRules`/`removeJBoxRules` 的拆分在 Task 4 定义,`rollbackToDirect`(P3)与卸载脚本(P6)分别消费。✅
