...
- 安装根 `/opt/j-box`(可注入,测试用临时路径);服务名 `jbox`(内核)、`jbox-panel`(面板)
...
test('mock exec 返回编排结果并记录调用', async () => {
  const ctx = createMockContext({ execResults: { '/etc/init.d/jbox status': { code: 0, stdout: 'running' } } })
  const r = await ctx.exec('/etc/init.d/jbox', ['status'])
  assert.equal(r.code, 0)
  assert.equal(r.stdout, 'running')
  assert.deepEqual(ctx.calls, [{ cmd: '/etc/init.d/jbox', args: ['status'] }])
})
...
**Interfaces:**
- Produces:
  - `paths.mjs`:`createPaths(root = '/opt/j-box') -> { root, bin, singbox, etc, configPath, dataDir, rulesetDir, initd: { core, panel } }`
    - `bin=${root}/bin`、`singbox=${root}/bin/sing-box`、`etc=${root}/etc`、`configPath=${root}/etc/config.json`、`dataDir=${root}/data`、`rulesetDir=${root}/data/rulesets`、`initd.core='/etc/init.d/jbox'`、`initd.panel='/etc/init.d/jbox-panel'`
...
test('createPaths 默认根与派生路径', () => {
  const p = createPaths()
  assert.equal(p.root, '/opt/j-box')
  assert.equal(p.singbox, '/opt/j-box/bin/sing-box')
  assert.equal(p.configPath, '/opt/j-box/etc/config.json')
  assert.equal(p.rulesetDir, '/opt/j-box/data/rulesets')
  assert.equal(p.initd.core, '/etc/init.d/jbox')
  assert.equal(p.initd.panel, '/etc/init.d/jbox-panel')
})
...
test('服务动作发出正确命令', async () => {
  ...
  assert.deepEqual(ctx.calls, [
    { cmd: '/etc/init.d/jbox', args: ['start'] },
    { cmd: '/etc/init.d/jbox', args: ['stop'] },
    { cmd: '/etc/init.d/jbox', args: ['restart'] },
    { cmd: '/etc/init.d/jbox-panel', args: ['enable'] },
    { cmd: '/etc/init.d/jbox-panel', args: ['disable'] },
  ])
})

test('失败返回 ok:false 与 stderr', async () => {
  const ctx = createMockContext({ defaultExec: { code: 1, stderr: 'no such service' } })
  const r = await startService(ctx, '/etc/init.d/jbox')
  assert.equal(r.ok, false)
  assert.equal(r.stderr, 'no such service')
})

test('serviceStatus 判定 running', async () => {
  const yes = createMockContext({ execResults: { '/etc/init.d/jbox status': { code: 0, stdout: 'running' } } })
  assert.equal((await serviceStatus(yes, '/etc/init.d/jbox')).running, true)
  const no = createMockContext({ execResults: { '/etc/init.d/jbox status': { code: 1, stdout: 'inactive' } } })
  assert.equal((await serviceStatus(no, '/etc/init.d/jbox')).running, false)
})
```
...
test('checkConfig 通过', async () => {
  const ctx = createMockContext({ execResults: { '/opt/j-box/bin/sing-box check -c /tmp/c.json': { code: 0 } } })
...
- Produces:
  - `applyDnsTakeover(ctx, { mode }) -> { changed, actions }` — `mode='hijack'` 时**什么都不做**(接管在 sing-box 配置内完成),返回 `{changed:false}`;`mode='dnsmasq'` 时用 uci 把 dnsmasq 上游指向 sing-box DNS 入站:
    ...
    并在改动前把原值备份到 uci 的 J-Box 命名空间(用文件备份更简单可靠):把 `uci show dhcp.@dnsmasq[0]` 的输出写入 `${paths.dataDir}/dnsmasq-backup.txt`(仅当备份不存在时写,避免二次覆盖)。
  - `restoreDnsTakeover(ctx, paths) -> { restored }` — 还原:`uci -q delete dhcp.@dnsmasq[0].server`;`uci -q delete dhcp.@dnsmasq[0].noresolv`;若存在备份文件,解析其中的 `server=` / `noresolv=` 行并重新 set/add_list;`uci commit dhcp`;`/etc/init.d/dnsmasq restart`;删除备份文件。**幂等**:无备份时也能安全执行(仅清除 J-Box 写入的值)。
...
test('dnsmasq 模式:备份 + 设上游 + 重启', async () => {
  ...
  // 备份落盘
  assert.ok(ctx.files['/opt/j-box/data/dnsmasq-backup.txt'].includes('223.5.5.5'))
})

test('dnsmasq 模式:已有备份不覆盖', async () => {
  const ctx = createMockContext({ files: { '/opt/j-box/data/dnsmasq-backup.txt': 'ORIGINAL' } })
  await applyDnsTakeover(ctx, paths, { mode: 'dnsmasq' })
  assert.equal(ctx.files['/opt/j-box/data/dnsmasq-backup.txt'], 'ORIGINAL')
})

test('还原:清除写入值并恢复备份,删备份文件', async () => {
  const ctx = createMockContext({ files: { '/opt/j-box/data/dnsmasq-backup.txt': "dhcp.cfg01411c.server='223.5.5.5'\ndhcp.cfg01411c.noresolv='0'\n" } })
  ...
  assert.ok(c.includes('uci commit dhcp'))
  assert.ok(c.includes('/etc/init.d/dnsmasq restart'))
  assert.equal(await ctx.exists('/opt/j-box/data/dnsmasq-backup.txt'), false)
})
...
- Produces(全部经 uci 操作 OpenWrt fw4,规则用固定 name 便于幂等增删):
  - `applyPanelLanRule(ctx, { port = 2026 }) -> { applied }` — 先删同名规则再新增,避免重复:
    - `uci -q delete firewall.jbox_panel`
    - `uci set firewall.jbox_panel=rule`,`...name='J-Box Panel (LAN)'`,`...src='lan'`,`...proto='tcp'`,`...dest_port='2026'`,`...target='ACCEPT'`
    - `uci commit firewall`;`/etc/init.d/firewall reload`
  - `applyIpv6Block(ctx, { enabled }) -> { applied }` — `enabled=true`(即 IPv6 代理关闭时拦截 v6 出站防泄漏)则建 `firewall.jbox_v6block` 规则:`src='lan'`、`dest='wan'`、`family='ipv6'`、`target='REJECT'`;`enabled=false` 则删除该规则。同样 commit+reload。
  - `removeJBoxRules(ctx) -> { removed }` — 删除两条规则并 commit+reload(卸载/恢复直连时用)。
...
import test from 'node:test'
import { createMockContext } from './context.mjs'
import { applyPanelLanRule, applyIpv6Block, removeJBoxRules } from './firewall.mjs'
...
test('面板 LAN 规则:先删后建 + reload', async () => {
  ...
  const c = cmds(ctx)
  assert.equal(c[0], 'uci -q delete firewall.jbox_panel')
  assert.ok(c.includes('uci set firewall.jbox_panel=rule'))
  assert.ok(c.includes('uci set firewall.jbox_panel.src=lan'))
  assert.ok(c.includes('uci set firewall.jbox_panel.dest_port=2026'))
  assert.ok(c.includes('uci set firewall.jbox_panel.target=ACCEPT'))
...
test('IPv6 拦截开启建 REJECT 规则', async () => {
  ...
  const c = cmds(ctx)
  assert.ok(c.includes('uci set firewall.jbox_v6block=rule'))
  assert.ok(c.includes('uci set firewall.jbox_v6block.family=ipv6'))
  assert.ok(c.includes('uci set firewall.jbox_v6block.target=REJECT'))
})
...
test('IPv6 拦截关闭则删除规则', async () => {
  ...
  const c = cmds(ctx)
  assert.ok(c.includes('uci -q delete firewall.jbox_v6block'))
  assert.ok(!c.some((x) => x.includes('jbox_v6block=rule')))
})

test('removeJBoxRules 清两条 + reload', async () => {
  const ctx = createMockContext()
  const r = await removeJBoxRules(ctx)
  assert.equal(r.removed, true)
  const c = cmds(ctx)
  assert.ok(c.includes('uci -q delete firewall.jbox_panel'))
  assert.ok(c.includes('uci -q delete firewall.jbox_v6block'))
  assert.ok(c.includes('/etc/init.d/firewall reload'))
})
```
...
```js
const PANEL_RULE = 'firewall.jbox_panel'
const V6BLOCK_RULE = 'firewall.jbox_v6block'
...
export const applyPanelLanRule = async (ctx, { port = 2026 } = {}) => {
  ...
  await ctx.exec('uci', ['set', `${PANEL_RULE}.name=J-Box Panel (LAN)`])
...
export const applyIpv6Block = async (ctx, { enabled }) => {
  ...
  if (enabled) {
    ...
    await ctx.exec('uci', ['set', `${V6BLOCK_RULE}.name=J-Box Block IPv6 Leak`])
...
export const removeJBoxRules = async (ctx) => {
  await ctx.exec('uci', ['-q', 'delete', PANEL_RULE])
  await ctx.exec('uci', ['-q', 'delete', V6BLOCK_RULE])
  await commitReload(ctx)
  return { removed: true }
...
- Produces:
  ...
  - `rollbackToDirect(ctx, paths) -> { ok, actions }` — 恢复裸直连:`stopService(core)` → `restoreDnsTakeover` → `removeJBoxRules`。**幂等**,任何步骤失败不阻断后续(尽最大努力恢复),返回执行过的 actions。
...
const okCtx = (over = {}) => createMockContext({
  execResults: {
    '/etc/init.d/jbox status': { code: 0, stdout: 'running' },
    ...over,
  },
})
...
test('冲突时不改系统', async () => {
  ...
  assert.ok(!cmds(ctx).some((c) => c.includes('jbox restart')))
})
...
test('校验失败:不写正式配置、不重启、给 badTags', async () => {
  ...
  assert.ok(!cmds(ctx).some((c) => c.includes('/etc/init.d/jbox restart')))
})
...
test('成功路径:写配置 + 防火墙 + 重启 + 验证', async () => {
  ...
  const c = cmds(ctx)
  assert.ok(c.includes('uci set firewall.jbox_panel=rule'))
  assert.ok(c.includes('/etc/init.d/jbox restart'))
})

test('IPv6 关闭时下发 v6 拦截规则', async () => {
  const ctx = okCtx()
  await deployConfig(ctx, paths, { config, profile: { ...profile, ipv6: false } })
  assert.ok(cmds(ctx).includes('uci set firewall.jbox_v6block=rule'))
})

test('重启失败 → 回滚恢复直连', async () => {
  const ctx = createMockContext({
    execResults: {
      '/etc/init.d/jbox restart': { code: 1, stderr: 'start failed' },
    },
  })
  const r = await deployConfig(ctx, paths, { config, profile })
  assert.equal(r.ok, false)
  ...
  const c = cmds(ctx)
  assert.ok(c.includes('/etc/init.d/jbox stop'))          // 回滚停服务
  assert.ok(c.includes('uci -q delete firewall.jbox_panel'))  // 撤规则
})

test('启动后未 running → 回滚', async () => {
  const ctx = createMockContext({
    execResults: { '/etc/init.d/jbox status': { code: 1, stdout: 'inactive' } },
  })
  const r = await deployConfig(ctx, paths, { config, profile })
  assert.equal(r.ok, false)
  assert.equal(r.stage, 'verify')
  assert.ok(cmds(ctx).includes('/etc/init.d/jbox stop'))
})
...
export const rollbackToDirect = async (ctx, paths) => {
  ...
  try { await removeJBoxRules(ctx); actions.push('remove-firewall') } catch { /* 尽力而为 */ }
