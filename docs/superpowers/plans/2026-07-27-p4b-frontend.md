...
- **后端 API 已就绪**(P4a,全部在 `/api/jbox/*`,均需登录):
  ...
  - 订阅:`POST /api/jbox/subscriptions/preview {url|content, renameOptions}`、`POST /api/jbox/subscriptions {url,name,renameOptions}`、`GET /api/jbox/subscriptions`、`DELETE /api/jbox/subscriptions/:id`、`POST /api/jbox/subscriptions/:id/refresh`
  - Profile:`GET|PUT /api/jbox/profile`、`GET /api/jbox/profile/defaults?region=XX`
  - 部署:`GET /api/jbox/config/preview`、`POST /api/jbox/deploy`(409=冲突/校验失败,500=启动失败;body 含 `stage`/`message`/`badTags`)、`GET /api/jbox/deploy/state`、`POST /api/jbox/rollback`
  - 服务:`GET /api/jbox/service/status`、`POST /api/jbox/service/core/:action`(start|stop|restart|enable|disable)、`GET /api/jbox/kernel/version`
  - 穿透:`POST /api/jbox/penetration {target}` → `{matched, chain, finalOutbound, chainError?}`
...
- 区域步骤调 `GET /api/jbox/profile/defaults?region=XX` 取推荐值,展示为人话摘要(如「中国网站直连,其他走代理」),用户可直接接受
...
- [ ] Step 3: 三语言文案齐全,面向小白
- [ ] Step 4: 手动走查:全新库 → 完整走完 → profile 与订阅确实落库(用 `GET /api/jbox/profile`、`/subscriptions` 核对)
...
- Produces:
  ...
  - 保存即 `PUT /api/jbox/profile`;顶部常驻「有未部署的更改」提示 + 「立即部署」按钮
...
- Produces:
  ...
  - 「紧急停止并恢复直连」按钮(调 `POST /api/jbox/rollback`),需二次确认,文案说明后果
  - 穿透查询:输入域名/IP → 展示命中的规则(第几条、规则集)、策略目标、策略链下钻、最终出口;`chainError` 时降级展示并说明原因
  - 部署状态展示(读 `GET /api/jbox/deploy/state`)
...
**Interfaces:**
- Produces:去除 AnGe-ClashBoard 残留品牌(PWA 安装后应用名为 J-Box);README 与实际功能一致
