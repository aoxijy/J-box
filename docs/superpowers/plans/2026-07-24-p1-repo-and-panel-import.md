...
**Goal:** 建立 J-Box monorepo 骨架,把 AnGe-ClashBoard 面板代码导入 `panel/`,在本机(macOS)跑通安装→构建→测试→启动闭环,并完成 J-Box 品牌化(名称/端口 2026)。
...
- 面板名称/品牌:**J-Box**(package name `j-box-panel`)
- 保留上游 MIT LICENSE 链:zashboard → AnGe-ClashBoard → J-Box(规格"授权"要求)
...
- 设计文档:`docs/superpowers/specs/2026-07-21-j-box-design.md`
- 实施路线图:`docs/superpowers/plans/2026-07-24-j-box-roadmap.md`
...
rsync -a \
  ...
  --exclude .runtime \
  /Users/ange/Code/AnGe-Board/AnGe-ClashBoard/ /Users/ange/Code/J-Box/panel/
```
...
原始 LICENSE 见本目录 LICENSE 文件。J-Box 对其做单后端裁剪与本机管理改造。
```
...
cd /Users/ange/Code/J-Box/panel
...
**Interfaces:**
- Produces: 默认端口 2026;`j-box-panel` 包名。后续阶段(安装脚本、LuCI 跳转、防火墙规则)一律引用 2026
...
`panel/package.json`:`"name": "j-box-panel"`,`"description": "J-Box panel - integrated sing-box management for OpenWrt"`。
`panel/index.html`:`<title>` 改为 `J-Box`。
...
```bash
git add -A
git commit -m "feat: J-Box 品牌化,面板默认端口改为 2026"
```
...
**Files:**
- Create: `scripts/dev-panel.sh`(替代上游 deploy-devboard.sh 的 J-Box 版)
...
for _ in $(seq 1 20); do
  if curl -sf "http://127.0.0.1:$port/api/health" >/dev/null; then
    echo "J-Box panel OK: http://127.0.0.1:$port"
    exit 0
  fi
  sleep 0.5
done
...
Expected: `J-Box panel OK: http://127.0.0.1:2026`
