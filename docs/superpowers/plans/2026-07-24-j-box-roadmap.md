...
> 规格:`docs/superpowers/specs/2026-07-21-j-box-design.md`
...
| 阶段 | 名称 | 产出 | 依赖 |
| --- | --- | --- | --- |
| P1 | 仓库落地与面板导入 | monorepo 脚手架;AnGe-ClashBoard 代码导入 `panel/`;构建/测试/本地开发闭环跑通;品牌与端口改为 J-Box/2026 | 无 |
...
| P5 | OpenWrt 侧 | procd init 脚本(jbox / jbox-panel)、tun 与 DNS 接管落地脚本、LuCI 兜底页(luci-app-jbox,JS + rpcd ACL)、紧急停止 | P3 |
...
- [x] P5 完成并合并(OpenWrt 侧:jbox/jbox-panel 两个 procd init 脚本、LuCI 兜底页、回滚不再拆面板通道、向导门控迁后端;305 测试 + 金标准 4/4;终审修复 3 Critical)
...
**P5/P6 处理:**
- 向导门控标志(`config/wizard-dismissed` 等)存在浏览器 localStorage 并经 storage 同步推给后端:工厂重置后,之前用过的浏览器会**抑制新机的引导流程**。应改为以后端状态为准(独立 jbox 字段)
...
**P6 安装脚本必须处理:**
- 两个 init 脚本铺到 `/etc/init.d/` 并 `chmod +x`;LuCI 三个文件铺到 `htdocs/luci-static/resources/view/jbox/`、`/usr/share/luci/menu.d/`、`/usr/share/rpcd/acl.d/`,装完需 `rm -f /tmp/luci-*cache*` 并重启 rpcd 使 ACL 生效
- 卸载时才调用 `removeJBoxRules`(含面板放行规则);`removeProxyRules` 仅供回滚使用
...
**已知残留(不阻塞):** LuCI master 已移除 `luci.setInitAction`(改用 `rc` 对象),若 P7 用 snapshot 固件需迁移;`restart()` 未套 `trap '' TERM`(仅在从 jbox 进程树内部调用时才有影响);deploy 预检只查存在性不查可执行位。

## 记录在案(来自 P6 终审)

**首次发版前必须确认:** 仓库标识 `aoxijy/J-box` 目前是占位(本地无 git remote),它被硬编码在 `scripts/install.sh`、`scripts/update.sh`、`.github/workflows/release.yml`、`README.md` 四处。与真实 GitHub 仓库不一致会让文档里每条命令 404。
...
**P7 真机验收补充项(终审建议):**
- 第一个冒烟点:`/opt/j-box/node/bin/node -v` 能否在 stock x86_64 OpenWrt 上跑起来(验证 musl + libstdc++ 捆绑)
...
真实 SystemContext 去 exec 宿主机的 `/etc/init.d/jbox status` 与 sing-box
