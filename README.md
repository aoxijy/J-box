<picture>
  ...
  <img src="docs/pic/logo.png" alt="J-Box" height="72">
</picture>
...
**后端设置**:IPv6、测速地址和 J-Box 统一更新的计划任务都在这里。
...
- **自动更新**:J-Box 可按天定时检查新版本，程序、sing-box 内核和 GeoSite / GeoIP 数据一起管理；已校验且版本一致的组件不重复下载。
...
```bash
curl -fsSL https://raw.githubusercontent.com/aoxijy/J-box/main/scripts/install.sh | sh
```

GitHub 访问不畅时用加速版(脚本内置了几个加速站,会依次探测自动挑一个能用的):

```bash
curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/aoxijy/J-box/main/scripts/install.sh | sh -s -- --mirror
```

已经有信得过的加速站,也可以指定具体前缀跳过探测:`sh -s -- --mirror <镜像前缀>`。

安装脚本先做预检(架构、存储、内存、系统),通过才下载对应架构的发布包、校验 SHA256、铺装到 `/opt/j-box/` 并启动面板。校验不过或下载失败,系统不会有任何改动。
...
```bash
curl -fsSL https://raw.githubusercontent.com/aoxijy/J-box/main/scripts/update.sh | sh
```

强制直连 GitHub 加 `-s -- --direct`,强制走加速加 `-s -- --mirror [前缀]`。不带参数时沿用安装时选的下载通道。

升级保留 `data/`(订阅、规则、面板密码)和已生效的运行配置,先读取本仓库 Release 的组件清单，再校验本地 sing-box 与 GeoSite / GeoIP 的版本和文件。相同且完整的组件直接复用，变化、缺失或损坏的组件才下载。程序、内核、Geo 数据均从 `aoxijy/J-box` 的 Release 获取，设备无需连接 SagerNet 或 MetaCubeX。升级前内核在跑的话，升级完会重新生成配置并自动恢复。
...
首次安装还会随包带一份现成的配置骨架：面板设置、目标分流（含各站点集引用的规则集链接）、出站分组（只带分组结构 —— 名字 / 类型 / 检测间隔 / 容差 / 图标，成员节点留空，导入自己的订阅后在同一处勾选）。「目标分流」里按网址引用的那批规则集（`github.com/aoxijy/guize` 的 `.mrs` 与文本名单）在包里已经编好 `.srs` 并带上状态，首次部署不必联网下载；装好后 24 小时内不会重新拉，之后按常规 24 小时周期刷新，拉不动时继续用包里这份。

## 卸载

```bash
# 停服务、清理系统改动、删程序文件;默认保留 data/
curl -fsSL https://raw.githubusercontent.com/aoxijy/J-box/main/scripts/uninstall.sh | sh

# 连 data/ 一起删,彻底清干净
curl -fsSL https://raw.githubusercontent.com/aoxijy/J-box/main/scripts/uninstall.sh | sh -s -- --purge
```
...
装完之后路由器自带界面里会多一个入口:**服务 → J-Box**。面板本身打不开时用它救场:
