...
var I18N = {
	'zh-Hans': {
		'Basic settings. Full management lives in the J-Box panel:':
			'基本设置,完整管理请到 J-Box 面板:',
		'sing-box core': 'sing-box 内核',
		'J-Box panel': 'J-Box 面板',
		'J-Box upgrade': 'J-Box 升级',
		'Stopping also turns off autostart (so it stays stopped after a reboot) and restores plain internet access: the IPv6 leak block and the J-Box DNS upstream are removed. The panel stays reachable.':
			'停止会同时关闭开机自启(重启后不会自己跑起来),并恢复正常上网:IPv6 泄漏拦截与 J-Box 写入的 DNS 上游都会被移除。面板仍然可以访问。',
		'Autostart': '开机自启',
		'running': '运行中',
		'stopped': '已停止',
		'on': '开',
		...
		'Update did not start. No response after 20 seconds — check the log below, or run the update over SSH instead: sh /opt/j-box/update.sh':
			'更新未能启动,20 秒内没有任何响应——请查看下方日志,或改用 SSH 手动执行:sh /opt/j-box/update.sh',
		'Update progress unavailable': '无法读取更新进度',
		'Cannot read update progress for this session. The update may still be running in the background — this usually means the LuCI session needs to log in again for newly granted permissions to take effect. You can also check directly over SSH: cat /tmp/jbox-update.status':
			'本次会话无法读取更新进度。更新可能仍在后台运行——这通常是因为需要重新登录 LuCI 让新授予的权限生效。也可以改用 SSH 直接查看:cat /tmp/jbox-update.status',
		...
		'Close': '关闭',
		'Remove J-Box from this router. Services are stopped, DNS and firewall changes are reverted, and the LuCI page disappears after the next refresh.':
			'从这台路由器上移除 J-Box。服务会被停止,DNS 与防火墙改动会被还原,刷新后本页面也会消失。',
		'Uninstall J-Box': '卸载 J-Box',
	...
	'zh-Hant': {
		'Basic settings. Full management lives in the J-Box panel:':
			'基本設定,完整管理請到 J-Box 面板:',
		'sing-box core': 'sing-box 核心',
		'J-Box panel': 'J-Box 面板',
		'J-Box upgrade': 'J-Box 升級',
		'Stopping also turns off autostart (so it stays stopped after a reboot) and restores plain internet access: the IPv6 leak block and the J-Box DNS upstream are removed. The panel stays reachable.':
			'停止會同時關閉開機自啟(重新啟動後不會自己執行),並恢復正常上網:IPv6 洩漏攔截與 J-Box 寫入的 DNS 上游都會被移除。面板仍然可以存取。',
		'Autostart': '開機自啟',
		'running': '執行中',
		'stopped': '已停止',
		'on': '開',
		...
		'Update did not start. No response after 20 seconds — check the log below, or run the update over SSH instead: sh /opt/j-box/update.sh':
			'更新未能啟動,20 秒內沒有任何回應——請查看下方日誌,或改用 SSH 手動執行:sh /opt/j-box/update.sh',
		'Update progress unavailable': '無法讀取更新進度',
		'Cannot read update progress for this session. The update may still be running in the background — this usually means the LuCI session needs to log in again for newly granted permissions to take effect. You can also check directly over SSH: cat /tmp/jbox-update.status':
			'本次工作階段無法讀取更新進度。更新可能仍在背景執行——這通常是因為需要重新登入 LuCI 讓新授予的權限生效。也可以改用 SSH 直接檢視:cat /tmp/jbox-update.status',
		...
		'Close': '關閉',
		'Remove J-Box from this router. Services are stopped, DNS and firewall changes are reverted, and the LuCI page disappears after the next refresh.':
			'從這台路由器上移除 J-Box。服務會被停止,DNS 與防火牆變更會被還原,重新整理後本頁面也會消失。',
		'Uninstall J-Box': '解除安裝 J-Box',
...
// 版本
// ---------------------------------------------------------------------------
var META_PATH = '/opt/j-box/meta.json';
var REPO = 'aoxijy/J-box';
...
// sing-box 内核的版本号是随 J-Box 发行版本一起钦定、写死进 meta.json 的构建期
// 常量(见 scripts/build-release.sh 的 SINGBOX_VERSION),不是探测运行中的二进制
// 现查出来的——这里只是读出这个钦定值展示给用户。之所以不提供独立于 J-Box 发行
...
// 旧配置。所以内核版本随 J-Box 整个发行版本一起走:要升级内核,走 J-Box
...
var UNINSTALL_PATH = '/opt/j-box/uninstall.sh';
...
// "更新失败"报给用户,而真实情况是 /tmp/jbox-update.status 一路推进到
...
// 说明)。
// ---------------------------------------------------------------------------
var UPDATE_PATH = '/opt/j-box/update.sh';
var UPDATE_LOG_PATH = '/tmp/jbox-update.log';
...
var UPDATE_STATUS_PATH = '/tmp/jbox-update.status';
...
// /tmp/jbox-update.status 的读权限是后来才加进 ACL 的,一个跨越了这次权限
...
// 状态文件读不到时的退路:直接把日志里最后一条 [j-box] 行当进度文案。
...
function lastProgressLine(logTail) {
	...
	for (var i = lines.length - 1; i >= 0; i--) {
		...
		if (line.indexOf('[j-box]') === 0) {
			return line.slice('[j-box]'.length).replace(/^\s+/, '');
		}
	}
	return '';
}
...
function runUpdate(channel, expectVersion) {
	var args = [ '--detach' ];
	if (channel === 'direct') {
		args.push('--direct');
	} else {
	...
	// 把面板刚探到的版本号交给 update.sh(--expect):它据此下载带版本号的资产并在解包后
	// 核对版本;不传的话 update.sh 只能自己探 releases/latest,没有 curl 的路由器上探不到
	// 就退回稳定资产名——加速镜像缓存的上一版同名包会被当成新版装上(2026-09-03 真机踩过)。
	if (typeof expectVersion === 'string' && /^v?[0-9A-Za-z._-]+$/.test(expectVersion))
		args.push('--expect', expectVersion);
...
var STYLE_CSS =
	...
	// 「J-Box 升级」横跨两列把这个洞填掉(渠道列表另有 max-width 兜着,不会
	// 被拉成横贯全屏的稀疏长条)。
...
return view.extend({
	...
	render: function (data) {
		...
		function showUninstallDialog() {
			...
			ui.showModal(tr('Uninstall J-Box'), [
				...
				E('div', { 'class': 'right', 'style': BTNROW }, [
					...
					E('button', { 'class': 'cbi-button cbi-button-negative',
						'click': ui.createHandlerFn(self, function () {
							...
							ui.showModal(tr('Uninstall J-Box'), [ E('p', { 'class': 'spinning' }, tr('Uninstalling...')) ]);
		...
		function startUpdate(latestVersion, channel) {
			...
			function handleStartingStuck() {
				...
				readUpdateLogTail().then(function (logRes) {
					showUpdateFailure(
						tr('Update did not start. No response after 20 seconds — check the log below, or run the update over SSH instead: sh /opt/j-box/update.sh'),
						logRes.text,
			...
			function handleStatusUnreadable() {
				...
				readUpdateLogTail().then(function (logRes) {
					if (finished) return;
					showUpdateFailure(
						tr('Cannot read update progress for this session. The update may still be running in the background — this usually means the LuCI session needs to log in again for newly granted permissions to take effect. You can also check directly over SSH: cat /tmp/jbox-update.status'),
						logRes.text,
			...
			runUpdate(channel, latestVersion).then(function () {
				if (finished) return;
				...
				if (window.console && window.console.debug) {
					window.console.debug('[j-box] fs.exec 的 XHR 出错(通常就是超时),' +
		...
		// 渠道已经在「J-Box 升级」卡片的下拉框里显式选好了(见下方 CHANNELS/selectedChannel),
		...
		// 检查结果作为「J-Box 升级」的副标题挂在标题后面(.ob-descr,和页面副标题
		// 同一套样式),不再单独占底部一行。
		...
		return E('div', {}, [
			...
			E('h2', { 'class': 'ob-head' }, [
				E('div', { 'class': 'ob-h2-left' }, [
					...
					E('span', { 'class': 'ob-descr' }, [
						tr('Basic settings. Full management lives in the J-Box panel:'),
			...
			// STYLE_CSS),三张自成一体的功能块:第一行 sing-box 内核 + J-Box
			// 面板,第二行 J-Box 升级横跨两列(.ob-card-wide,否则右下角会空一格)。
			// DOM 顺序即视觉顺序(CSS 网格按源码顺序逐行填格,不需要额外的
			// grid-area/order 声明)。卸载不在网格里,它在页面标题行右侧。
			E('div', { 'class': 'ob-wrap' }, [
				E('div', { 'class': 'ob-grid' }, [
					// 「停止」本身就会恢复正常上网(init 脚本的 stop_service 会摘掉 J-Box 写入的
					...
					// 独立的"升级内核"按钮:内核版本与 J-Box 发行版本是绑定发布的,配置是
					// 照着这个确切内核版本生成的,独立升级内核有打破这层对应关系、生成的配置被
					// 新内核拒绝启动的风险。升级入口统一指向下面「J-Box 升级」卡片里的
					// 「立即更新」——内核随整个发行版本一起换。
					E('div', { 'class': 'ob-card' }, [
						// 版本号右对齐放在标题行里:它本来是卡片底部一行带标签的
						// 「内核版本: x.y.z」,加一条分隔线和一段说明。标签、分隔线、
						// 说明都是冗余的——内核卡片里没有升级入口,升级统一在
						// 「J-Box 升级」卡片,这层关系不必再用文字重复一遍。
						cardTitle(tr('sing-box core'), core, singboxVersion || tr('Not installed')),
						serviceButtons('jbox', core, [ 'stop', 'disable' ],
							tr('Stopping also turns off autostart (so it stays stopped after a reboot) and restores plain internet access: the IPv6 leak block and the J-Box DNS upstream are removed. The panel stays reachable.'))
					]),

					// 「J-Box 面板」卡片同理是一个自成一体的功能块:标题行是标题 + 状态
					// 徽标 + 版本号,下面是启停控制。渠道选择、探测与升级操作篇幅大、也不是
					// "这张卡片自己的版本信息",单独成一张紧跟在后面的「J-Box 升级」卡片
					// (见下方),不再像旧版那样挤进面板卡片里。面板地址也不在这里了——它
					// 挪到了页面副标题(见上方 .ob-descr)。
					E('div', { 'class': 'ob-card' }, [
						cardTitle(tr('J-Box panel'), panel, installed || tr('Not installed')),
						serviceButtons('jbox-panel', panel, null)
					]),

					// 「J-Box 升级」独立成一张卡片(负责人手绘草图的要求):标题行右侧是
