#!/bin/sh
# J-Box 发布打包脚本(POSIX sh,不依赖 bash;OpenWrt 上不会跑这个脚本,
...
#
# 产出 <outdir>/j-box-<version>-linux-<arch>.tar.gz 与同名 .sha256(留档用),
# 以及内容完全一致但文件名不带版本号的 <outdir>/j-box-linux-<arch>.tar.gz 与
...
# `node/lib/`,并配合 `openwrt/initd/jbox-panel` 里的
# `LD_LIBRARY_PATH=/opt/j-box/node/lib` 生效。
#
...
# ---- 版本号:优先用调用方传入的 JBOX_VERSION(CI 里由 tag 提供),
# 本地试跑则退化为 git describe,再退化为固定占位符。----
VERSION="${JBOX_VERSION:-}"
...
log "打包 j-box $VERSION,目标 linux-$ARCH(sing-box 架构名: $SINGBOX_ARCH)"

# 内核只从当前仓库的源码补丁构建。CI 可传入同一轮已构建的目录以复用测试产物;
# 版本、架构、补丁/构建脚本和二进制哈希都必须匹配,没有旧 Release 下载兜底。
KERNEL_BUILD_DIR=${JBOX_KERNEL_BUILD_DIR:-"$CACHE_DIR/kernel-build"}
if [ -z "${JBOX_KERNEL_BUILD_DIR:-}" ]; then
...
STAGE=$(mktemp -d "${TMPDIR:-/tmp}/j-box-release.XXXXXX")
...
python3 "$SCRIPT_DIR/check-geodata.py" "$STAGE/panel/server/resources/geodata" "${JBOX_GEO_CHECK_BINARY:-$PANEL_DIR/.tools/sing-box}" "$SINGBOX_VERSION"
...
# ---- 8b. 卸载脚本 ----
# 随产物一起铺到 /opt/j-box/uninstall.sh:LuCI 兜底页要能在「面板已经坏了、
...
# ---- 8c. 升级脚本 ----
# 同理随产物铺到 /opt/j-box/update.sh:LuCI 兜底页的一键升级只能 exec 本地
...
VERSIONED_NAME="j-box-${VERSION}-linux-${ARCH}.tar.gz"
STABLE_NAME="j-box-linux-${ARCH}.tar.gz"
