#!/bin/sh
# J-Box 一键安装脚本(POSIX sh,兼容 OpenWrt ash;不使用 bash 专有语法)。
#
# 用法:
...
# - 若 /opt/j-box 已存在完整安装,拒绝安装并提示改用 update.sh;但如果里面
...
check_openwrt() {
  [ -r /etc/openwrt_release ] || die "未检测到 OpenWrt 系统(缺少 /etc/openwrt_release)。J-Box 只支持安装在 OpenWrt 路由器上。"
}
...
map_arch() {
  ...
  case "$RAW_ARCH" in
    x86_64) ARCH="x64" ;;
    aarch64) ARCH="arm64" ;;
    *) die "不支持的 CPU 架构:${RAW_ARCH:-未知}。J-Box 目前仅支持 x86_64 与 aarch64(arm64)路由器。" ;;
  esac
}

# 找到 /opt/j-box 所在(或将会所在)的文件系统,供 df 检测可用空间——
...
check_storage() {
  ...
  if [ "$kb" -lt "$MIN_FREE_KB" ]; then
    die "可用存储不足:检测到约 $((kb / 1024))MB,J-Box 至少需要 512MB 可用空间。请清理存储后重试。"
  fi
}
...
check_memory() {
  ...
  if [ "$mem_kb" -lt "$MIN_MEM_KB" ]; then
    die "内存不足:检测到约 $((mem_kb / 1024))MB,J-Box 至少需要 512MB 内存。"
  fi
}
...
check_conflicts() {
  ...
  if [ -n "$found" ]; then
    warn "检测到以下代理类插件已安装,可能与 J-Box 冲突,建议先停用它们:$found"
  fi
}
...
#   https://<前缀>/https://github.com/aoxijy/J-box/releases/...
...
if [ -n "$LATEST_TAG" ]; then
  ASSET="j-box-${LATEST_TAG}-linux-${ARCH}.tar.gz"
...
# ---------- 下载到临时目录(此时仍未触碰 /opt) ----------
TMP_DL=$(mktemp -d "${TMPDIR:-/tmp}/j-box-install.XXXXXX") || die "无法创建临时目录。"
...
# ---------- init 脚本 ----------
cp "$INSTALL_ROOT/openwrt/initd/jbox" /etc/init.d/jbox || die "无法安装 /etc/init.d/jbox。"
cp "$INSTALL_ROOT/openwrt/initd/jbox-panel" /etc/init.d/jbox-panel || die "无法安装 /etc/init.d/jbox-panel。"
chmod +x /etc/init.d/jbox /etc/init.d/jbox-panel

# ---------- LuCI 三文件 ----------
mkdir -p /www/luci-static/resources/view/jbox || die "无法创建 LuCI 视图目录。"
cp "$INSTALL_ROOT/openwrt/luci/htdocs/luci-static/resources/view/jbox/status.js" \
  /www/luci-static/resources/view/jbox/status.js || die "无法安装 LuCI 视图文件。"
...
cp "$INSTALL_ROOT/openwrt/luci/root/usr/share/luci/menu.d/luci-app-jbox.json" \
  /usr/share/luci/menu.d/luci-app-jbox.json || die "无法安装 LuCI 菜单文件。"
...
# procd 服务的返回码不总是可靠(见 openwrt/initd/jbox 注释),这里不把非零当作
...
echo ""
echo "========================================"
echo " J-Box 安装完成($VERSION)"
echo "========================================"
echo "面板地址: $PANEL_URL"
echo "首次打开面板需要设置管理密码。"
echo "如面板无法访问,可在路由器管理界面(LuCI)→ 服务 → J-Box 中查看/重启服务,或使用紧急停止恢复直连。"
echo ""
