...
if mode == 'create':
    ...
    manifest = {
        ...
        'build_inputs': inputs(),
        'jbox_commit': subprocess.check_output(['git', '-C', str(ROOT), 'rev-parse', 'HEAD'], text=True).strip(),
        'go_version': subprocess.check_output([go, 'version'], text=True).strip(),
        'clang_archive_sha256': os.environ.get('JBOX_CLANG_SHA256'),
        'sysroot_archive_sha256': os.environ.get('JBOX_SYSROOT_SHA256'),
