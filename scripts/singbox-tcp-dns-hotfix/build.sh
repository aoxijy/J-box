...
hotfix_work=$(mktemp -d "${TMPDIR:-/tmp}/jbox-tcp-dns-build.XXXXXX")
hotfix_go=${JBOX_GO_BINARY:-go}
...
"$hotfix_go" version
if [ -n "${JBOX_SINGBOX_SOURCE_ARCHIVE:-}" ]; then
  cp "$JBOX_SINGBOX_SOURCE_ARCHIVE" "$hotfix_work/source.tar.gz"
else
...
CGO_ENABLED=0 GOMAXPROCS=2 "$hotfix_go" test -p 2 -count=1 -timeout 30s \
  ./dns/transport ./common/urltest -run '^TestJBox(TCPDNS|HTTP)' -v
...
mkdir -p "$hotfix_source_bundle/j-box-patch"
cp "$hotfix_dir"/*.sh "$hotfix_dir"/*.py "$hotfix_dir"/*.go "$hotfix_dir"/*.patch "$hotfix_dir/README.md" "$hotfix_source_bundle/j-box-patch/"
cp "$hotfix_dir/../dt-needed.py" "$hotfix_source_bundle/j-box-patch/"
