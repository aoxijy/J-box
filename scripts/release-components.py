...
def build(stage, output):
    ...
    for kind, (version, roots) in layouts.items():
        name = f"j-box-{meta['version']}-linux-{meta['arch']}-{kind}.tar.gz"
        target = output / name
        def selected(entry):
    ...
    name = f"j-box-{meta['version']}-linux-{meta['arch']}-components.json"
    ...
    stable = f"j-box-linux-{meta['arch']}-components.json"
