#!/bin/sh
# 直接把当前工作区的改动推到一台已经装好 J-Box 的路由器上,跳过 打 tag → CI 出包
...
#
# 用法:
#   JBOX_DEV_PASS=root sh scripts/deploy-dev.sh [主机]      # 默认 192.168.3.35
#   sh scripts/deploy-dev.sh                                   # 不给密码则走 ssh 密钥
#
# 密码不写在脚本里:这是要进仓库的文件,凭据从环境变量来。给了 JBOX_DEV_PASS 就
# 用 expect 代填(OpenWrt 默认只开密码登录),没给就当作已经配好密钥。

set -eu

HOST="${1:-192.168.3.35}"
USER="${JBOX_DEV_USER:-root}"
...
_TMPCMD=$(mktemp "${TMPDIR:-/tmp}/jbox-deploy.XXXXXX")
_TMPREMOTE=$(mktemp "${TMPDIR:-/tmp}/jbox-remote.XXXXXX")
trap 'rm -f "$_TMPCMD" "$_TMPREMOTE"' EXIT INT TERM

if [ -n "${JBOX_DEV_PASS:-}" ]; then
  command -v expect >/dev/null 2>&1 || die "设置了 JBOX_DEV_PASS 但系统没有 expect。"
  export JBOX_DEV_PASS
  _run() {
    printf '%s\n' "$1" > "$_TMPCMD"
    expect -c "
      set timeout 900
      ...
      expect {
        \"assword:\" { send -- \"\$env(JBOX_DEV_PASS)\r\"; exp_continue }
        eof
      }
      catch wait result
      log_user 1
...
GEO_BUNDLE="${JBOX_GEO_BUNDLE_DIR:-$ROOT/.build-cache/geodata}"
...
info "同步 init 脚本..."
push "" "$ROOT/openwrt/initd/jbox" "/etc/init.d/jbox"
remote "chmod +x /etc/init.d/jbox" >/dev/null

# 升级脚本也同步:面板「后端设置 → J-Box 更新」调的是 /opt/j-box/update.sh,
...
info "同步 LuCI 视图..."
push "" "$ROOT/openwrt/luci/htdocs/luci-static/resources/view/jbox/" "/www/luci-static/resources/view/jbox/"
...
# ---------- 重启 ----------
info "重启面板..."
remote "/etc/init.d/jbox-panel restart >/dev/null 2>&1; rm -rf /tmp/luci-*cache* 2>/dev/null; true"
