...
用法: sh uninstall.sh [--purge]
  ...
  --purge     同时删除 /opt/j-box/data(数据库、订阅、规则集等)。默认保留。
  -h, --help  显示本帮助
EOF
}
...
if [ ! -e "$INSTALL_ROOT" ]; then
  info "未检测到 J-Box 安装($INSTALL_ROOT 不存在),无需卸载。"
  exit 0
fi

# ---------- 停止并禁用两个服务 ----------
# jbox 的 stop 会顺带做 P5 的安全清理(见文件头注释);restart 才会跳过清理,
# 这里调用的是普通 stop,清理一定会跑。
info "停止服务..."
if [ -x /etc/init.d/jbox-panel ]; then
  /etc/init.d/jbox-panel stop >/dev/null 2>&1 || true
  /etc/init.d/jbox-panel disable >/dev/null 2>&1 || true
fi
if [ -x /etc/init.d/jbox ]; then
  /etc/init.d/jbox stop >/dev/null 2>&1 || true
  /etc/init.d/jbox disable >/dev/null 2>&1 || true
fi
...
if command -v uci >/dev/null 2>&1; then
  ...
  # (firewall.jbox_srv_*)——都是我们写的,一个不留;留着的话以后任何服务占了
  # 那个端口就直接暴露到公网。
  uci -q delete firewall.jbox_panel || true
  uci -q delete firewall.jbox_dns || true
  uci -q delete firewall.jbox_v6block || true
  for _ob_rule in $(uci -q show firewall 2>/dev/null | sed -n 's/^firewall\.\(jbox_srv_[A-Za-z0-9_]*\)=rule$/\1/p'); do
    uci -q delete "firewall.$_ob_rule" || true
  done
  # 仅在确有变更时才 commit/reload——避免一次无意义的全 LAN 防火墙重载,也避免
  # 顺带提交用户在别处暂存的改动(与 openwrt/initd/jbox 的 jbox_cleanup 同一套顾虑)。
...
rm -f /etc/init.d/jbox /etc/init.d/jbox-panel
rm -f /www/luci-static/resources/view/jbox/status.js
# 目录本身也要删:留着一个空的 jbox/ 目录既不干净,也会让人误以为还装着。
# 用 rmdir 而不是 rm -rf——只在确实空了的时候删,避免误伤别人的东西。
rmdir /www/luci-static/resources/view/jbox 2>/dev/null || true
rm -f /usr/share/luci/menu.d/luci-app-jbox.json
rm -f /usr/share/rpcd/acl.d/luci-app-jbox.json
...
if [ "$PURGE" -eq 1 ]; then
  ...
  echo ""
  echo "J-Box 已卸载,数据保留在 $INSTALL_ROOT/data。"
