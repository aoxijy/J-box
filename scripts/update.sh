#!/bin/sh
# J-Box 升级脚本(POSIX sh,兼容 OpenWrt ash)。
#
# 用法:
...
#                                    # 输出被子进程重定向到 ${TMPDIR:-/tmp}/jbox-update.log。
...
# 覆盖掉真正一次升级正在写着的进度状态。
STATUS_PATH="${TMPDIR:-/tmp}/jbox-update.status"
CANCEL_FLAG="${TMPDIR:-/tmp}/jbox-update.cancel"
STATUS_PID=""

info() { echo "[j-box] $*"; }
warn() { echo "[j-box] 警告:$*" >&2; }
die() {
  echo "[j-box] 错误:$*" >&2
  write_status failed "" "" "$*"
  exit 1
...
# 本脚本随发布包铺到 /opt/j-box/update.sh(供 LuCI 兜底页一键升级调用)。升级
...
if [ "$_probe_or_cancel_scan" != "1" ] && [ "${JBOX_UPDATE_RELOCATED:-0}" != "1" ]; then
  case "$0" in
    "$INSTALL_ROOT"/*)
      _self_copy="/tmp/jbox-update.$$.sh"
      cp -f -- "$0" "$_self_copy" || die "无法把升级脚本复制到 /tmp,请改用:wget -O- <脚本地址> | sh"
      chmod +x "$_self_copy" 2>/dev/null || true
      JBOX_UPDATE_RELOCATED=1
      export JBOX_UPDATE_RELOCATED
      exec sh "$_self_copy" "$@"
      ;;
  esac
fi
...
# (releases/download/<tag>/j-box-<tag>-linux-<arch>.tar.gz):每个版本 URL 唯一,
...
EXPECT_VERSION="${JBOX_UPDATE_EXPECT:-}"
...
if [ "$_probe_or_cancel_scan" != "1" ]; then
  CHANNEL_OVERRIDE="${JBOX_UPDATE_CHANNEL_OVERRIDE:-}"
  case "$CHANNEL_OVERRIDE" in
    ''|direct|mirror) ;;
    *) CHANNEL_OVERRIDE="" ;;
  esac
  CLI_MIRROR_PREFIX="${JBOX_UPDATE_MIRROR_PREFIX:-}"
...
# 装好的 OpenWrt 上调用,ACL 已经把 exec 权限限制在 /opt/j-box/update.sh 这一份
# 装好的文件上。
#
...
if [ -n "$PROBE_CHANNEL" ]; then
  ...
  ASSET="j-box-linux-${ARCH}.tar.gz"
  ASSET_URL="https://github.com/$REPO/releases/latest/download/$ASSET"
  SHA_URL="$ASSET_URL.sha256"

  TMP_DL=$(mktemp -d "${TMPDIR:-/tmp}/j-box-probe.XXXXXX" 2>/dev/null) || {
    echo "fail 无法创建临时目录"
    exit 0
  }
...
# (JBOX_UPDATE_CHANNEL_OVERRIDE/JBOX_UPDATE_MIRROR_PREFIX,见下方真正派生
...
UPDATE_LOG="${TMPDIR:-/tmp}/jbox-update.log"
if [ "$DETACH" = "1" ]; then
  ...
  if command -v setsid >/dev/null 2>&1; then
    JBOX_UPDATE_CHANNEL_OVERRIDE="$CHANNEL_OVERRIDE" JBOX_UPDATE_MIRROR_PREFIX="$CLI_MIRROR_PREFIX" JBOX_UPDATE_EXPECT="$EXPECT_VERSION" JBOX_UPDATE_DISPATCHED=1 \
...
check_installed() {
  if [ ! -d "$INSTALL_ROOT" ] || [ ! -f "$INSTALL_ROOT/meta.json" ]; then
    die "未检测到现有 J-Box 安装($INSTALL_ROOT)。首次安装请使用 install.sh。"
  fi
}
...
[ "${JBOX_UPDATE_DISPATCHED:-0}" = "1" ] || rm -f "$CANCEL_FLAG" 2>/dev/null || true
...
if [ -n "$EXPECT_VERSION" ]; then
  ASSET="j-box-${EXPECT_VERSION}-linux-${ARCH}.tar.gz"
...
cleanup() {
  ...
  # 设这个 trap,不会跟这里冲突。
  if [ "${JBOX_UPDATE_RELOCATED:-0}" = "1" ]; then
    rm -f -- "$0"
  fi
  # 文件已经换过、面板还没拉起来就走到这里(比如铺 LuCI 文件时 die 了):无论如何把面板
  # 起来,用户至少还能进面板看到发生了什么;卡在"面板停着"是最糟的结局
  if [ "${POST_SWAP:-0}" = "1" ] && [ "${PANEL_STARTED:-0}" != "1" ] && [ -x /etc/init.d/jbox-panel ]; then
    /etc/init.d/jbox-panel start >/dev/null 2>&1 || true
  fi
  [ -n "${UPDATE_LOCK:-}" ] && rm -rf "$UPDATE_LOCK" 2>/dev/null
  return 0
}
TMP_DL=$(mktemp -d "${TMPDIR:-/tmp}/j-box-update.XXXXXX") || die "无法创建临时目录。"
...
CORE_WAS_RUNNING=0
if [ -x /etc/init.d/jbox ] && /etc/init.d/jbox status 2>/dev/null | grep -q running; then
  CORE_WAS_RUNNING=1
fi

info "停止服务..."
if [ -x /etc/init.d/jbox-panel ]; then
  /etc/init.d/jbox-panel stop >/dev/null 2>&1 || true
fi
if [ -x /etc/init.d/jbox ]; then
  # 会顺带触发 P5 的安全清理(摘掉指向旧内核的 DNS 接管、移除 v6 拦截),
  # 这正是升级窗口期间希望的状态:内核马上要被换掉,不该让残留的接管卡住 LAN 上网。
  /etc/init.d/jbox stop >/dev/null 2>&1 || true
fi
...
INITD_DIR="${JBOX_INITD_DIR:-/etc/init.d}"
SWAPPED_NEW=""
rollback_components() {
  _rb_failed=""
...
for _initd in jbox jbox-panel; do
  if [ -e "$INITD_DIR/$_initd" ]; then
    cp "$INITD_DIR/$_initd" "$STAGE_DIR/initd-backup/$_initd" || swap_failed "无法备份现有的 $INITD_DIR/$_initd。"
  fi
done
cp "$INSTALL_ROOT/openwrt/initd/jbox" "$INITD_DIR/jbox" || swap_failed "无法安装 $INITD_DIR/jbox。"
cp "$INSTALL_ROOT/openwrt/initd/jbox-panel" "$INITD_DIR/jbox-panel" || swap_failed "无法安装 $INITD_DIR/jbox-panel。"
chmod +x "$INITD_DIR/jbox" "$INITD_DIR/jbox-panel"
...
mkdir -p /www/luci-static/resources/view/jbox || warn "无法创建 LuCI 视图目录(不影响面板本身,LuCI 页面可能是旧的)。"
cp "$INSTALL_ROOT/openwrt/luci/htdocs/luci-static/resources/view/jbox/status.js" \
  /www/luci-static/resources/view/jbox/status.js || warn "无法安装 LuCI 视图文件(不影响面板本身,LuCI 页面可能是旧的)。"

mkdir -p /usr/share/luci/menu.d || warn "无法创建 LuCI 菜单目录(不影响面板本身,LuCI 页面可能是旧的)。"
cp "$INSTALL_ROOT/openwrt/luci/root/usr/share/luci/menu.d/luci-app-jbox.json" \
  /usr/share/luci/menu.d/luci-app-jbox.json || warn "无法安装 LuCI 菜单文件(不影响面板本身,LuCI 页面可能是旧的)。"
...
_ACL_SRC="$INSTALL_ROOT/openwrt/luci/root/usr/share/rpcd/acl.d/luci-app-jbox.json"
_ACL_DST=/usr/share/rpcd/acl.d/luci-app-jbox.json
_acl_changed=0
...
info "启动面板..."
/etc/init.d/jbox-panel enable || warn "设置面板开机自启失败,可稍后在 LuCI → 服务 → J-Box 中手动开启。"
/etc/init.d/jbox-panel start || warn "面板启动命令返回了非零状态,请稍后访问面板地址确认;如不可用可到 LuCI → 服务 → J-Box 中重试。"
PANEL_STARTED=1
...
if [ "$CORE_WAS_RUNNING" = "1" ]; then
  ...
  if [ -x "$INSTALL_ROOT/node/bin/node" ] && [ -f "$DEPLOY_CLI" ]; then
    ...
    if JBOX_ROOT="$INSTALL_ROOT" ZASHBOARD_DB_PATH="$INSTALL_ROOT/data/jbox.sqlite" \
