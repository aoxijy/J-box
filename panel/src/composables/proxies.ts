...
import { effectiveSiteSetOrder, managedOutbounds, siteSetNames } from '@/store/jboxSiteSets'
import { jboxSubscriptions } from '@/store/jboxSubscriptions'
...
const getRenderGroups = () => {
  ...
  if (proxiesTabShow.value === PROXY_TAB_TYPE.PROVIDER) {
    return jboxSubscriptions.value.map((sub) => sub.name)
  }
...
// 站点集 = 策略,其余带成员的组 = 节点组。名单从 J-Box 自己的档案来(见
// store/jboxSiteSets.ts);没拉到时退回 zashboard 原来按成员形状猜的那套。
