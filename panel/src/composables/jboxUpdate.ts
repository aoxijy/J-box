...
import {
  ...
  fetchUpdateStatus,
  runUpdate,
  type JBoxUpdateChannel,
  type JBoxUpdateStatus,
} from '@/api/jbox'
import { showNotification } from '@/helper/notification'
import { serverAuthenticated, serverPasswordSet } from '@/store/auth'
import { computed, ref, watch } from 'vue'

const ACTIVE_KEY = 'jbox/update-active'
...
export const updateInfo = ref<JBoxUpdateStatus | null>(null)
...
const finish = (stage: string, message: string) => {
  ...
  dismissed = false
  if (stage === 'done') {
    showNotification({ content: 'jbUpdateDone', type: 'alert-success' })
    window.setTimeout(() => window.location.reload(), 1500)
  } else if (stage === 'failed') {
    showNotification({ content: 'jbUpdateFailed', params: { message }, type: 'alert-error', timeout: 8000 })
  } else if (stage === 'cancelled') {
    showNotification({ content: 'jbUpdateCancelled', type: 'alert-warning' })
  }
}
...
export const startUpdate = async (channel: JBoxUpdateChannel) => {
  updateStarting.value = true
  try {
    await runUpdate(channel)
    showNotification({ content: 'jbUpdateStarted', type: 'alert-info' })
    polling = true
    wasRunning = true
    offline = 0
    dismissed = false
