...
import {
  fetchServiceStatus,
  runServiceAction,
  type JBoxServiceAction,
  type JBoxServiceStatus,
} from '@/api/jbox'
import { showNotification } from '@/helper/notification'
import { computed, onBeforeUnmount, onMounted, ref } from 'vue'
import { useI18n } from 'vue-i18n'

export const serviceStatus = ref<JBoxServiceStatus | null>(null)
export const pendingAction = ref<JBoxServiceAction | null>(null)

const ACTION_LABEL_KEYS: Record<JBoxServiceAction, string> = {
...
export const useKernelActions = () => {
  ...
  const runKernelAction = async (action: JBoxServiceAction) => {
  ...
  const EXPECTED: Partial<Record<JBoxServiceAction, (s: JBoxServiceStatus) => boolean>> = {
  ...
  const waitForState = async (action: JBoxServiceAction) => {
