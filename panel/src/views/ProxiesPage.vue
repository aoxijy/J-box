<template>
  <div class="flex h-full min-h-0 flex-col overflow-hidden">
    ...
    >
      ...
      <!-- 订阅标签渲染 J-Box 自己的订阅(见 store/jboxSubscriptions.ts 的说明:
           Clash 的 provider 概念在 J-Box 里不存在)。分列和策略 / 节点页签同一个设置(GitHub #33) -->
      <div
      ...
      >
        ...
        >
          <SubscriptionCard
            v-for="sub in filterContent(jboxSubscriptions, idx - 1)"
            :key="sub.id"
            :subscription="sub"
...
import { fetchServiceStatus, type JBoxSubscription } from '@/api/jbox'
...
import { refreshSubscription } from '@/api/jbox'
import { loadJBoxNodeGroups, loadJBoxSiteSets, nodeProviders } from '@/store/jboxSiteSets'
import { PROXY_TAB_TYPE, ROUTE_NAME, SETTINGS_TAB } from '@/constant'
import {
  loadJBoxSubscriptions,
  notifySubscriptionSaved,
  jboxSubscriptions,
} from '@/store/jboxSubscriptions'
...
// 订阅标签用的是 J-Box 自己的订阅列表(不是 Clash provider),进页面就拉一次。
onMounted(async () => {
  void loadJBoxSubscriptions()
  // 节点管理的数据要先到,fetchProxies 才能给条目配上图标;顺序反了图标就等下一次刷新
  await Promise.all([loadJBoxSiteSets(), loadJBoxNodeGroups()])
  void fetchProxies()
})
...
const editing = ref<JBoxSubscription | null>(null)
const requestEdit = (sub: JBoxSubscription) => {
  editing.value = sub
...
const columnItemCount = computed(() => {
  ...
  if (proxiesTabShow.value === PROXY_TAB_TYPE.PROVIDER) return jboxSubscriptions.value.length
