...
import { failoverDisplayName, failoverMembersOf, isFailoverGroup, isFailoverInternalTag } from '@/store/jboxFailover'
...
let jboxMetaPromise: Promise<unknown> | null = null
const ensureJBoxMeta = () => {
  if (siteSetIcons.value.size && managedOutbounds.value.length) return Promise.resolve()
  if (!jboxMetaPromise) {
    jboxMetaPromise = Promise.allSettled([loadJBoxSiteSets(), loadJBoxNodeGroups()]).finally(() => {
      jboxMetaPromise = null
    })
  }
  return jboxMetaPromise
}
