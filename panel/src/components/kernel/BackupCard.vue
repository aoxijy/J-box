...
import {
  ...
  refreshSubscription,
  type JBoxBackup,
  type JBoxBackupOptions,
  type JBoxBackupSubscriptionsMode,
  type JBoxProfile,
  type JBoxSubscription,
} from '@/api/jbox'
import { applyManagedStorageSnapshot } from '@/helper/persistentStorage'
import { loadJBoxSubscriptions, jboxSubscriptions } from '@/store/jboxSubscriptions'
...
const props = defineProps<{ profile: JBoxProfile | null }>()
const emit = defineEmits<{ imported: [] }>()
const { t } = useI18n()

// 导出时可勾可不勾的三块,默认都带
const EXPORT_OPTIONS: { key: keyof JBoxBackupOptions; label: string }[] = [
...
const include = reactive<JBoxBackupOptions>({ subscriptions: true, clientRoutes: true, servers: true })
// 这台路由器上有没有这三块:订阅看订阅列表,终端分流 / 共享网络看档案。没有的置灰、不勾
const available = computed<JBoxBackupOptions>(() => ({
  subscriptions: jboxSubscriptions.value.length > 0,
...
watch(
  ...
  () => {
    for (const key of Object.keys(include) as (keyof JBoxBackupOptions)[]) include[key] = available.value[key]
  },
  { immediate: true },
)
// 导入带面板设置的文件后会刷新页面让设置生效;刷新前把要弹的提示(文案 key + 参数)存进
// sessionStorage,回来再弹
const IMPORTED_FLAG = 'jbox/backup-imported'
type DoneToast = { content: string; params: Record<string, string> }
const refreshProgress = ref<{ done: number; total: number } | null>(null)
onMounted(() => {
  void loadJBoxSubscriptions()
  try {
...
const pending = ref<JBoxBackup | null>(null)
const pendingName = ref('')
// 订阅和节点:覆盖还是追加(其他配置不分,一律覆盖)
const subscriptionsMode = ref<JBoxBackupSubscriptionsMode>('replace')
...
// 拉一份 JSON,浏览器存成文件:j-box-backup-20260905-2130.json
const doExport = async () => {
  exporting.value = true
  try {
    const data = await fetchBackup({ ...include })
    const d = new Date()
    const name = `j-box-backup-${d.getFullYear()}${pad2(d.getMonth() + 1)}${pad2(d.getDate())}-${pad2(d.getHours())}${pad2(d.getMinutes())}.json`
...
const handleFile = (file: File) => {
  ...
  reader.onload = () => {
    try {
      const data = JSON.parse(String(reader.result)) as JBoxBackup
      if (!data || data.format !== 'j-box-backup' || typeof data.profile !== 'object') {
  ...
  try {
    ...
    const toRefresh =
      r.imported.subscriptions > 0 && Array.isArray(file.subscriptions)
        ? (file.subscriptions as Partial<JBoxSubscription>[]).filter((s) => s && typeof s.id === 'string' && ((Array.isArray(s.urls) && s.urls.length > 0) || Boolean(s.url)))
        : []
    if (toRefresh.length) {
