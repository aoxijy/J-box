...
import type { JBoxDnsRewriteRule, JBoxProfile } from '@/api/jbox'
import { fetchDnsRewriteDefaults } from '@/api/jbox'
...
const rules = computed<JBoxDnsRewriteRule[]>(() => props.profile.dns?.rewrite?.rules ?? [])
...
const targetText = (rule: JBoxDnsRewriteRule) => (rule.domain ? rule.domain : (rule.addresses ?? []).join(', '))
...
const needsRestart = (before: JBoxDnsRewriteRule[], after: JBoxDnsRewriteRule[]) => {
  const key = (list: JBoxDnsRewriteRule[]) => list.map((r) => `${r.id}\u0000${normalizeDomain(r.source)}\u0000${r.enabled !== false}`).sort().join('\n')
  return key(before) !== key(after)
}
// 整份规则表一次写回(数组是整体替换的,不会被默认值合并回来);服务端校验不过会报错回来
const persist = async (next: JBoxDnsRewriteRule[], enabled = rewriteEnabled.value) => {
  saving.value = true
...
const toggleRule = (rule: JBoxDnsRewriteRule, enabled: boolean) =>
...
const editing = ref<JBoxDnsRewriteRule | null>(null)
const draft = ref<Draft | null>(null)
const openEditor = (rule: JBoxDnsRewriteRule | null) => {
  editing.value = rule
  draft.value = rule
...
const pendingDelete = ref<JBoxDnsRewriteRule | null>(null)
const askDelete = (rule: JBoxDnsRewriteRule) => {
...
const restoreDefaults = async () => {
  ...
  let defaults: JBoxDnsRewriteRule[]
  try {
