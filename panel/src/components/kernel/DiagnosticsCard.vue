...
import { fetchDiagnostics } from '@/api/jbox'
...
// 和备份卡片同一套下载方式:拉 JSON,浏览器存成文件 j-box-diagnostics-20260908-1030.json
const doExport = async () => {
  ...
  try {
    ...
    const d = new Date()
    const name = `j-box-diagnostics-${d.getFullYear()}${pad2(d.getMonth() + 1)}${pad2(d.getDate())}-${pad2(d.getHours())}${pad2(d.getMinutes())}.json`
