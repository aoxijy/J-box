...
import type { JBoxCustomPolicy, JBoxCustomRule, JBoxProfile, JBoxRoutingPolicy, JBoxUserGroup } from '@/api/jbox'
import { fetchNodeGroups, importRuleList, RULESET_TAG_PATTERN } from '@/api/jbox'
...
const toggleEnabled = async (policy: JBoxRoutingPolicy) => {
...
const policies = computed<JBoxRoutingPolicy[]>(() => props.profile.routing.policies || [])
// 拖拽要求 v-model 绑一个 ref(vuedraggable 会整个替换数组),所以列表在本地存一份
const rows = ref<JBoxRoutingPolicy[]>([])
watch(
  policies,
  (value) => {
    rows.value = [...value]
...
const customPolicy = computed<JBoxCustomPolicy>(() => {
...
const outboundGroups = ref<JBoxUserGroup[]>([])
...
const editing = ref<JBoxRoutingPolicy | null>(null)
const draft = ref<JBoxRoutingPolicy | null>(null)
...
const openEditor = (policy: JBoxRoutingPolicy | null) => {
...
const persist = async (next: JBoxRoutingPolicy[]) => {
...
const saveDraft = async () => {
  ...
  if (editingCustom.value) {
    ...
    const customRules: JBoxCustomRule[] = kept.map((r) => ({
      type: r.type as JBoxCustomRule['type'],
      value: r.value.trim(),
...
const pendingDelete = ref<JBoxRoutingPolicy | null>(null)
const askDelete = (policy: JBoxRoutingPolicy) => {
