...
export default defineComponent({
  ...
  setup() {
    ...
    // 订阅标签下的「全部刷新」现在刷的是 J-Box 订阅,不再是 Clash provider——
    // J-Box 从不生成 provider,原来那个按钮点了永远是空转。
    const handlerClickUpdateAllProviders = async () => {
      if (isUpgrading.value) return
      isUpgrading.value = true
      try {
        await refreshAllJBoxSubscriptions()
        await fetchProxies()
      } finally {
        ...
        return {
          ...
          count:
            type === PROXY_TAB_TYPE.PROVIDER
              ? jboxSubscriptions.value.length
