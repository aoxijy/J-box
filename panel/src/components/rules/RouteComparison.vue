...
<script lang="ts">
import type { JBoxTerminalCapability } from '@/api/jbox'
// 设备能力只问一次(同一次会话里不会变),各个查询共用
let capabilityPromise: Promise<JBoxTerminalCapability> | null = null
</script>

<script setup lang="ts">
import type { JBoxDnsAssumption, JBoxPenetrationResult, JBoxRouteTest, JBoxRuleAssumption, JBoxTerminalEntry, JBoxTerminalMissing, JBoxTerminalTest } from '@/api/jbox'
import { fetchTerminalCapability, queryPenetration, testRoute, testTerminal } from '@/api/jbox'
...
// ---------- 规则路由(推算) ----------
const rule = ref<JBoxPenetrationResult | null>(null)
...
const capability = ref<JBoxTerminalCapability | null>(null)
const terminal = ref<JBoxTerminalTest | null>(null)
const actual = ref<JBoxRouteTest | null>(null)
...
const assumptionLine = (a: JBoxRuleAssumption) => {
  const r = a.rule
...
const dnsAssumptionLines = (list?: JBoxDnsAssumption[]) => {
  const line = (a: JBoxDnsAssumption) => {
...
const termBlock = computed<{ kind: 'incapable'; missing: JBoxTerminalMissing[] } | { kind: 'setup'; message: string } | null>(() => {
...
const reasonText = (e: JBoxTerminalEntry) => {
