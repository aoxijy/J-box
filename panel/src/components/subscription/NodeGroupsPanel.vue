...
import type { JBoxFailoverLane, JBoxFailoverSettings, JBoxGroupType, JBoxUserGroup } from '@/api/jbox'
import { fetchNodeGroups, saveNodeGroups } from '@/api/jbox'
...
const groups = ref<JBoxUserGroup[]>([])
...
const matchedNodes = (group: JBoxUserGroup) => {
...
const memberSummary = (group: JBoxUserGroup) => {
...
const editing = ref<JBoxUserGroup | null>(null)
const draft = ref<JBoxUserGroup | null>(null)
...
const openEditor = (group: JBoxUserGroup | null) => {
  editing.value = group
...
const FAILOVER_DEFAULTS: JBoxFailoverSettings = { timeoutMs: 5000, failureThreshold: 2, restorePrimary: true, recoveryHoldMs: 60000 }
...
const makeLane = (members: string[] = []): JBoxFailoverLane => ({ id: newLaneId(), name: '', icon: '', members })
// 故障转移草稿必须有的字段:页签(默认「主用」「备用 1」两个)、按秒的检测间隔、容差、高级参数
const ensureFailoverFields = (d: JBoxUserGroup) => {
  d.mode = 'static'
...
const validCount = (lane: JBoxFailoverLane) => lane.members.filter((m) => nodeNameSet.value.has(m)).length
const laneRoleLabel = (index: number) => (index === 0 ? t('failoverPrimary') : t('failoverBackupN', { n: index }))
const laneLabelOf = (lane: JBoxFailoverLane) => {
...
const pendingLane = ref<JBoxFailoverLane | null>(null)
...
const askDeleteLane = (lane: JBoxFailoverLane) => {
...
const lastType = ref<JBoxGroupType>('urltest')
const blockedMembers = ref<string[]>([])
let normalBackup: { mode: JBoxUserGroup['mode']; members: string[]; keywords: string[]; interval?: string } | null = null
let failoverBackup: { lanes: JBoxFailoverLane[] } | null = null
const onTypeChange = () => {
  const d = draft.value
  if (!d) return
...
const autoTypes = ref<JBoxGroupType[]>(['urltest'])
...
const AUTO_SUFFIX: Record<JBoxGroupType, string> = {
  urltest: '自动',
  selector: '手动',
...
const persist = async (next: JBoxUserGroup[]) => {
...
const saveDraft = async (typeChangeConfirmed = false) => {
  ...
  try {
    ...
    const base: JBoxUserGroup = { ...draft.value, name, id: draft.value.id || `g-${Date.now()}`, testUrl: kernelTestUrl(draft.value.testUrl) }
    let item: JBoxUserGroup
    if (isFailover.value) {
...
const toggleEnabled = async (group: JBoxUserGroup) => {
...
const builtinDraft = ref<JBoxUserGroup | null>(null)
const openBuiltinEditor = (group: JBoxUserGroup) => {
...
const pendingDelete = ref<JBoxUserGroup | null>(null)
const deleting = ref(false)

const askDelete = (group: JBoxUserGroup) => {
