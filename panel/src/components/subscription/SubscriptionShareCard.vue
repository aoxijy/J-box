...
import type { JBoxSubscription, JBoxSubscriptionShare } from '@/api/jbox'
import { createSubscriptionShare, deleteSubscriptionShare, regenerateSubscriptionShare, updateSubscriptionShare } from '@/api/jbox'
...
const props = defineProps<{ subscriptions: JBoxSubscription[]; shares: JBoxSubscriptionShare[] }>()
const emit = defineEmits<{ changed: [] }>()
const COLLAPSE_KEY = 'subscription-share'
const shareExpanded = computed(() => Boolean(collapseGroupMap.value[COLLAPSE_KEY]))
const dialogOpen = ref(false)
const editing = ref<JBoxSubscriptionShare | null>(null)
const generatedShare = ref<JBoxSubscriptionShare | null>(null)
const qrDataUrl = ref('')
...
const shareUrl = (share: JBoxSubscriptionShare) => `${share.protocol || currentProtocol()}://${share.host || currentHost()}/sub/${share.token}`
...
const selectedNames = (share: JBoxSubscriptionShare) => (Array.isArray(share.subscriptionIds) ? share.subscriptionIds : []).map((id) => props.subscriptions.find((s) => s.id === id)?.name || id)
...
const openEdit = (share: JBoxSubscriptionShare) => { reset(); editing.value = share; form.name = share.name; form.protocol = share.protocol || currentProtocol(); form.host = share.host || currentHost(); form.subscriptionIds = [...share.subscriptionIds]; dialogOpen.value = true }
const makeQr = async (share: JBoxSubscriptionShare) => { generatedShare.value = share; qrDataUrl.value = await QRCode.toDataURL(shareUrl(share), { margin: 1, width: 240 }) }
...
const toggle = async (share: JBoxSubscriptionShare) => { if (busy.value) return; busy.value = true; try { await updateSubscriptionShare(share.id, { name: share.name, host: share.host, protocol: (share.protocol || currentProtocol()) as 'http' | 'https', subscriptionIds: share.subscriptionIds, enabled: share.enabled === false }) ; emit('changed') } finally { busy.value = false } }
const regenerate = async (share: JBoxSubscriptionShare) => { if (busy.value) return; busy.value = true; try { await regenerateSubscriptionShare(share.id); emit('changed') } finally { busy.value = false } }
const remove = async (share: JBoxSubscriptionShare) => { if (busy.value || !window.confirm(`确定删除“${share.name}”？`)) return; busy.value = true; try { await deleteSubscriptionShare(share.id); emit('changed') } finally { busy.value = false } }
