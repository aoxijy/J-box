...
import type { JBoxRenameOptions, JBoxSubscription, JBoxSubscriptionAutoUpdate, JBoxSubscriptionPreview } from '@/api/jbox'
import { createSubscription, previewSubscription, updateSubscription } from '@/api/jbox'
import DialogWrapper from '@/components/common/DialogWrapper.vue'
import { PlusIcon, TrashIcon } from '@heroicons/vue/24/outline'
import { debounce } from 'lodash'
import { showNotification } from '@/helper/notification'
import { notifySubscriptionSaved } from '@/store/jboxSubscriptions'
...
const props = defineProps<{ subscription?: JBoxSubscription | null }>()
...
const hasUrlSource = (s?: JBoxSubscription | null) => Boolean(s?.url || s?.urls?.length)
...
const initialAutoUpdate = (): JBoxSubscriptionAutoUpdate => ({
...
const autoUpdate = ref<JBoxSubscriptionAutoUpdate>(initialAutoUpdate())

const renameOptions = ref<JBoxRenameOptions>({})
const handleRenameOptionsChange = (value: JBoxRenameOptions) => {
...
const effectiveRenameOptions = computed<JBoxRenameOptions>(() => ({
  ...renameOptions.value,
  overrides: overrides.value,
  disabled: disabledTags.value,
}))

const preview = ref<JBoxSubscriptionPreview | null>(null)
