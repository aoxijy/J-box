...
import { isFailoverInternalTag } from '@/store/jboxFailover'
...
const KIND_OF: Record<Tab, JBoxTrafficDim> = { clients: 'client', nodes: 'node', hosts: 'host' }
const DRILL_DIMS: Record<Tab, JBoxTrafficDim[]> = {
  clients: ['host', 'node'],
  nodes: ['client', 'host'],
  hosts: ['client', 'node'],
}
...
const error = ref('')
const monthData = ref<JBoxTrafficMonth | null>(null)
const month = ref('')
// 服务端的"今天"(路由器时区),别用浏览器的
const today = ref('')
const selectedDay = ref<string | null>(null)
const detail = ref<JBoxTrafficDay | null>(null)
...
const mergeFailoverNodeRows = (source: JBoxTrafficRow[]) => {
  if (!source.length) return source
  const merged = new Map<string, JBoxTrafficRow>()
