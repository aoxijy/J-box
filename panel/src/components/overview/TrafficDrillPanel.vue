...
const { t } = useI18n()
const by = ref<JBoxTrafficDim>(props.dims[0])
const cache = ref<Partial<Record<JBoxTrafficDim, JBoxTrafficDrill>>>({})
// 加载中 / 出错都按维度记:快速切页签时先发的请求回来不能把当前页签的 spinner 收掉或把错误贴过来
const loadingBy = ref<Partial<Record<JBoxTrafficDim, boolean>>>({})
const errorBy = ref<Partial<Record<JBoxTrafficDim, string>>>({})
...
const LABEL_KEY: Record<JBoxTrafficDim, string> = {
  client: 'trafficByClient',
  node: 'trafficByNode',
  host: 'trafficByHost',
}
const dimLabel = (d: JBoxTrafficDim) => t(LABEL_KEY[d])
...
const rows = computed<JBoxTrafficRow[]>(() => current.value?.rows ?? [])
...
type Entry = { type: 'row'; row: JBoxTrafficRow } | { type: 'rest' }
