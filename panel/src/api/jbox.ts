// Thin fetch wrappers around the J-Box backend (panel/server/api/{profile,subscriptions,deploy}.mjs).
...
export interface JBoxProfileRoutingCategory {
  ruleset: string
  target: string
}
...
export interface JBoxCustomRule {
...
export interface JBoxCustomPolicy {
  name?: string
  icon?: string
  iconScale?: number
  enabled?: boolean
  // 顺序即匹配顺序,先命中的先生效
  rules?: JBoxCustomRule[]
}

// 「出站」页签:每个站点集的 selector 里能选到哪几类东西
export interface JBoxOutboundOptions {
  direct?: boolean
  reject?: boolean
  groups?: boolean
}

export interface JBoxProfileRouting {
  proxyTag?: string
  ...
  regionId?: string
  regionMode?: string
  outboundOptions?: JBoxOutboundOptions
  // 前置自定义分流(固定置顶那一条)
  custom?: JBoxCustomPolicy
  policies?: JBoxRoutingPolicy[]
  adBlock?: boolean
  adRuleset?: string
  // 改版前的老字段,界面不再写;服务端读出来时会翻译成上面的新模型
  categories?: JBoxProfileRoutingCategory[]
  directRulesets?: string[]
  fallback?: string
}

// DNS 劫持方式:off 不碰 DNS / hijack 防火墙(nft)劫持 / dnsmasq 转发(默认)
export type JBoxDnsMode = 'off' | 'hijack' | 'dnsmasq'
// DNS 重写的一条规则(server/engine/dns-rewrite.mjs):源域名(精确或 *.泛域名)→ 目标域名(动态解析)或固定地址
export interface JBoxDnsRewriteRule {
  id: string
  enabled?: boolean
  source: string
  domain?: string
...
export interface JBoxProfileDns {
  ...
  rewrite?: { enabled?: boolean; initialized?: number; rules: JBoxDnsRewriteRule[] }
}
...
export const fetchDnsFilter = () => requestJson<DnsFilterStatus>('/api/jbox/dns-filter')
export const saveDnsFilter = (settings: DnsFilterSettings) => requestJson('/api/jbox/dns-filter', { method: 'PUT', body: JSON.stringify(settings) })
export const applyDnsFilter = (update = false, listId = '') => requestJson<DnsFilterStatus>('/api/jbox/dns-filter/apply', { method: 'POST', body: JSON.stringify({ update, ...(listId ? { listId } : {}) }) })
export const fetchDnsFilterSummary = () => requestJson<DnsFilterSummary>('/api/jbox/dns-filter/summary')
export const fetchDnsFilterRecords = (search: string, result: string, page: number, pageSize = 20) => requestJson<{ rows: DnsFilterRecord[]; total: number; page: number; pageSize: number }>(`/api/jbox/dns-filter/records?${new URLSearchParams({ search, result, page: String(page), pageSize: String(pageSize) })}`)
export const fetchDnsFilterPreview = (url: string, search: string, page: number, pageSize = 20, action: DnsFilterPreviewAction = 'all') => requestJson<DnsFilterPreview>(`/api/jbox/dns-filter/preview?${new URLSearchParams({ url, search, action, page: String(page), pageSize: String(pageSize) })}`)

// The backend deep-merges patches onto this shape (see server/store/jbox-store.mjs), so a
// profile is always fully populated — no field is ever missing on GET.
// 自动更新计划(面板进程内的定时器)
// channel 是自动更新走的通道;checkChannel 是卡片上手动检查 / 更新那个下拉框上次选的通道
export interface JBoxUpdatePlans {
  jbox?: { auto?: boolean; hour?: number; days?: number; channel?: 'auto' | 'direct' | 'mirror'; checkChannel?: 'auto' | 'direct' | 'mirror' }
}

// 「共享网络」里的一台服务器:本机开的一个入站(server/engine/servers.mjs)
// mixed:SOCKS5 + HTTP 共用一个端口,只给局域网用(server/engine/servers.mjs)
export type JBoxServerProtocol = 'shadowsocks' | 'vless' | 'tuic' | 'hysteria2' | 'mixed'
export interface JBoxServer {
  id: string
  enabled: boolean
  name: string
  protocol: JBoxServerProtocol
  port: number
...
export interface JBoxClientRoute {
  id: string
  enabled: boolean
  name: string
  sources: string[]
...
export interface JBoxProfile {
  updates?: JBoxUpdatePlans
  servers?: JBoxServer[]
  clientRoutes?: JBoxClientRoute[]
...
export interface JBoxProfileDefaults {
  region: string
  dns: JBoxProfileDns
  routing: JBoxProfileRouting
}
...
export interface JBoxRenameFeatureEntry {
  label: string
  keywords: string[]
}

export interface JBoxRenameOptions {
  // 重命名总开关:关掉后节点保留机场原始名字(手工改名、订阅名前缀照常),地区仍识别用于国旗和节点组;默认开
  enabled?: boolean
  regionDict?: JBoxRenameRegionEntry[]
  ...
  featureDict?: JBoxRenameFeatureEntry[]
  template?: string
  unknownLabel?: string
  seqPad?: number
}
...
export interface JBoxSubscriptionAutoUpdate {
  enabled: boolean
  days: number
  hour: number
}

export interface JBoxSubscription {
  id: string
  name: string
  url: string
  // 停用(false)的订阅节点不进内核,重启内核生效;没有这个字段 = 启用
  enabled?: boolean
  autoUpdate?: JBoxSubscriptionAutoUpdate | null
  ...
  format: string
  nodeCount: number
  renameOptions?: JBoxRenameOptions
  createdAt: number
  updatedAt: number
}

export interface JBoxSubscriptionShare {
  id: string
  name: string
  host: string
  protocol: 'http' | 'https' | ''
...
export interface JBoxNodeSummary {
  tag: string
  originalTag: string
  type: string
  server: string
...
export interface JBoxRenamePreviewEntry {
  originalTag: string
  newTag: string
...
export interface JBoxNodeGroup {
  name: string
  type: string
  nodeTags: string[]
}
...
export interface JBoxSkippedNode {
  name: string
  type: string
  reason?: 'unsupported-type' | 'unsupported-plugin' | 'invalid'
  detail?: string
}
export interface JBoxSubscriptionPreview {
  format: string
  nodes: JBoxNodeSummary[]
  skipped: JBoxSkippedNode[]
  // 被过滤关键词剔除的条目
  excluded?: Array<{ name: string }>
  // 被逐条禁用的条目
  disabled?: Array<{ name: string }>
  preview: JBoxRenamePreviewEntry[]
  groups: JBoxNodeGroup[]
}

export interface JBoxSubscriptionSaveResult {
  id: string
  name: string
  nodeCount: number
  skipped: JBoxSkippedNode[]
...
export interface JBoxDeployResult {
  ok: boolean
...
// GET /deploy/state persists across reloads (store/jbox-store.mjs's DEFAULT_DEPLOY_STATE),
// so 'idle' (never deployed) is a real value here even though a live deployNow() call itself
// never returns it.
export interface JBoxDeployState {
  stage: JBoxDeployResult['stage'] | 'idle'
  message: string
  at: number
  badTags: string[]
}
...
export interface JBoxConfigOutbound {
  type: string
  tag: string
  outbounds?: string[]
}

export interface JBoxConfigPreview {
  outbounds?: JBoxConfigOutbound[]
  [key: string]: unknown
}
...
export const fetchProfile = async (): Promise<JBoxProfile> => {
  const data = await requestJson<{ profile: JBoxProfile }>('/api/jbox/profile')
  return data.profile
}

// 「DNS 重写」的两条默认项(恢复默认用)
export const fetchDnsRewriteDefaults = async (): Promise<JBoxDnsRewriteRule[]> => {
  const data = await requestJson<{ dnsRewriteDefaults?: JBoxDnsRewriteRule[] }>('/api/jbox/profile/defaults?region=cn')
  return data.dnsRewriteDefaults ?? []
}

export const fetchProfileDefaults = async (region: string): Promise<JBoxProfileDefaults> => {
  const data = await requestJson<{ defaults: JBoxProfileDefaults }>(
    `/api/jbox/profile/defaults?region=${encodeURIComponent(region)}`,
  )
  return data.defaults
}

export const saveProfile = async (patch: Record<string, unknown>): Promise<JBoxProfile> => {
  const data = await requestJson<{ profile: JBoxProfile }>('/api/jbox/profile', {
    method: 'PUT',
...
export const fetchSubscriptions = async (): Promise<JBoxSubscription[]> => {
  const data = await requestJson<{ subscriptions: JBoxSubscription[] }>('/api/jbox/subscriptions')
  return data.subscriptions
}

export const fetchSubscriptionShares = async (): Promise<JBoxSubscriptionShare[]> => {
  const data = await requestJson<{ shares: JBoxSubscriptionShare[] }>('/api/jbox/subscription-shares')
  return Array.isArray(data.shares) ? data.shares : []
}

export const createSubscriptionShare = async (payload: { name: string; host: string; protocol: 'http' | 'https'; subscriptionIds: string[] }): Promise<JBoxSubscriptionShare> => {
  const data = await requestJson<{ share: JBoxSubscriptionShare }>('/api/jbox/subscription-shares', {
    method: 'POST',
...
export const updateSubscriptionShare = async (id: string, payload: { name: string; host: string; protocol: 'http' | 'https'; subscriptionIds: string[]; enabled?: boolean; regenerate?: boolean }): Promise<JBoxSubscriptionShare> => {
  const data = await requestJson<{ share: JBoxSubscriptionShare }>(`/api/jbox/subscription-shares/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
  })
  return data.share
...
export const regenerateSubscriptionShare = async (id: string): Promise<JBoxSubscriptionShare> => {
  const data = await requestJson<{ share: JBoxSubscriptionShare }>(`/api/jbox/subscription-shares/${encodeURIComponent(id)}/regenerate`, { method: 'POST' })
  return data.share
}

export const deleteSubscriptionShare = async (id: string): Promise<void> => {
  await requestJson(`/api/jbox/subscription-shares/${encodeURIComponent(id)}`, { method: 'DELETE' })
}
...
export const previewSubscription = async (payload: {
  ...
  name?: string
  renameOptions?: JBoxRenameOptions
}): Promise<JBoxSubscriptionPreview> => {
  return requestJson<JBoxSubscriptionPreview>('/api/jbox/subscriptions/preview', {
    method: 'POST',
    body: JSON.stringify(payload),
  })
}
...
export const createSubscription = async (payload: {
  ...
  name: string
  renameOptions?: JBoxRenameOptions
  autoUpdate?: JBoxSubscriptionAutoUpdate
}): Promise<JBoxSubscriptionSaveResult> => {
  return requestJson('/api/jbox/subscriptions', {
    method: 'POST',
    body: JSON.stringify(payload),
  })
}
...
export const updateSubscription = async (
  id: string,
  payload: { name?: string; url?: string; urls?: string[]; content?: string; renameOptions?: JBoxRenameOptions; autoUpdate?: JBoxSubscriptionAutoUpdate; enabled?: boolean },
): Promise<JBoxSubscriptionSaveResult> => {
  return requestJson(`/api/jbox/subscriptions/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
  })
}

export interface JBoxLatencyResult {
  ok: boolean
  ms?: number
  error?: string
}
...
export const testNodeLatency = async (payload: {
  ...
  tags: string[]
  timeoutMs?: number
}): Promise<JBoxLatencyResult[]> => {
  const data = await requestJson<{ results: JBoxLatencyResult[] }>('/api/jbox/nodes/latency', {
    method: 'POST',
    body: JSON.stringify(payload),
  })
  return data.results
...
export type JBoxGroupType = 'urltest' | 'selector' | 'failover'

// 故障转移的主备页签:顺序即优先级(第一个主用,后面依次备用),id 稳定(拖拽、保存、运行状态都按它认),
// name 可选(不决定主备顺序),members 只放真实节点名
export interface JBoxFailoverLane {
  id: string
  name: string
...
export interface JBoxFailoverSettings {
  // 单次节点端到端探测的等待上限
  timeoutMs: number
  // 当前页签连续几轮确认没有可用节点才转移
...
// 名字不用 JBoxNodeGroup:那个已经被"按地区自动切分的组"占了(见上方,形状是
...
export type JBoxGroupMode = 'static' | 'dynamic'

export interface JBoxUserGroup {
  id: string
  name: string
  type: JBoxGroupType
  mode: JBoxGroupMode
  ...
  lanes?: JBoxFailoverLane[]
  failover?: JBoxFailoverSettings
}

// 故障转移组的运行状态(服务端 system/failover-manager.mjs 维护)
export type JBoxFailoverLaneHealth = 'up' | 'down' | 'unknown'
export interface JBoxFailoverLaneStatus {
  id: string
  name: string
  index: number
  role: string
...
export interface JBoxFailoverGroupStatus {
  id: string
  tag: string
  ...
  inFlight: boolean
  settings: { interval?: string; intervalMs?: number; tolerance?: number; testUrl?: string } & Partial<JBoxFailoverSettings>
  lanes: JBoxFailoverLaneStatus[]
}
export interface JBoxFailoverStatus {
  version: string | null
  paused: string
  lastError?: string
  running?: boolean
  groups: JBoxFailoverGroupStatus[]
}
export const fetchFailoverStatus = async (): Promise<JBoxFailoverStatus> =>
  requestJson<JBoxFailoverStatus>('/api/jbox/failover/status')
export const refreshFailover = async (): Promise<void> => {
  await requestJson<{ ok: boolean }>('/api/jbox/failover/refresh', { method: 'POST' })
}

export interface JBoxGroupsPayload {
  groups: JBoxUserGroup[]
  types: JBoxGroupType[]
...
export const fetchNodeGroups = async (): Promise<JBoxGroupsPayload> =>
  requestJson<JBoxGroupsPayload>('/api/jbox/groups')

// 规则集「详情」:一个 geosite/geoip 分类里到底有哪些域名/IP。
// 服务端把 .srs 交给内核自己解码(sing-box rule-set decompile),所以看到的就是
// 内核会匹配的那份;本地没有的分类会现下一份。
export interface JBoxRulesetEntries {
...
): Promise<JBoxRulesetEntries> => {
  const params = new URLSearchParams({ url, offset: String(offset), limit: String(limit) })
  if (q) params.set('q', q)
  return requestJson<JBoxRulesetEntries>(`/api/jbox/rulesets/preview?${params.toString()}`)
}

// 导入规则:预览时一次返回规则集解析后的全部域名 / IP,保存站点集后即不再依赖原 URL。
export const importRuleList = async (url: string): Promise<JBoxRulesetEntries> => {
  const params = new URLSearchParams({ url })
  return requestJson<JBoxRulesetEntries>(`/api/jbox/rulesets/import?${params.toString()}`)
}
...
export const fetchRulesetEntries = async (
  ...
  if (q) params.set('q', q)
  return requestJson<JBoxRulesetEntries>(`/api/jbox/rulesets/entries?${params.toString()}`)
}
...
export const saveNodeGroups = async (
  ...
  requestJson('/api/jbox/groups', { method: 'PUT', body: JSON.stringify({ groups }) })

// 拖拽排序:传全部订阅 id 的新顺序;服务端把节点池也按这个顺序重排(选择器、内核出站顺序都跟着)
export const reorderSubscriptions = async (ids: string[]): Promise<{ ok: boolean; subscriptions: JBoxSubscription[]; changed?: boolean }> => {
  return requestJson('/api/jbox/subscriptions/order', { method: 'PUT', body: JSON.stringify({ ids }) })
}
...
export const refreshSubscription = async (
  ...
  return requestJson(`/api/jbox/subscriptions/${encodeURIComponent(id)}/refresh`, {
    method: 'POST',
...
export const deployNow = async (): Promise<JBoxDeployResult> => {
  const response = await fetchServerApi('/api/jbox/deploy', {
    method: 'POST',
    headers: { Accept: 'application/json' },
  })

  const data = (await response.json().catch(() => null)) as Partial<JBoxDeployResult> | null
...
export const fetchDeployState = async (): Promise<JBoxDeployState> => {
  const data = await requestJson<{ state: JBoxDeployState }>('/api/jbox/deploy/state')
  return data.state
}
...
export const fetchConfigPreview = async (): Promise<JBoxConfigPreview> => {
  const data = await requestJson<{ config: JBoxConfigPreview }>('/api/jbox/config/preview')
  return data.config
}
...
export interface JBoxServiceInfo {
  running: boolean
  raw: string
  // 只有内核那份带:开机自启是否开着
  autostart?: boolean
...
export interface JBoxConflictService {
  id: string
  label: string
  running: boolean
}

export interface JBoxServiceStatus {
  core: JBoxServiceInfo
  panel: JBoxServiceInfo
  conflicts: JBoxConflictService[]
}

export type JBoxServiceAction = 'start' | 'stop' | 'restart' | 'enable' | 'disable'

export interface JBoxServiceActionResult {
...
export interface JBoxKernelVersion {
  version: string
  raw: string
...
export const fetchServiceStatus = async (): Promise<JBoxServiceStatus> => {
  return requestJson<JBoxServiceStatus>('/api/jbox/service/status')
}
...
export const runServiceAction = async (action: JBoxServiceAction): Promise<JBoxServiceActionResult> => {
  return requestJson<JBoxServiceActionResult>(`/api/jbox/service/core/${action}`, { method: 'POST' })
}

export const fetchKernelVersion = async (): Promise<JBoxKernelVersion> => {
  return requestJson<JBoxKernelVersion>('/api/jbox/kernel/version')
}
...
export interface JBoxPenetrationRuleCondition {
...
export interface JBoxPenetrationMatched {
  index: number
  rule: JBoxPenetrationRuleCondition
  outbound?: string
  action?: string
...
export interface JBoxRuleAssumption {
  index: number
...
export interface JBoxDnsAssumption {
  ruleIndex: number
  needs: string[]
  sourceIpCidr: string[]
  server?: string
...
export interface JBoxDnsRewriteHit {
  source: string
  domain: string
  addresses: string[]
}

export interface JBoxPenetrationResult {
  matched: JBoxPenetrationMatched | null
  // 要看终端来源 IP / 目标端口 / 地址族才能判、这次查询没给的规则:推算按"不满足它"的情况继续,这里把前提列出来
  assumed?: JBoxRuleAssumption[]
  ...
  dns?:
    | { skipped: true }
    | { error: string }
    | { ruleIndex: number | null; rejected?: boolean; server?: { tag: string; type?: string; server?: string; detour?: string }; viaProxy?: boolean; assumed?: JBoxDnsAssumption[]; rewrite?: JBoxDnsRewriteHit }
...
export const queryPenetration = async (target: string): Promise<JBoxPenetrationResult> => {
  return requestJson<JBoxPenetrationResult>('/api/jbox/penetration', {
    method: 'POST',
...
export type JBoxPolicyEntryFamily = 'domain' | 'ip' | 'other'
export interface JBoxPolicyEntry {
  type: string
  family: JBoxPolicyEntryFamily
  content: string
  // 来源:规则集名,或 'custom'(站点集里手写的条件)
  source: string
}
export interface JBoxPolicyEntries {
  name: string
  fallback: boolean
...
export const fetchPolicyEntries = async (params: {
  ...
  offset?: number
  limit?: number
}): Promise<JBoxPolicyEntries> => {
  const search = new URLSearchParams()
  for (const [k, v] of Object.entries(params)) {
    if (v !== undefined && v !== '' && v !== null) search.set(k, String(v))
  }
  return requestJson<JBoxPolicyEntries>(`/api/jbox/policies/entries?${search.toString()}`)
}

// ---- J-Box 自身升级 / Geo 规则集刷新 ----
export interface JBoxUpdateProgress {
  stage: string
  pid?: string
  bytes: number | null
  total: number | null
...
export interface JBoxUpdateStatus {
  version: string
  singboxVersion: string
  builtAt: string
  channel: { mode: 'direct' | 'mirror'; prefix: string }
  status: JBoxUpdateProgress
  logTail: string
}
export const fetchUpdateStatus = () => requestJson<JBoxUpdateStatus>('/api/jbox/update/status')
export const checkUpdate = () =>
  requestJson<{ current: string; latest: string; via: string; hasUpdate: boolean }>('/api/jbox/update/check')
export const runUpdate = (channel: 'auto' | 'direct' | 'mirror') =>
  requestJson<{ ok: boolean; output: string }>('/api/jbox/update/run', { method: 'POST', body: JSON.stringify({ channel }) })
export const cancelUpdate = () => requestJson<{ result: string }>('/api/jbox/update/cancel', { method: 'POST' })

export type JBoxUpdateChannel = 'auto' | 'direct' | 'mirror'
// 「真实路由」:DNS 决策 + 内核解析 + 真实访问一次并从连接表里读实际链路
export interface JBoxRouteTest {
  target: string
  dns:
    | { skipped: true }
    | { error: string }
    ...
    | { ruleIndex: number | null; rejected?: boolean; server?: { tag: string; type?: string; server?: string; detour?: string }; viaProxy?: boolean; stale?: 'direct' | 'proxy'; runtimeLeaf?: string; runtimeChain?: string[]; fakeIpRule?: number; assumed?: JBoxDnsAssumption[]; rewrite?: JBoxDnsRewriteHit }
...
export const testRoute = (target: string, port?: number) =>
  requestJson<JBoxRouteTest>('/api/jbox/route-test', { method: 'POST', body: JSON.stringify({ target, port }) })
...
export type JBoxTerminalMissing = 'root' | 'netns' | 'veth' | 'lan' | 'conntrack' | 'dhcp'
export interface JBoxTerminalCapability {
  ok: boolean
  missing: JBoxTerminalMissing[]
  lan?: { device: string; address: string; mask: number | null } | null
}
export interface JBoxTerminalEntry {
...
export type JBoxTerminalKernelDnsResult = 'exchanged' | 'cached' | 'optimistic' | 'failed' | 'rejected' | 'action' | 'pending'
export type JBoxTerminalKernelDns =
  ...
  | {
    ...
    outbound: string
    chain: string[]
    result: JBoxTerminalKernelDnsResult
    rcode: string
    ttl: number | null
    answers: string[]
    ms: number | null
    ...
    fakeIpLocal: boolean
    v6?: { result: JBoxTerminalKernelDnsResult; rcode: string; answers: string[]; ms: number | null; error: string }
  }
export interface JBoxTerminalTest {
  target: string
  mode: 'lan'
  capable: boolean
  missing?: JBoxTerminalMissing[]
  ...
  kernelDns?: JBoxTerminalKernelDns
  entry?: JBoxTerminalEntry
...
export type JBoxLatencySample = { time: string; delay: number; node?: string }
export type JBoxLatencyHistory = Record<string, JBoxLatencySample[]>
export const fetchLatencyHistory = () => requestJson<{ history: JBoxLatencyHistory; updatedAt: number }>('/api/jbox/latency-history')
// 只回最近一次写入的时刻:代理页每 15 秒轮询它,变了才拉整份
export const fetchLatencyHistoryVersion = () => requestJson<{ updatedAt: number }>('/api/jbox/latency-history/version')
// 面板手动测出来的超时:内核那边只是删记录,得自己报上去
export const postLatencySamples = (samples: Array<{ name: string; time: string; delay: number }>) =>
  requestJson<{ ok: boolean; history: JBoxLatencyHistory }>('/api/jbox/latency-history/samples', { method: 'POST', body: JSON.stringify({ samples }) })
// 让服务端立刻读一次内核把新结果记下来(手动测完后调,不用等它下一个 tick)
export const syncLatencyHistory = () => requestJson<{ history: JBoxLatencyHistory }>('/api/jbox/latency-history/sync', { method: 'POST', body: '{}' })

// ---- 每日流量(server/api/traffic.mjs)。up = 发往外网的字节(出口),down = 收到的(入口)
export interface JBoxTrafficRow {
  key: string
  up: number
  down: number
  conns: number
...
export interface JBoxTrafficDaySummary {
  day: string
  up: number
  down: number
  conns: number
}
export interface JBoxTrafficMonth {
  month: string
  today: string
  days: JBoxTrafficDaySummary[]
...
export interface JBoxTrafficDay {
  day: string
  today: string
  total: { up: number; down: number; conns: number }
  nodes: JBoxTrafficRow[]
  hosts: JBoxTrafficRow[]
  clients: JBoxTrafficRow[]
  hostsCount: number
  clientsCount: number
  // 总量减去各节点之和:没采样到的短连接
  other: { up: number; down: number }
  // 24 小时曲线:每小时的进站 / 出站字节数(和总量同源)
  hours: JBoxTrafficHour[]
...
export interface JBoxTrafficHour {
  hour: number
  up: number
  down: number
  conns?: number
...
export interface JBoxTrafficUsage {
  rows: number
  days: number
  bytes: number
...
export const fetchTrafficUsage = () =>
  requestJson<JBoxTrafficUsage>('/api/jbox/traffic/usage')
...
export const fetchTrafficMonth = (month?: string, countDirect = true) =>
  requestJson<JBoxTrafficMonth>(`/api/jbox/traffic/month?${month ? `month=${encodeURIComponent(month)}` : ''}${directParam(countDirect)}`)
// hour 给了就只看那个小时的明细(0~23),不给是整天
export const fetchTrafficDay = (day: string, limit = 500, hour?: number | null, countDirect = true) =>
  requestJson<JBoxTrafficDay>(
    `/api/jbox/traffic/day?day=${encodeURIComponent(day)}&limit=${limit}${hour === null || hour === undefined ? '' : `&hour=${hour}`}${directParam(countDirect)}`,
  )
// 一条记录的构成:kind/key 定位点开的那条(终端 IP / 节点名 / 域名),by 是拆成哪一维
export type JBoxTrafficDim = 'client' | 'node' | 'host'
export interface JBoxTrafficDrill {
  day: string
  hour?: number | null
  kind: JBoxTrafficDim
  key: string
...
export const fetchTrafficDrill = (
  ...
  requestJson<JBoxTrafficDrill>(
    `/api/jbox/traffic/drill?day=${encodeURIComponent(day)}&kind=${kind}&key=${encodeURIComponent(key)}&by=${by}&limit=${limit}${hour === null || hour === undefined ? '' : `&hour=${hour}`}${directParam(countDirect)}`,
  )

// 导出 / 导入(server/api/backup.mjs):档案 + 节点组,可选订阅和节点
export interface JBoxBackup {
  format: string
  version: number
  exportedAt: string
  jboxVersion?: string
...
export interface JBoxBackupOptions {
  subscriptions: boolean
  clientRoutes: boolean
  servers: boolean
}
export type JBoxBackupSubscriptionsMode = 'replace' | 'append'
export interface JBoxBackupImportResult {
  ok: boolean
  imported: {
    profile: boolean
    groups: number
    ...
    nodes: number
    subscriptionsMode: JBoxBackupSubscriptionsMode | null
    panelSettings?: number
    backgroundImage?: boolean
  }
}
// 诊断包(server/api/diagnostics.mjs):版本、固件、内核状态、脱敏配置、最近日志,给 issue 用
export const fetchDiagnostics = () => requestJson<Record<string, unknown>>('/api/jbox/diagnostics')

export const fetchBackup = (opts: JBoxBackupOptions) =>
  requestJson<JBoxBackup>(
    `/api/jbox/backup?subscriptions=${opts.subscriptions ? 1 : 0}&clientRoutes=${opts.clientRoutes ? 1 : 0}&servers=${opts.servers ? 1 : 0}`,
  )
export const importBackup = (data: JBoxBackup, subscriptionsMode: JBoxBackupSubscriptionsMode = 'replace') =>
  requestJson<JBoxBackupImportResult>(`/api/jbox/backup/import?subscriptions=${subscriptionsMode}`, {
    method: 'POST',
    body: JSON.stringify(data),
  })

// 共享网络 · 保存前的端口检测(server/api/servers.mjs)
export interface JBoxPortCheck {
  ok: boolean
  reason?: 'invalid' | 'reserved' | 'server' | 'listening'
  name?: string
}
export const checkServerPort = (port: number, id: string) =>
  requestJson<JBoxPortCheck>(`/api/jbox/servers/port-check?port=${port}&id=${encodeURIComponent(id)}`)

// 终端分流选来源用:DHCP 租约里的设备 + 今天流量里出现过的来源 IP
export const fetchKnownClients = () => requestJson<{ clients: Array<{ ip: string; name: string; mac?: string }> }>('/api/jbox/clients')
