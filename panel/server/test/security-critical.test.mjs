...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'jbox-security-critical-test-'))
...
const ensurePasswordConfigured = async () => {
  ...
  const match = setCookie.match(/jbox_access_session=[^;]+/)
...
// 评审实测:已设密状态下,GET /API/jbox/profile(大写)不进守卫、却仍匹配路由,
// 未认证客户端可读到 /API/storage 里的明文密码、可跑 POST /api/jbox/deploy 真实部署。
// 修复后这四种变体在"已设密 + 未带 cookie"时都必须被守卫拦在 401,而不是命中真实 handler。

test('Critical 1 回归:已设密未带 cookie 时 GET /API/jbox/profile(全大写)必须是 401,不能是 200', async () => {
...
test('Critical 1 回归:已设密未带 cookie 时 GET /Api/JBox/Profile(混合大小写)必须是 401,不能是 200', async () => {
  await ensurePasswordConfigured()
  const res = await fetch(`${baseUrl}/Api/JBox/Profile`)
  assert.notEqual(res.status, 200)
  assert.equal(res.status, 401)
...
test('Critical 1 回归:已设密未带 cookie 时 GET //api/jbox/profile(双斜杠)必须是 401,不能是 200', async () => {
  await ensurePasswordConfigured()
  const res = await fetch(`${baseUrl}//api/jbox/profile`)
  assert.notEqual(res.status, 200)
  assert.equal(res.status, 401)
...
test('Critical 1 回归:已设密未带 cookie 时 POST /API/jbox/deploy(大写)不得真正触发部署', async () => {
  await ensurePasswordConfigured()
...
test('健全性检查:小写 + 正确 cookie 的 GET /api/jbox/profile 仍然是 200(未过度拦截)', async () => {
  await ensurePasswordConfigured()
  const res = await fetch(`${baseUrl}/api/jbox/profile`, { headers: { cookie: authCookie } })
  assert.equal(res.status, 200)
  const body = await res.json()
  assert.ok(body.profile)
})

// ==================== Critical 2:PUT /api/storage 抹掉 J-Box 状态与密码 ====================

test('isProtectedStorageKey:jbox/*、config/access-*、背景图 sentinel 判定为受保护', () => {
  assert.equal(isProtectedStorageKey('jbox/profile'), true)
  assert.equal(isProtectedStorageKey('jbox/clash-secret'), true)
...
test('Critical 2 回归:PUT /api/storage 传入不含 jbox/* 的快照,已存 profile/订阅/clash-secret 仍在且值不变', async () => {
  await ensurePasswordConfigured()
...
test('Critical 2 回归:GET /api/storage 的返回不含任何 config/access-password* 与 jbox/* 键', async () => {
  await ensurePasswordConfigured()
  storeForTesting.setProfile({ region: 'US' }) // 确保 jbox/profile 键确实存在于表里
  ...
  assert.ok(!keys.some((k) => k.startsWith('jbox/')), `不应含 jbox/* 键,实际: ${keys.join(',')}`)
...
test('Critical 2 回归:PUT /api/storage 试图写 jbox/profile → 被服务端忽略,真实 profile 不受影响', async () => {
  await ensurePasswordConfigured()
  ...
  const res = await fetch(`${baseUrl}/api/storage`, {
    method: 'PUT',
    headers: { 'content-type': 'application/json', cookie: authCookie },
    body: JSON.stringify({ entries: { 'jbox/profile': JSON.stringify({ region: 'EVIL' }) } }),
  })
  assert.equal(res.status, 200)
