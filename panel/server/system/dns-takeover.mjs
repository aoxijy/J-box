...
export const DNS_FORWARD_CONF_NAME = 'j-box.conf'
...
// 基线 → 备份文件正文(uci show 形态,段名沿用 uci 里的),init 脚本的 jbox_cleanup 也按这个格式读
...
export const restoreDnsTakeover = async (ctx, paths) => {
  ...
  try {
    if (hasBackup) {
      // 有备份 = J-Box 确实接管过 dnsmasq:整段清空后按备份重建,恢复到接管前状态
      await ctx.exec('uci', ['-q', 'delete', 'dhcp.@dnsmasq[0].server'])
      ...
      // 一并清空并 commit 进闪存。只精确撤销 J-Box 可能写入的那一条,幂等无害。
