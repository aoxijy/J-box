...
const fixture = t => {
  ...
  fs.mkdirSync(assets)
  write(source, 'meta.json', JSON.stringify({ version: 'v0.2.0', arch: 'x64', singboxVersion: '1.14.0-jbox-tcp3', nodeVersion: '24.18.0' }))
  ...
  write(source, 'bin/sing-box.BUILD-INFO.json', JSON.stringify({ version: '1.14.0-jbox-tcp3', jbox_commit: 'new' }))
...
for (const changed of [[], ['geo'], ['kernel'], ['geo', 'kernel']]) {
  test(`真实升级 shell 按需下载并组合：变化 ${changed.join('+') || '无'}，全部来自自己的 Release`, t => {
    ...
    write(local, 'bin/sing-box.BUILD-INFO.json', JSON.stringify({ version: '1.14.0-jbox-tcp3', jbox_commit: 'old' }))
