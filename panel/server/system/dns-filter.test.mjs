...
test('native 1.14 DNS filters, exceptions, AAAA, regex, rewrite priority and observable records', { skip: !available, timeout: 20000 }, async (t) => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'jbox-dns-filter-test-'))
  ...
  const previous = raw.get('jbox/dns-filter-lists')
  await prepareDnsFilter({ store, ctx, paths, force: true, fetchImpl: async () => { throw new Error('offline') } })
  assert.equal(JSON.parse(raw.get('jbox/dns-filter-lists')).list.hash, JSON.parse(previous).list.hash)
  assert.match(JSON.parse(raw.get('jbox/dns-filter-lists')).list.error, /offline/)
  await prepareDnsFilter({ store, ctx, paths, force: true, fetchImpl: async () => new Response('/(invalid/') })
  assert.equal(JSON.parse(raw.get('jbox/dns-filter-lists')).list.hash, JSON.parse(previous).list.hash)
  assert.match(JSON.parse(raw.get('jbox/dns-filter-lists')).list.error, /正则无效/)
