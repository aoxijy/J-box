...
test('ensureTlsKeypair:没证书就调 sing-box 生成并落盘;已有就不动', async () => {
  const ctx = createMockContext({ execResults: { [`${paths.singbox} generate tls-keypair j-box.local --months 120`]: { code: 0, stdout: PEM } } })
  ...
  const bad = createMockContext({ execResults: { [`${paths.singbox} generate tls-keypair j-box.local --months 120`]: { code: 1, stderr: 'boom' } } })
