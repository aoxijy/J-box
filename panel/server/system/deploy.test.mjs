...
test('内核二进制缺失 → 重启前预检拦截,精确归因而不依赖 procd 吞掉的退出码', async () => {
  ...
  const c = cmds(ctx)
  assert.ok(!c.includes('/etc/init.d/jbox restart'), '二进制缺失时不应尝试重启内核')
  assert.ok(c.includes('/etc/init.d/jbox stop'), '预检失败也要回滚到直连')
})
...
test('重启失败 → 回滚恢复直连', async () => {
  ...
  assert.ok(c.includes('uci -q delete firewall.jbox_v6block'))  // 撤代理规则(而非面板放行)
})
...
test('落盘之后阶段抛出异常 → 回滚到直连并返回 stage:error', async () => {
  ...
  assert.ok(cmds(ctx).includes('/etc/init.d/jbox stop'))    // 回滚:停服务
})
...
test('重启失败且回滚也没成 → 提示写明恢复直连未完成、哪一步、为什么;不再笼统说"已恢复直连"', async () => {
  ...
  // 回滚全成功时照旧说"已恢复直连"
  const fine = await deployConfig(createMockContext({ files: { [paths.singbox]: '#!/bin/sh\n', [TUN_DEVICE]: '' }, execResults: { '/etc/init.d/jbox restart': { code: 1, stderr: 'start failed' } } }), paths, { config, profile })
...
// "open /opt/j-box/data/rulesets/geosite-cn.srs: no such file or directory"。
...
const configWithRulesets = {
  ...
  route: {
    ...
    rule_set: [{
      ...
      path: '/opt/j-box/data/rulesets/geosite-cn.srs',
    }],
  },
}
...
test('规则集已存在时不再下载(GitHub 连不上也能照常部署)', async () => {
  ...
  // 旧来源标记不参与选择，始终使用包内数据。
  ctx.files['/opt/j-box/data/rulesets/.source'] = 'metacubex\n'
  let called = false
...
test('isCancelled 在落盘前为真 → 直接退出、什么都不动;在 DNS / 防火墙之后为真 → 回滚到直连;内核起了之后为真 → 不报成功也不回滚', async () => {
  ...
  assert.ok(!cmds(early).includes('/etc/init.d/jbox restart'))
  ...
  assert.ok(!cmds(mid).includes('/etc/init.d/jbox restart'))
  assert.ok(cmds(mid).includes('/etc/init.d/jbox stop'))
  // 3) 内核已起(restart 之后)才取消:不报成功、不回滚,留给随后的停止动作
  let restarted = false
  const late = okCtx({ '/etc/init.d/jbox restart': () => { restarted = true; return { code: 0 } } })
  const r3 = await deployConfig(late, paths, { config, profile, isCancelled: () => restarted })
  assert.equal(r3.stage, 'cancelled')
  assert.equal(r3.ok, false)
  assert.ok(!cmds(late).includes('/etc/init.d/jbox stop'))
})
...
test('tun 设备不存在:先 modprobe,还没有就明说要装 kmod-tun 并回滚直连', async () => {
  ...
  assert.ok(!c.some((x) => /jbox restart/.test(x)), '没有 tun 设备就不该去重启内核')
...
test('auto_redirect 在 nftables 层起不来 → 关掉 auto_redirect 重新生成配置再起一次,成功但带降级说明', async () => {
  ...
  // 起了两次,没有回滚直连
  assert.equal(cmds(ctx).filter((c) => c === '/etc/init.d/jbox restart').length, 2)
  assert.ok(!cmds(ctx).includes('/etc/init.d/jbox stop'))
})
...
test('确认在跑之后才崩、且是 auto_redirect 那类 → 后台盯到后降级重来一次,降级说明从 lateCrashWatch 回来', async () => {
  ...
  assert.equal(cmds(ctx).filter((c) => c === '/etc/init.d/jbox restart').length, 1)
  state.crashed = true
...
test('部署时把走代理站点集的 geosite 解码进转发名单:元数据记实际 domains、条目数和超集说明;解不开就 all 并说明(第三轮 阶段 2)', async () => {
  ...
  assert.ok(ctx.writes.some((w) => w.path === '/tmp/dnsmasq.cfg.d/j-box.conf' && /server=\/youtube\.com\/127\.0\.0\.1#7853/.test(w.content)))
