...
test('真实 Clash API：HTTP 指定地址、手动选择、自动优选、故障切换及恢复', {
  skip: !fs.existsSync(binary) && !inCI ? 'Set JBOX_TEST_SINGBOX to the newly built bundled core' : false,
  timeout: 45_000,
}, async (t) => {
  ...
  const url = `http://${destination}/probe-204?source=jbox`
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'jbox-http-latency-'))
  ...
  await t.test('直连与单节点：原样发送 HTTP HEAD，经过指定节点，超时仍失败', async () => {
    ...
    assert.ok(requests.length && requests.every((r) => r.method === 'HEAD' && r.url === '/probe-204?source=jbox'))
