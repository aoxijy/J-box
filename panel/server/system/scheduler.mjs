...
// 程序、内核、Geo 数据一起由 J-Box 更新脚本处理，订阅保留独立计划。
...
export const runScheduledTasks = async ({ store, ctx, paths, fetchImpl = globalThis.fetch, subscriptionFetchImpl, lookup, runDeploy, now = new Date(), log = () => {} }) => {
  ...
  const ob = updates.jbox || {}
  if (ob.auto && Number(ob.hour) === hour && state.jboxDay !== today) {
    const days = Math.max(1, Number(ob.days) || 1)
    const last = state.jboxLastAt ? new Date(state.jboxLastAt) : null
    ...
    if (due) {
      try {
        ...
        if (!status.running) {
          ...
          state.jboxLastAt = now.toISOString()
          if (compareVersions(latest, meta.version) > 0) {
            const r = await startUpdate(ctx, paths, ob.channel || 'auto')
            log(`[schedule] j-box update ${meta.version} -> ${latest}: ${r.ok ? 'started' : r.output}`)
          } else {
            log(`[schedule] j-box up to date (${meta.version})`)
          }
        }
      } catch (err) {
        log(`[schedule] j-box update check failed: ${err instanceof Error ? err.message : err}`)
      }
    }
  }
