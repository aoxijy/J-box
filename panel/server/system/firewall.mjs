...
const DNS_RULE = 'firewall.jbox_dns'
const V6BLOCK_RULE = 'firewall.jbox_v6block'
// 共享网络每台服务器一条:firewall.jbox_srv_<id>
const SERVER_RULE_PREFIX = 'jbox_srv_'
...
const wantedRules = async (ctx) => {
  ...
  for (const raw of String(stdout || '').split('\n')) {
    const m = raw.trim().match(/^firewall\.(jbox_[A-Za-z0-9_]+)\.(name|dest_port)='?([^']*)'?$/)
    if (!m) continue
...
const liveRules = async (ctx) => {
  ...
  for (const raw of text.split('\n')) {
    const m = raw.match(/comment "!fw4: (J-Box[^"]*)"/)
...
export const applyPanelLanRule = async (ctx, { port = 2026, commit = true } = {}) => {
  const changed = await ensureRule(ctx, PANEL_RULE, { name: 'J-Box Panel (LAN)', src: 'lan', proto: 'tcp', dest_port: port, target: 'ACCEPT' })
  if (changed && commit) await commitReload(ctx)
  return { applied: true, changed }
}

export const applyDnsLanRule = async (ctx, { port = 7853, commit = true } = {}) => {
  const changed = await ensureRule(ctx, DNS_RULE, { name: 'J-Box DNS (LAN)', src: 'lan', proto: 'tcp udp', dest_port: port, target: 'ACCEPT' })
  if (changed && commit) await commitReload(ctx)
  return { applied: true, changed }
}
...
export const applyIpv6Block = async (ctx, { enabled, commit = true }) => {
  const changed = await ensureRule(ctx, V6BLOCK_RULE, enabled
    ? { name: 'J-Box Block IPv6 Leak', src: 'lan', dest: 'wan', family: 'ipv6', target: 'REJECT' }
    : null)
  if (changed && commit) await commitReload(ctx)
  return { applied: enabled === true, changed }
}

// 现有的共享网络放行规则(uci 里以 jbox_srv_ 开头的具名 rule 段)
export const listServerRules = async (ctx) => {
  const { code, stdout } = await ctx.exec('uci', ['show', 'firewall'])
  if (code !== 0) return []
  const names = new Set()
  for (const line of String(stdout || '').split('\n')) {
    const m = line.match(/^firewall\.(jbox_srv_[A-Za-z0-9_]+)=rule\s*$/)
    if (m) names.add(m[1])
  }
  return [...names]
}
...
export const applyServerPortRules = async (ctx, servers = [], { commit = true } = {}) => {
  ...
  const desired = new Map(servers.filter(serverWanExposed).map((s) => [
    ...
    { name: `J-Box Share ${s.name || s.id}`, src: 'wan', proto: serverFirewallProto(s), dest_port: s.port, target: 'ACCEPT' },
  ]))
  let changed = false
