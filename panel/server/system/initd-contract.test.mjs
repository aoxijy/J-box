...
test('内核脚本引用的二进制与配置路径与 createPaths 一致', () => {
  // 脚本内用 JBOX_ROOT 变量拼接而非写字面量绝对路径,逐行精确匹配(整行锚定,
  // 而非子串包含),这样任何一侧的路径后缀漂移(哪怕只是加了后缀)都会被捕获。
  assert.match(core, /^JBOX_ROOT=\/opt\/j-box$/m, 'JBOX_ROOT 根路径漂移')
  assert.match(core, /^BIN="\$JBOX_ROOT\/bin\/sing-box"$/m, '二进制路径漂移')
  assert.match(core, /^CONF="\$JBOX_ROOT\/etc\/config\.json"$/m, '配置路径漂移')
...
test('内核停止清理:dnsmasq 清理仅在接管标记(备份文件)存在时执行', async () => {
  ...
  // 脚本用 "$DATA/dnsmasq-backup.txt"(DATA="$JBOX_ROOT/data")拼出同一路径;
  // 逐段核对文件名与目录变量,防止两侧漂移。
  ...
  assert.match(
    core,
    /jbox_cleanup\(\)\s*\{[^}]*if \[ -f "\$DNSMASQ_BACKUP" \];\s*then[^]*?delete dhcp\.@dnsmasq\[0\]\.server[^]*?delete dhcp\.@dnsmasq\[0\]\.noresolv[^]*?add_list dhcp\.@dnsmasq\[0\]\.server=[^]*?done < "\$DNSMASQ_BACKUP"/,
    'jbox_cleanup 必须把 dnsmasq 清理整体置于备份文件存在性判断之内,且按备份重建上游',
  )
})

// 审查第 3 项:停止时要按备份把 dnsmasq 还原到接管前,而不是只摘掉自己写的条目。
// 把脚本里的 jbox_cleanup 原样抽出来,uci / dnsmasq / firewall 换成记录状态的桩,真的跑一遍。
const runCleanup = ({ backup, servers, noresolv, commitFails = false, installedForward = null }) => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'jbox-initd-'))
  if (installedForward !== null) { fs.mkdirSync(path.join(dir, 'confdir')); fs.writeFileSync(path.join(dir, 'confdir', 'j-box.conf'), installedForward) }
  const fn = core.match(/^jbox_cleanup\(\) \{[^]*?^\}/m)
  assert.ok(fn, '抽不出 jbox_cleanup')
  const body = fn[0]
...
set -u
D='${dir}'
DATA="$D"; DNSMASQ_BACKUP="$D/dnsmasq-backup.txt"; NFT_TABLE="inet jbox"; JBOX_DNS_UPSTREAM='127.0.0.1#7853'
...
DNSMASQ_FORWARD_CONF=j-box.conf
jbox_dnsmasq_confdir() { echo "$D/confdir"; }
${body}
jbox_cleanup
echo "servers=$(tr '\\n' ',' < "$SV")"
echo "noresolv=$(cat "$NR" 2>/dev/null)"
echo "backup=$([ -f "$DNSMASQ_BACKUP" ] && echo yes || echo no)"
echo "forward=$([ -f "$D/confdir/j-box.conf" ] && echo yes || echo no)"
echo "log=$(tr '\\n' ',' < "$LOG" 2>/dev/null)"
`
...
test('内核启动:dnsmasq 模式先把 dnsmasq 上游重新指向内核,再拉起 sing-box(干净重启后不再打环)', async () => {
  ...
  assert.equal(configMetaPath(paths), '/opt/j-box/etc/config.meta.json')
  assert.match(core, /^CONF_META="\$JBOX_ROOT\/etc\/config\.meta\.json"$/m, 'init 脚本元数据路径与 deploy.mjs 的 configMetaPath 不一致')
  assert.match(core, /jbox_dnsmasq_mode\(\)\s*\{[^]*?if \[ -f "\$CONF_META" \]; then[^]*?"dnsMode\\": \*\\"dnsmasq[^]*?return \$\?[^]*?fi[^]*?DNSMASQ_OUTBOUND_TAG/, 'jbox_dnsmasq_mode 必须先看元数据的 dnsMode,再退回 grep 出站 tag')
  const start = core.match(/^start_service\(\)\s*\{([^]*?)^\}/m)
  assert.ok(start, '无法提取 start_service 函数体')
  const body = start[1]
  const takeoverAt = body.indexOf('jbox_apply_takeover')
  const markAt = body.indexOf('jbox_apply_dns_mark')
  const procdAt = body.indexOf('procd_open_instance')
  assert.ok(takeoverAt !== -1 && markAt !== -1 && procdAt !== -1, 'start_service 必须调用 jbox_apply_takeover 与 jbox_apply_dns_mark')
  ...
  assert.match(core, /jbox_apply_takeover\(\)\s*\{[^]*?plan=\*\) _ob_plan=[^]*?\[ "\$_ob_plan" = "none" \] && return 0[^]*?_ob_want=" \$JBOX_DNS_UPSTREAM"[^]*?_ob_want_noresolv=1/, 'plan=none 必须在"没有条目就全量接管兜底"之前返回')
  // 幂等:先比对再改,避免面板 deploy 之后的 restart 再重启一次 dnsmasq
  assert.match(core, /jbox_apply_takeover\(\)\s*\{[^]*?sort\)" \][^]*?return 0[^]*?uci -q commit dhcp/, 'jbox_apply_takeover 必须先比对当前值、一致就直接返回')
})

test('内核启动:起了看门狗,内核在 dnsmasq 模式下起不来就把 dnsmasq 还给系统(procd 放弃重试不会调 stop)', () => {
  const start = core.match(/^start_service\(\)\s*\{([^]*?)^\}/m)
  assert.ok(start, '无法提取 start_service 函数体')
  assert.ok(start[1].indexOf('jbox_watch_start') > start[1].indexOf('procd_close_instance'), '看门狗必须在 procd_close_instance 之后启动')
  const watch = core.match(/^jbox_watch_start\(\)\s*\{([^]*?)^\}/m)
  assert.ok(watch, '无法提取 jbox_watch_start 函数体')
  assert.match(watch[1], /jbox_dnsmasq_mode \|\| return 0/, '只有 dnsmasq 模式才需要看门狗')
  // 崩溃循环里 procd 每 5 秒重拉一次,status 文本在两次崩溃之间照样是 running(实测被骗过);
  // pidof 又会被别的 sing-box 进程骗到。只能看 procd 里实例 PID 是否连续稳定。
  assert.match(core, /^jbox_instance_pid\(\)\s*\{[^}]*ubus call service list/m, '实例 PID 必须从 procd(ubus service list)取')
  assert.match(watch[1], /_ob_pid="\$\(jbox_instance_pid\)"[^]*?\[ "\$_ob_pid" = "\$_ob_last" \]/, '稳定的判断必须是同一个 PID 连续出现,而不是 status 文本')
  assert.ok(!/init\.d\/jbox status/.test(watch[1]), '看门狗不得用 status 文本判断在跑')
  assert.match(watch[1], /jbox_cleanup/, '没起来时必须走和 stop 一样的完整清理')
...
test('内核启动:dnsmasq 模式给 dnsmasq 自己的外部查询打 sing-box 的 fwmark 防打环,停止时撤掉', () => {
  ...
  assert.match(core, /jbox_apply_dns_mark\(\)\s*\{[^]*?if ! jbox_dnsmasq_mode; then[^]*?nft delete table \$NFT_TABLE/, '非 dnsmasq 模式启动时必须删掉标记表')
  assert.match(core, /meta skuid \$_ob_uid meta l4proto \{ tcp, udp \} th dport 53 meta mark set \$SINGBOX_OUTPUT_MARK/)
  assert.match(core, /jbox_cleanup\(\)\s*\{[^]*?nft delete table \$NFT_TABLE/, 'jbox_cleanup 必须撤掉标记表')
})
...
test('内核脚本自定义 restart():跳过清理,否则 USE_PROCD=1 下 restart=stop;start 会自己撤掉刚下发的接管', () => {
  ...
  // stop_service → jbox_cleanup。不覆盖 restart() 的话,每次 deployConfig 重启内核
  // 都会立刻把刚写入的 DNS 接管和 v6 拦截撤销,却仍然报告部署成功。
  assert.match(core, /^restart\(\)\s*\{/m, '缺少自定义 restart(),重启路径会退回 stop;start 触发清理')
  // 行锚定断言默认值必须是 0:如果有人把顶层默认改成 JBOX_SKIP_CLEANUP=1,会静默
  // 关掉 stop 时的清理(重开 DNS-outage 类问题),但之前的 /JBOX_SKIP_CLEANUP=1/
  // 断言(不锚定位置)对这种改法完全不敏感——因为 restart() 里本来就有一处合法的 =1。
  assert.match(core, /^JBOX_SKIP_CLEANUP=0$/m, '默认必须是 0,否则 stop 时的清理会被静默关闭')
  ...
  assert.match(restartBody[1], /JBOX_SKIP_CLEANUP=1/, 'restart() 函数体内必须设置跳过清理的标记')
  assert.match(
    core,
    /stop_service\(\)\s*\{[^}]*JBOX_SKIP_CLEANUP[^}]*jbox_cleanup/s,
    'stop_service 必须依据该标记决定是否调用 jbox_cleanup',
  )
})
...
test('面板脚本设置 LD_LIBRARY_PATH 指向捆绑的 node/lib(P6 终审 Critical 1)', () => {
  ...
  assert.match(
    ...
    '面板 init 脚本必须设置 LD_LIBRARY_PATH=$JBOX_ROOT/node/lib(即 /opt/j-box/node/lib)',
  )
})
...
const runCleanStaleNft = ({ tableExists, singboxRunning, hasNft = true }) => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'jbox-nft-'))
  const fn = core.match(/^jbox_clean_stale_nft\(\) \{[^]*?^\}/m)
  assert.ok(fn, '抽不出 jbox_clean_stale_nft')
  const harness = `
set -u
D='${dir}'
LOG="$D/log"; : > "$LOG"
...
test('清理排在起内核之前(顺序反了就白清)', () => {
  ...
  const iClean = start[0].indexOf('jbox_clean_stale_nft')
...
// 复审 R1:把 jbox_apply_takeover 原样抽出来,uci / dnsmasq 换成记录状态的桩,按状态文件的
...
const runTakeover = ({ state, servers, noresolv, meta = '{"dnsMode": "dnsmasq"}', forwardSrc = null, installedForward = null }) => {
  ...
  if (installedForward !== null) { fs.mkdirSync(path.join(dir, 'confdir')); fs.writeFileSync(path.join(dir, 'confdir', 'j-box.conf'), installedForward) }
  const fn = core.match(/^jbox_apply_takeover\(\) \{[^]*?^\}/m)
  assert.ok(fn, '抽不出 jbox_apply_takeover')
  const mode = core.match(/^jbox_dnsmasq_mode\(\) \{[^]*?^\}/m)
  assert.ok(mode, '抽不出 jbox_dnsmasq_mode')
...
DNSMASQ_FORWARD_SRC="$D/dnsmasq-forward.conf"; DNSMASQ_FORWARD_CONF=j-box.conf
JBOX_DNS_UPSTREAM='127.0.0.1#7853'; DNSMASQ_OUTBOUND_TAG=dnsmasq
...
${mode[0]}
${body}
jbox_apply_takeover 2>/dev/null
echo "servers=$(tr '\\n' ',' < "$SV")"
echo "noresolv=$(cat "$NR" 2>/dev/null)"
echo "forward=$(cat "$D/confdir/j-box.conf" 2>/dev/null | tr '\\n' ',')"
echo "log=$(tr '\\n' ',' < "$LOG" 2>/dev/null)"
`
  const out = execFileSync('sh', ['-c', harness], { encoding: 'utf8' })
  fs.rmSync(dir, { recursive: true, force: true })
