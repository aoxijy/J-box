...
const INSTALLED = '/tmp/dnsmasq.cfg.d/j-box.conf'
const FORWARD_SRC = dnsForwardFilePath(paths)
const STATE = '/opt/j-box/data/dnsmasq-takeover.txt'
const BACKUP = '/opt/j-box/data/dnsmasq-backup.txt'
...
test('重写例外在 domains ↔ all 之间保留,none 和回滚只移除 J-Box 的文件', async () => {
...
test('还原:清除写入值并恢复备份,删备份文件', async () => {
  const ctx = createMockContext({
    files: { '/opt/j-box/data/dnsmasq-backup.txt': "dhcp.cfg01411c.server='223.5.5.5'\ndhcp.cfg01411c.noresolv='0'\n" },
  })
...
test('还原:一并删掉开机照抄用的状态文件和转发文件,切模式 / 回滚后开机不再接管', async () => {
  ...
  assert.equal(await ctx.exists('/tmp/dnsmasq.d/j-box.conf'), false)
})
...
test('还原:多上游备份(同行多个引号值)全部恢复,而非只恢复第一个', async () => {
  const ctx = createMockContext({
    files: { '/opt/j-box/data/dnsmasq-backup.txt': "dhcp.cfg.server='1.1.1.1' '8.8.8.8'\ndhcp.cfg.noresolv='0'\n" },
  })
  const r = await restoreDnsTakeover(ctx, paths)
  assert.equal(r.restored, true)
  const c = cmds(ctx)
...
test('还原:uci commit 失败要抛错,备份文件必须还在(下次还能重来),暂存的半截改动要 revert', async () => {
  const ctx = createMockContext({
    files: { '/opt/j-box/data/dnsmasq-backup.txt': "dhcp.cfg01411c.server='9.9.9.9'\ndhcp.cfg01411c.noresolv='1'\n" },
  ...
  assert.equal(await ctx.exists('/opt/j-box/data/dnsmasq-backup.txt'), true)
  const c = cmds(ctx)
...
test('还原:重建原上游的 add_list 失败也抛错、留备份;delete / del_list 返回非零不算失败(目标不存在是常态)', async () => {
  ...
  assert.equal(await ok.exists('/opt/j-box/data/dnsmasq-backup.txt'), false)
})

test('转发计划 none(全部直连):接管过就还原到接管前的上游;状态文件写 plan=none;没接管过就一个字不动', async () => {
  // 接管过:备份在,uci 里是我们写的全量转发
  const uci = statefulUci({ servers: ['127.0.0.1#7853'], noresolv: '1', files: { '/opt/j-box/data/dnsmasq-backup.txt': "dhcp.cfg01411c.server='223.5.5.5' '192.168.3.5'\ndhcp.cfg01411c.noresolv='0'\n", '/opt/j-box/data/dnsmasq-takeover.txt': 'plan=all\nserver=127.0.0.1#7853\nnoresolv=1\n' } })
  const ctx = uci.ctx
  ...
  assert.equal(ctx.files['/opt/j-box/data/dnsmasq-takeover.txt'], 'plan=none\n')
  ...
  assert.equal(ctx2.files['/opt/j-box/data/dnsmasq-takeover.txt'], 'plan=none\n')
})
...
test('all → domains:原上游、定向域名上游和 noresolv=1 都从备份基线里恢复,再叠加我们的按域名条目', async () => {
  ...
  assert.equal(uci.ctx.files['/opt/j-box/data/dnsmasq-takeover.txt'], `plan=domains\nforward=${INSTALLED}\nnoresolv=1\n`)
  // 再切回 none:完整还原到接管前
  await applyDnsTakeover(uci.ctx, paths, { mode: 'dnsmasq', forward: { mode: 'none', domains: [] } })
  assert.deepEqual(uci.state, { servers: ['9.9.9.9', '/corp.example/192.168.3.5'], noresolv: '1' })
  assert.equal(uci.ctx.files['/opt/j-box/data/dnsmasq-takeover.txt'], 'plan=none\n')
  assert.equal(await uci.ctx.exists(INSTALLED), false)
