...
import { applyPanelLanRule, applyIpv6Block, commitFirewall, removeProxyRules, removeJBoxRules } from './firewall.mjs'
...
test('removeJBoxRules 清两条(含面板放行)+ reload,仅供卸载使用', async () => {
  const ctx = createMockContext()
...
test('共享网络放行:先清掉旧的 jbox_srv_*,再按启用的服务器逐条加,SS 放 tcp udp', async () => {
  const { applyServerPortRules, removeProxyRules } = await import('./firewall.mjs')
  const { createMockContext } = await import('./context.mjs')
  const ctx = createMockContext({ execResults: { 'uci show firewall': { code: 0, stdout: 'firewall.jbox_srv_old=rule\nfirewall.jbox_srv_old.name=x\nfirewall.@rule[0]=rule\n' } } })
  ...
  assert.ok(!cmds.some((c) => c.includes('firewall.jbox_srv_mx')), cmds.join('\n'))
  assert.ok(cmds.includes('uci -q delete firewall.jbox_srv_old'))
  assert.ok(cmds.includes('uci set firewall.jbox_srv_ab_1=rule'))
  assert.ok(cmds.includes('uci set firewall.jbox_srv_ab_1.proto=tcp udp'))
  assert.ok(cmds.includes('uci set firewall.jbox_srv_ab_1.dest_port=8388'))
  assert.ok(cmds.includes('uci set firewall.jbox_srv_hy.proto=udp'))
  assert.ok(cmds.includes('uci set firewall.jbox_srv_hy.src=wan'))
  assert.equal(cmds.filter((c) => c === 'uci commit firewall').length, 1)
  ctx.calls.length = 0
  await removeProxyRules(ctx)
  assert.ok(ctx.calls.map((c) => [c.cmd, ...c.args].join(' ')).includes('uci -q delete firewall.jbox_srv_old'))
})
...
test('applyDnsLanRule:只放行 LAN 到 7853 的 tcp/udp;removeProxyRules 会一起删掉', async () => {
  ...
  assert.ok(sets.includes('firewall.jbox_dns.src=lan'))
  assert.ok(sets.includes('firewall.jbox_dns.proto=tcp udp'))
  assert.ok(sets.includes('firewall.jbox_dns.dest_port=7853'))
  assert.ok(sets.includes('firewall.jbox_dns.target=ACCEPT'))
  ctx.calls.length = 0
  await removeProxyRules(ctx)
  assert.ok(ctx.calls.some((c) => c.cmd === 'uci' && c.args.join(' ') === '-q delete firewall.jbox_dns'))
})

test('规则已经是目标状态:一个字不动、不 commit、不 reload;commit:false 时由调用方统一 reload 一次', async () => {
  const present = [
    "firewall.jbox_panel=rule",
    "firewall.jbox_panel.name='J-Box Panel (LAN)'",
    "firewall.jbox_panel.src='lan'",
    "firewall.jbox_panel.proto='tcp'",
    "firewall.jbox_panel.dest_port='2026'",
    "firewall.jbox_panel.target='ACCEPT'",
  ].join('\n')
  const ctx = createMockContext({ execResults: { 'uci -q show firewall.jbox_panel': { code: 0, stdout: present } } })
  const r = await applyPanelLanRule(ctx, { port: 2026 })
  ...
  assert.ok(cmds(ctx).includes('uci set firewall.jbox_panel.dest_port=2027'))
...
const UCI_TWO_RULES = [
  ...
  "firewall.jbox_panel.name='J-Box Panel (LAN)'",
  "firewall.jbox_panel.dest_port='2026'",
  'firewall.jbox_dns=rule',
  "firewall.jbox_dns.name='J-Box DNS (LAN)'",
  "firewall.jbox_dns.dest_port='7853'",
  // 别人的段不该被当成我们的
  'firewall.@rule[0]=rule',
  "firewall.@rule[0].name='Allow-DHCP'",
].join('\n')
const nftWith = (panelPort, { dns = true } = {}) => [
  `\ttcp dport ${panelPort} counter packets 3 bytes 180 accept comment "!fw4: J-Box Panel (LAN)"`,
  ...(dns
    ? [
        '\ttcp dport 7853 counter packets 0 bytes 0 accept comment "!fw4: J-Box DNS (LAN)"',
        '\tudp dport 7853 counter packets 0 bytes 0 accept comment "!fw4: J-Box DNS (LAN)"',
      ]
    : []),
