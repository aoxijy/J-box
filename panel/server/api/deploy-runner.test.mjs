...
test('withDeployLock:别的进程持锁且活着就等它释放;锁过期或进程已死就直接接管;自己用完释放', async () => {
  ...
  const now = () => t
  const sleeps = []
  const sleep = async (ms) => { sleeps.push(ms); t += ms; if (sleeps.length === 3) m.delete('jbox/deploy-lock') }
  // 另一个活着的进程(pid 999)持锁 → 等到它释放(第 3 次 sleep 时释放)
  m.set('jbox/deploy-lock', JSON.stringify({ pid: 999, at: 1000 }))
  const r = await withDeployLock(store, async () => 'done', { sleep, now, pid: 1, alive: () => true })
  assert.equal(r, 'done')
  assert.equal(sleeps.length, 3)
  assert.ok(!m.has('jbox/deploy-lock'), '用完要释放')
  // 锁是死进程留下的 → 不等
  m.set('jbox/deploy-lock', JSON.stringify({ pid: 999, at: t }))
  const calls = []
  await withDeployLock(store, async () => calls.push(JSON.parse(m.get('jbox/deploy-lock')).pid), { sleep, now, pid: 1, alive: () => false })
...
test('withDeployLock:持有者按心跳刷新时间戳,另一进程看到锁龄超过 3 分钟但心跳新鲜就继续等;锁带 token,PID 被复用也认得出不是自己的', async () => {
  ...
  const first = JSON.parse(m.get('jbox/deploy-lock'))
  assert.equal(first.pid, 101)
  assert.ok(first.token)
  t += 200_000; beats[0]()                       // 3 分 20 秒后心跳,at 刷新
  assert.equal(JSON.parse(m.get('jbox/deploy-lock')).at, t)
  ...
  assert.ok(!m.has('jbox/deploy-lock'))
  // PID 复用:锁上是 pid 202 的旧 token,新的 pid 202 进程不会把它当自己的——持有者已死就接管
  m.set('jbox/deploy-lock', JSON.stringify({ pid: 202, at: t, token: 'stale-token' }))
  const r = await withDeployLock(store, async () => JSON.parse(m.get('jbox/deploy-lock')).token, { now, pid: 202, alive: () => false, setIntervalImpl, clearIntervalImpl })
