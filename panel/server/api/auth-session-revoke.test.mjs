...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'jbox-session-revoke-test-'))
...
const cookieOf = (res) => (res.headers.get('set-cookie') || '').match(/jbox_access_session=[^;]+/)?.[0]
const profileStatus = async (cookie) => (await fetch(`${baseUrl}/api/jbox/profile`, { headers: { cookie } })).status

test('每次登录签发不同的随机会话;退出登录后该会话在服务端被撤销,拷走的 cookie 不再可用;别的会话不受影响', async () => {
  mod.db.exec("DELETE FROM app_storage WHERE key LIKE 'config/access-%' OR key = 'jbox/sessions'")
  ...
  // 会话记录在受保护的 jbox/ 前缀下,不会被 /api/storage 回显或覆盖
  assert.ok(mod.isProtectedStorageKey('jbox/sessions'))
  const stored = JSON.parse(mod.db.prepare('SELECT value FROM app_storage WHERE key = ?').get('jbox/sessions').value)
...
test('服务端自己看过期:把记录里的 expiresAt 改到过去,cookie 还在也不认', async () => {
  ...
  const row = mod.db.prepare('SELECT value FROM app_storage WHERE key = ?').get('jbox/sessions')
  const sessions = JSON.parse(row.value)
  sessions[id].expiresAt = Date.now() - 1000
  mod.db.prepare('UPDATE app_storage SET value = ? WHERE key = ?').run(JSON.stringify(sessions), 'jbox/sessions')
