...
export const loadEntries = async (ctx, paths, tag) => {
  ...
  if (!(await ctx.exists(srsPath))) throw new Error(`安装包缺少规则集 ${tag}，请更新或重新安装 J-Box`)
...
export const registerRulesetRoutes = (app, { ctx, paths, store, fetchImpl = globalThis.fetch } = {}) => {
  ...
  // GET /api/jbox/policies/entries?name=AI&tab=all|domain|ip&q=&sort=type|content|source&dir=asc|desc&offset=0&limit=100
  ...
  // GET /api/jbox/rulesets/preview?url=https://…/Check.list&q=&offset=0&limit=50
  ...
  // GET /api/jbox/rulesets/entries?tag=geosite-cn&q=&offset=0&limit=50
