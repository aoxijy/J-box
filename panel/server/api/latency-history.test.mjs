...
test('GET 整份;POST samples 校验字段并记入;POST sync 让服务端读一次内核再返回整份', async () => {
  ...
  try {
    ...
    const ok = await post('/api/jbox/latency-history/samples', { samples: [{ name: 'A', time: '2026-09-06T10:00:00.000Z', delay: 0 }] })
    assert.equal(ok.status, 200)
    assert.deepEqual((await ok.json()).history.A, [{ time: '2026-09-06T10:00:00.000Z', delay: 0 }])
    assert.equal((await post('/api/jbox/latency-history/samples', { samples: [{ name: 'A', time: 'bad', delay: 0 }] })).status, 400)
    assert.equal((await post('/api/jbox/latency-history/samples', { samples: 'x' })).status, 400)
    assert.equal((await post('/api/jbox/latency-history/samples', { samples: [{ name: 'A', time: '2026-09-06T10:00:00.000Z', delay: -1 }] })).status, 400)
    const sync = await post('/api/jbox/latency-history/sync', {})
    assert.equal(sync.status, 200)
    assert.equal(synced, 1)
    assert.deepEqual((await sync.json()).history.B.map((s) => s.delay), [50])
    const get = await fetch(`${baseUrl}/api/jbox/latency-history`)
