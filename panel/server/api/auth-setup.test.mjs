...
// (config/access-*、jbox/*),密码一旦被某个用例设过就会残留到后续用例,直接操作
// 底层 db 才能做到"整表清空"的测试隔离语义。
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'jbox-auth-setup-test-'))
...
test('未设密时:GET /api/jbox/profile(受守卫的普通路由)得 403 PASSWORD_SETUP_REQUIRED', async () => {
...
test('POST /api/auth/setup 成功:设密 + 启用认证 + 签发 cookie,同一 cookie 可访问受保护路由', async () => {
  ...
  assert.ok(cookie, 'setup 应该签发 jbox_access_session cookie')
