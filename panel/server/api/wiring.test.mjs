...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'jbox-wiring-test-'))
...
const JBOX_ROUTES = [
  { method: 'GET', path: '/api/jbox/subscriptions' },
  { method: 'GET', path: '/api/jbox/profile' },
  { method: 'GET', path: '/api/jbox/config/preview' },
  { method: 'GET', path: '/api/jbox/service/status' },
  { method: 'GET', path: '/api/jbox/kernel/version' },
  { method: 'POST', path: '/api/jbox/penetration' },
]

test('未设密时:每条 J-Box 路由都是 403 PASSWORD_SETUP_REQUIRED', async () => {
  for (const route of JBOX_ROUTES) {
...
test('设密后携带 cookie:各 J-Box 路由真正打到 handler(非 404,契约字段吻合)', async () => {
  ...
  const versionRes = await fetch(`${baseUrl}/api/jbox/kernel/version`, { headers })
  ...
  const penRes = await fetch(`${baseUrl}/api/jbox/penetration`, {
    method: 'POST',
...
// 匹配此前仍是大小写不敏感的,导致 /api/jbox/deploy/STATE 这类变体在"已认证"状态下
// 仍会命中真实 handler(只是没有构成认证绕过,因为守卫已经放行了这次请求)。
test('已认证下:GET /api/jbox/deploy/STATE(大写路径段)应为 404,而不是命中 /deploy/state handler', async () => {
  const res = await fetch(`${baseUrl}/api/jbox/deploy/STATE`, { headers: { cookie: authCookie } })
  assert.equal(res.status, 404)
})

test('已认证下:GET /api/jbox/service/STATUS(大写路径段)应为 404,而不是命中 /service/status handler', async () => {
  const res = await fetch(`${baseUrl}/api/jbox/service/STATUS`, { headers: { cookie: authCookie } })
  assert.equal(res.status, 404)
})

test('已认证下:POST /api/jbox/subscriptions/PREVIEW(大写路径段)应为 404,而不是命中 /preview handler', async () => {
  // 注意用 POST(该路由实际注册的方法):若误用 GET,不管 caseSensitive 与否都会因为
  // "方法不存在"而 404,测不出案例真正要验证的东西。
  const res = await fetch(`${baseUrl}/api/jbox/subscriptions/PREVIEW`, {
    method: 'POST',
