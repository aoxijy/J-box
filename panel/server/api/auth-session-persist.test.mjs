...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'jbox-session-persist-test-'))
...
test('面板进程重启后旧 cookie 仍然有效(会话密钥落库),改密后立即失效', async () => {
  ...
  const cookie = (setupRes.headers.get('set-cookie') || '').match(/jbox_access_session=[^;]+/)?.[0]
  assert.ok(cookie, 'setup 应签发 session cookie')

  // 同一份数据库、另一个进程实例(= 升级替换文件后重新启动的面板)
  const reused = await fetch(`${second.baseUrl}/api/jbox/profile`, { headers: { cookie } })
  assert.equal(reused.status, 200, '进程重启后旧 cookie 必须仍然有效,不能把用户踢回登录页')

  // 密钥确实存下来了,而且在受保护前缀下(不回显给浏览器、不被设置同步清掉)
  const stored = first.db.prepare('SELECT value FROM app_storage WHERE key = ?').get('jbox/session-secret')
  assert.ok(stored && stored.value.length >= 32)
  assert.ok(first.isProtectedStorageKey('jbox/session-secret'))
  ...
  const afterChange = await fetch(`${second.baseUrl}/api/jbox/profile`, { headers: { cookie } })
