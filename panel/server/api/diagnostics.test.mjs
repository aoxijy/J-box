...
test('诊断包:秘密和节点地址一个都不能剩,该有的信息都在', async () => {
  ...
  // 该有的信息
  assert.equal(bundle.format, 'j-box-diagnostics')
  assert.equal(bundle.versions.jBox, 'v0.1.135')
...
test('GET /api/jbox/diagnostics 返回诊断包;读不到配置 / 命令失败也不炸', async () => {
  const app = express()
  registerDiagnosticsRoutes(app, { store, ctx: mkCtx(), paths })
  const server = app.listen(0)
  await new Promise((r) => server.once('listening', r))
  try {
    const res = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/diagnostics`)
    assert.equal(res.status, 200)
    assert.equal((await res.json()).format, 'j-box-diagnostics')
  } finally {
  ...
  assert.equal(bare.versions.jBox, null)
