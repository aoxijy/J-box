...
export const BACKUP_FORMAT = 'j-box-backup'
...
// 读写函数作为 panelStorage 传进来。密码(config/access-*)和 jbox/* 不在其中。
...
export const buildBackup = (store, {
  subscriptions = true, clientRoutes = true, servers = true, now = () => new Date(), jboxVersion = '', panelStorage = null,
} = {}) => {
...
export const applyBackup = (store, data, { subscriptionsMode = 'replace', panelStorage = null } = {}) => {
  ...
  if (!isPlainObject(data) || data.format !== BACKUP_FORMAT) return { error: '不是 J-Box 导出的文件' }
...
export const registerBackupRoutes = (app, { store, readVersion = async () => '', panelStorage = null } = {}) => {
  ...
  // GET /api/jbox/backup?subscriptions=1|0&clientRoutes=1|0&servers=1|0(都默认 1)
  router.get('/backup', async (req, res) => {
    const flag = (name) => String(req.query[name] ?? '1') !== '0'
    let jboxVersion = ''
    try { jboxVersion = (await readVersion()) || '' } catch { /* 拿不到就空着 */ }
  ...
  // POST /api/jbox/backup/import?subscriptions=replace|append  body = 导出的那份 JSON。
