...
test('GET /traffic/month:缺参数用当月,格式错 400,读之前先 flush', async () => {
  ...
  try {
    const r = await fetch(`${base}/api/jbox/traffic/month`)
    ...
    assert.equal((await fetch(`${base}/api/jbox/traffic/month?month=2026-13`)).status, 400)
    assert.equal((await fetch(`${base}/api/jbox/traffic/month?month=2026/09`)).status, 400)
  } finally {
    await close()
  }
})
...
test('GET /traffic/day:节点/域名明细 + 未采样差额;非法日期 400', async () => {
  ...
  const { base, close } = await startApp(collector, () => new Date(2026, 8, 3, 10))
  try {
    const r = await fetch(`${base}/api/jbox/traffic/day?day=2026-09-03`)
    const body = await r.json()
    ...
    const empty = await (await fetch(`${base}/api/jbox/traffic/day?day=2026-09-02`)).json()
    assert.deepEqual(empty.total, { up: 0, down: 0, conns: 0 })
    assert.deepEqual(empty.nodes, [])
    assert.equal((await fetch(`${base}/api/jbox/traffic/day?day=2026-09-32`)).status, 400)
    assert.equal((await fetch(`${base}/api/jbox/traffic/day`)).status, 400)
  } finally {
    await close()
  }
})
...
test('GET /traffic/day:访问终端按来源 IP,能从 DHCP 租约翻出主机名', async () => {
  ...
  try {
    const body = await (await fetch(`${base}/api/jbox/traffic/day?day=2026-09-03`)).json()
...
test('GET /traffic/drill:一条记录按另一维拆;按终端拆时带主机名;参数校验 400', async () => {
  ...
  const { base, close } = await startApp(collector, () => new Date(2026, 8, 3, 10), { ctx, paths: { dhcpLeases: '/tmp/dhcp.leases' } })
  try {
    const q = (s) => fetch(`${base}/api/jbox/traffic/drill?${s}`)
...
test('GET /clients:租约里的设备 + 今天流量里的来源 IP', async () => {
  ...
  const { base, close } = await startApp(collector, () => new Date(2026, 8, 3, 10), { ctx, paths: { dhcpLeases: '/tmp/dhcp.leases' } })
  try {
    const body = await (await fetch(`${base}/api/jbox/clients`)).json()
...
test('GET /traffic/day?hour=13:明细换成那个小时的,总量取小时桶;drill 也按小时;hour 非法 400', async () => {
  ...
  const { base, close } = await startApp(collector, () => new Date(2026, 8, 3, 10))
  try {
    const body = await (await fetch(`${base}/api/jbox/traffic/day?day=2026-09-03&hour=13`)).json()
    ...
    const whole = await (await fetch(`${base}/api/jbox/traffic/day?day=2026-09-03`)).json()
    assert.equal(whole.hour, null)
    assert.equal(whole.nodes.length, 2)
    assert.equal((await fetch(`${base}/api/jbox/traffic/day?day=2026-09-03&hour=24`)).status, 400)
    await fetch(`${base}/api/jbox/traffic/drill?day=2026-09-03&kind=host&key=a.com&by=client&hour=13`)
...
test('direct=0:「统计直连流量」关掉——月 / 日总量、小时桶、节点 / 站点 / 终端列表都扣掉直连那份,drill 按出站拆时去掉直连行', async () => {
  ...
  const { base, close } = await startApp(collector, () => new Date(2026, 8, 3, 10))
  try {
    const month = await (await fetch(`${base}/api/jbox/traffic/month?month=2026-09&direct=0`)).json()
    ...
    const on = await (await fetch(`${base}/api/jbox/traffic/month?month=2026-09`)).json()
    assert.equal(on.direct.excluded, false)
    assert.equal(on.days.find((d) => d.day === '2026-09-03').up, 100)

    const day = await (await fetch(`${base}/api/jbox/traffic/day?day=2026-09-03&direct=0`)).json()
    ...
    const drill = await (await fetch(`${base}/api/jbox/traffic/drill?day=2026-09-03&kind=host&key=a.com&by=node&direct=0`)).json()
    assert.deepEqual(drill.rows.map((r) => r.key), ['A'])
    assert.equal(drill.count, 1)
    assert.deepEqual(drill.sum, { up: 60, down: 500 })
    const directDrill = await (await fetch(`${base}/api/jbox/traffic/drill?day=2026-09-03&kind=node&key=%E7%9B%B4%E8%BF%9E&by=host&direct=0`)).json()
