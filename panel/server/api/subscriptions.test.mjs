...
test('POST preview 用 content(多协议 sharelink)→ 返回 nodes/preview/groups,且不落库', async () => {
  ...
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions/preview', { content: SHARELINK_MULTI })
...
test('POST preview 显式传 renameOptions:null 时按默认选项处理,不误判 400', async () => {
  ...
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions/preview', {
...
test('POST 创建订阅后 GET 列表可见;DELETE 后消失(同时移除节点)', async () => {
  ...
  try {
    const createRes = await postJson(baseUrl, '/api/jbox/subscriptions', {
    ...
    const listRes = await fetch(`${baseUrl}/api/jbox/subscriptions`)
    ...
    const delRes = await fetch(`${baseUrl}/api/jbox/subscriptions/${created.id}`, { method: 'DELETE' })
    assert.equal(delRes.status, 200)
    assert.deepEqual(await delRes.json(), { ok: true, changed: true })

    const listRes2 = await fetch(`${baseUrl}/api/jbox/subscriptions`)
...
test('DELETE 不存在的 id 仍是幂等的 ok:true', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await fetch(`${baseUrl}/api/jbox/subscriptions/does-not-exist`, { method: 'DELETE' })
    assert.equal(res.status, 200)
...
test('两条订阅含同名节点 → 保存后 tag 全局唯一(dedupeNodeTags 集成断言)', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'Sub A' })
    await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://b', name: 'Sub B' })
...
test('创建时 fetch 返回 500 → 400 且不写入任何数据', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'Sub A' })
    assert.equal(res.status, 400)
    const body = await res.json()
    assert.ok(body.error)
    assert.deepEqual(store.getSubscriptions(), [])
...
test('创建时缺 url 或 name → 400', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res1 = await postJson(baseUrl, '/api/jbox/subscriptions', { name: 'no url' })
    assert.equal(res1.status, 400)
    const res2 = await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a' })
...
test('SSRF 防护:非 http/https 协议(file://)→ 400,且从未真正调用 fetchImpl', async () => {
  ...
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions', {
      url: 'file:///etc/passwd',
      name: 'File',
    })
    assert.equal(res.status, 400)
...
test('refresh 时拉取失败 → 400,已存订阅与节点保持不变', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const createRes = await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'Sub A' })
    const created = await createRes.json()
    const nodesBefore = store.getNodes()
    const subsBefore = store.getSubscriptions()

    shouldFail = true
    const refreshRes = await fetch(`${baseUrl}/api/jbox/subscriptions/${created.id}/refresh`, { method: 'POST' })
...
test('refresh 未知 id → 404', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await fetch(`${baseUrl}/api/jbox/subscriptions/nope/refresh`, { method: 'POST' })
...
test('refresh 成功后只替换该订阅节点,其它订阅节点不受影响', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const createA = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'Sub A' })).json()
    await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://b', name: 'Sub B' })
    assert.equal(createA.nodeCount, 1)

    aContent = GROWN_A
    const refreshRes = await fetch(`${baseUrl}/api/jbox/subscriptions/${createA.id}/refresh`, { method: 'POST' })
...
test('round2:合法的 302 重定向链(公网 → 公网)仍然放行', async () => {
  ...
  const { baseUrl, close } = await startApp(fetchImpl)
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions/preview', { url: 'https://public-a.example.com/sub' })
    assert.equal(res.status, 200)
    const body = await res.json()
...
test('round2:重定向跳数超过上限(4 跳全部合法目标)→ 400', async () => {
  ...
  const { baseUrl, close } = await startApp(fetchImpl)
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions/preview', { url: 'https://hop-0.example.com/sub' })
    assert.equal(res.status, 400)
    const body = await res.json()
...
test('round2:域名解析失败(NXDOMAIN 等) → 400,且从未真正拉取', async () => {
  ...
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions/preview', { url: 'https://nx.invalid/sub' })
    assert.equal(res.status, 400)
    const body = await res.json()
    assert.ok(body.error)
    assert.equal(called, false)
...
test('刷新失败时保留原有节点,不会把订阅刷成 0 个节点', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'https://sub.example.com/x', name: 'S' })).json()
    assert.equal(created.nodeCount, 2)

    serveGarbage = true
    const res = await postJson(baseUrl, `/api/jbox/subscriptions/${created.id}/refresh`, {})
    assert.equal(res.status, 400)
...
test('开了订阅名前缀时,改名字必须重新解析(否则节点上挂着旧前缀)', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', {
...
test('没开前缀时改名字仍然不重新拉取', async () => {
  ...
  const { baseUrl, close } = await startApp(fetchImpl)
  try {
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'https://sub.example.com/x', name: 'A' })).json()
    await fetch(`${baseUrl}/api/jbox/subscriptions/${created.id}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
...
test('换订阅链接会重新拉取并替换该订阅的节点', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'https://sub.example.com/old', name: 'S' })).json()
    assert.equal(created.nodeCount, 2)
...
test('改成一个拉不通的链接 → 400,原有名称/链接/节点全部保留', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'https://sub.example.com/ok', name: '原名' })).json()
...
test('PATCH 不存在的订阅 → 404', async () => {
  ...
  const { baseUrl, close } = await startApp(fetchImpl)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/subscriptions/nope`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
...
test('url 和 content 都没有 → 400', async () => {
  ...
  const { baseUrl, close } = await startApp(fetchImpl)
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions', { name: 'x' })
    assert.equal(res.status, 400)
...
test('拉取失败时把系统 fetch 藏在 cause 里的原因带出来', async () => {
  ...
  const { baseUrl, close } = await startApp(fetchImpl)
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions/preview', { url: 'https://1.2.3.4/sub/x' })
    assert.equal(res.status, 400)
    const body = await res.json()
...
test('PUT /order:按给定 id 顺序重排订阅,节点池跟着重排;id 集合不对 → 400', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const a = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'https://a.test/sub', name: 'A' })).json()
    const b = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'https://b.test/sub', name: 'B' })).json()
    assert.deepEqual(store.getSubscriptions().map((s) => s.name), ['A', 'B'])
    assert.equal(store.getNodes()[0].subscriptionId, a.id)
    const put = (ids) => fetch(`${baseUrl}/api/jbox/subscriptions/order`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ids }) })
...
test('刷新订阅期间删掉了另一条订阅 → 刷新完成后它不会复活(按此刻的列表写回,不用拉取前的快照)', async () => {
  ...
  store = app.store
  try {
    const a = await (await postJson(app.baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'Sub A' })).json()
    await postJson(app.baseUrl, '/api/jbox/subscriptions', { url: 'http://b', name: 'Sub B' })
    assert.equal(store.getSubscriptions().length, 2)
    deleteDuringFetch = true
    const res = await postJson(app.baseUrl, `/api/jbox/subscriptions/${a.id}/refresh`, {})
    assert.equal(res.status, 200)
...
test('多个地址:逐个拉取、节点按地址顺序合在一起、完全相同的节点只留一份;记录里存 urls,url 是第一条', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions', { urls: ['http://a', 'http://b'], name: 'Sub' })
    assert.equal(res.status, 200)
    const body = await res.json()
...
test('多个地址里有一个拉不动:整次失败、报错点名那个地址,什么都不落库', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions', { urls: ['http://a', 'http://bad'], name: 'Sub' })
    assert.equal(res.status, 400)
...
test('刷新和改地址都按 urls 逐个重拉;只改名字不拉', async () => {
  ...
  try {
    ...
    const shrink = await fetch(`${baseUrl}/api/jbox/subscriptions/${created.id}`, {
...
test('老字段 url 照旧能用:只传 url 时记录里 urls 就是它一条', async () => {
  ...
  try {
    await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'Sub' })
...
test('每个动作都如实报 changed:上游没动的刷新、只改名字是 false;换了节点、换地址、删除是 true;任何动作都不自动重启', async () => {
  ...
  const { baseUrl, close } = await startApp(fetchImpl)
  const patch = (id, data) => fetch(`${baseUrl}/api/jbox/subscriptions/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify(data) }).then((r) => r.json())
  const refresh = (id) => postJson(baseUrl, `/api/jbox/subscriptions/${id}/refresh`, {}).then((r) => r.json())
  try {
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'A' })).json()
    ...
    const order = await (await fetch(`${baseUrl}/api/jbox/subscriptions/order`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ids: [created.id] }) })).json()
    assert.equal(order.changed, false, '只有一条,顺序没变')

    const del = await (await fetch(`${baseUrl}/api/jbox/subscriptions/${created.id}`, { method: 'DELETE' })).json()
    assert.equal(del.changed, true)
    const delAgain = await (await fetch(`${baseUrl}/api/jbox/subscriptions/${created.id}`, { method: 'DELETE' })).json()
    assert.equal(delAgain.changed, false, '本来就不存在')
    // 没有任何 /apply 一类的接口:重启由用户在面板上点
    const apply = await postJson(baseUrl, '/api/jbox/subscriptions/apply', {})
...
test('autoUpdate:新建时存下(归一化到 1~30 天、0~23 点),改它不重拉,关掉就是 null;粘贴来的订阅没有计划', async () => {
  ...
  try {
    ...
    await postJson(baseUrl, '/api/jbox/subscriptions', { content: HK_LINE, name: 'P', autoUpdate: { enabled: true, days: 1, hour: 4 } })
...
test('refresh 期间只改了自动更新开关 → 刷新照常写节点,开关保持新值,不被拉取前的快照改回去', async () => {
  ...
  try {
    release()
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'old', autoUpdate: { enabled: true, days: 1, hour: 3 } })).json()
...
test('refresh 期间改了名字或来源 → 这次结果作废(报错、节点不动),新名字和新来源保留', async () => {
  ...
  try {
    release()
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://a', name: 'old' })).json()
    const { refreshSubscriptionById } = await import('./subscriptions.mjs')
    const slow = deferredFetch()
    const refreshing = refreshSubscriptionById(store, created.id, { fetchImpl: slow.fetchImpl, lookup: fakePublicLookup })
    await new Promise((r) => setTimeout(r, 20))
...
test('创建时所有 UA 都被 403 → 400,错误里逐个列出 UA 和状态码;网络错误不换 UA、只请求一次', async () => {
  ...
  try {
    ...
    assert.match(body.error, /J-Box\/1\.0 → HTTP 403/)
  ...
  try {
    const res = await postJson(app2.baseUrl, '/api/jbox/subscriptions', { url: 'http://sub.example.com/a', name: 'Sub A' })
    assert.equal(res.status, 400)
...
test('#42:内网 / 回环 / CGNAT 的订阅地址正常拉取(127.0.0.1、192.168.x.x、localhost → 127.0.0.1、::ffff:7f00:1、100.64.x)', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const res = await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'http://192.168.198.123:25500/sub?target=clash', name: 'LAN' })
    assert.equal(res.status, 200)
...
test('SSRF 防护(仍保留):未指定地址 / 链路本地(0.0.0.0、::、169.254.x、fe80::)→ 400,且从未真正拉取;file:// 照拒', async () => {
  ...
  for (const host of targets) {
    ...
    const { baseUrl, close } = await startApp(fetchImpl)
    try {
      const res = await postJson(baseUrl, '/api/jbox/subscriptions/preview', { url: `http://${host}:9999/sub` })
...
test('重定向逐跳仍校验:302 到内网地址放行并真的去拉;302 到链路本地 → 400、从未打到目标', async () => {
  const run = async (location) => {
    ...
    const { baseUrl, close } = await startApp(fetchImpl)
    try {
      const res = await postJson(baseUrl, '/api/jbox/subscriptions/preview', { url: 'https://public.example.com/sub' })
...
test('#37:所有 UA 都被 403 时改用 curl 拉(同一个 UA、逐个试),拿到节点就用;没注入 curl 的测试路径不会去跑系统 curl', async () => {
  ...
  try {
    const res = await postJson(b2, '/api/jbox/subscriptions/preview', { url: 'https://public.example.com/sub' })
    assert.equal(res.status, 400)
...
test('#40:PATCH enabled 只改开关不重拉,开关翻转算节点池变了(changed:true);停用的订阅节点不在 activeNodes 里,记录和节点池原样', async () => {
  ...
  const { baseUrl, store, close } = await startApp(fetchImpl)
  try {
    const created = await (await postJson(baseUrl, '/api/jbox/subscriptions', { url: 'https://public.example.com/sub', name: 'A' })).json()
    const id = created.id
    const total = store.getNodes().length
    assert.ok(total >= 1)
    const off = await fetch(`${baseUrl}/api/jbox/subscriptions/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ enabled: false }) })
    ...
    // 再存一次同样的开关:没变化
    const same = await (await fetch(`${baseUrl}/api/jbox/subscriptions/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ enabled: false }) })).json()
    assert.equal(same.changed, false)
    const on = await (await fetch(`${baseUrl}/api/jbox/subscriptions/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ enabled: true }) })).json()
    assert.equal(on.changed, true)
    assert.equal(activeNodes(store).length, total)
    const bad = await fetch(`${baseUrl}/api/jbox/subscriptions/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ enabled: 'yes' }) })
...
test('F1:curl 兜底传到一半失败 → 刷新失败,原有节点池和更新时间原样保留', async () => {
  ...
  try {
    ...
    assert.equal(store.getNodes().length, 2)
    phase = 'refresh'
    const res = await fetch(`${baseUrl}/api/jbox/subscriptions/${created.id}/refresh`, { method: 'POST' })
    assert.equal(res.status, 400)
