...
test('GET /api/jbox/config/preview 返回组装好的配置,不落盘', async () => {
  const ctx = okCtx()
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/config/preview`)
...
test('GET /api/jbox/config/preview 用 store 中现存节点组装分组', async () => {
...
test('POST /api/jbox/deploy 成功路径 → 200,持久化部署态,内核开机自启(enable)', async () => {
  const ctx = okCtx()
  const { baseUrl, store, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/deploy`, { method: 'POST' })
    assert.equal(res.status, 200)
    const body = await res.json()
    ...
    assert.ok(cmds(ctx).includes('/etc/init.d/jbox enable')) // P3 遗留项:开机自启
  } finally {
    await close()
  }
})
...
test('POST /api/jbox/deploy 部署成功但 enableService 抛错 → 响应仍是 200/running,GET /deploy/state 也仍是 running(不被误标为 error)', async () => {
  const ctx = okCtx()
  ...
  try {
    ...
    const stateRes = await fetch(`${baseUrl}/api/jbox/deploy/state`)
...
test('POST /api/jbox/rollback 中 disableService 抛错 → 500 JSON(handler 级 try/catch,而不是未处理异常)', async () => {
  ...
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/rollback`, { method: 'POST' })
...
test('POST /api/jbox/deploy 冲突路径 → 409,未写任何文件,不 enable/disable', async () => {
  ...
  try {
    ...
    assert.ok(!cmds(ctx).some((c) => c.includes('/etc/init.d/jbox enable')))
    assert.ok(cmds(ctx).some((c) => c.includes('/etc/init.d/jbox disable')))
  } finally {
    await close()
  }
})

test('POST /api/jbox/deploy 校验失败 → 409,给 badTags,不写正式配置、不重启', async () => {
...
test('POST /api/jbox/deploy 校验失败但旧内核还在跑(比如点的是重启)→ 不动开机自启', async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/jbox/deploy`, { method: 'POST' })
    assert.equal(res.status, 409)
    assert.ok(!cmds(ctx).some((c) => c.includes('/etc/init.d/jbox enable') || c.includes('/etc/init.d/jbox disable')))
  } finally {
    await close()
  }
})

test('POST /api/jbox/deploy 重启失败 → 500,回滚命令出现,disable 内核开机自启', async () => {
  const ctx = createMockContext({
    files: { [paths.singbox]: '#!/bin/sh\n', [TUN_DEVICE]: '', [`${paths.geoDir}/geosite-cn.srs`]: 'SRS', [`${paths.geoDir}/geoip-cn.srs`]: 'SRS' },
    execResults: {
  ...
  try {
    ...
    const c = cmds(ctx)
    assert.ok(c.includes('/etc/init.d/jbox stop')) // 回滚:停服务
    assert.ok(c.includes('uci -q delete firewall.jbox_panel')) // 回滚:撤防火墙规则
    assert.ok(c.includes('/etc/init.d/jbox disable')) // P3 遗留项:回滚后关闭开机自启
...
test('POST /api/jbox/deploy 启动后未 running(verify 阶段)→ 500,同样 disable', async () => {
  const ctx = createMockContext({
    files: { [paths.singbox]: '#!/bin/sh\n', [TUN_DEVICE]: '', [`${paths.geoDir}/geosite-cn.srs`]: 'SRS', [`${paths.geoDir}/geoip-cn.srs`]: 'SRS' },
...
test('POST /api/jbox/deploy 落盘前步骤(mkdirp)抛出异常 → 500 JSON(不是默认 HTML 错误页),部署态标记 error', async () => {
  const ctx = okCtx()
...
test('GET /api/jbox/deploy/state 返回最近一次部署结果', async () => {
  const ctx = okCtx()
  const { baseUrl, close } = await startApp(ctx)
  try {
    const before = await (await fetch(`${baseUrl}/api/jbox/deploy/state`)).json()
    assert.equal(before.state.stage, 'idle') // 默认态

    await fetch(`${baseUrl}/api/jbox/deploy`, { method: 'POST' })

    const after = await (await fetch(`${baseUrl}/api/jbox/deploy/state`)).json()
...
test('POST /api/jbox/rollback → 恢复直连并 disable 内核开机自启', async () => {
  const ctx = createMockContext({ defaultExec: { code: 0 } })
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/rollback`, { method: 'POST' })
...
test('POST /api/jbox/rollback 命令全失败:不抛、200,但 ok:false 且逐步列出失败(含关自启)', async () => {
...
test('POST /api/jbox/deploy 起来之后又死了(死循环)→ verify 失败,带上内核最后那句 FATAL', async () => {
  const statuses = [
    { code: 0, stdout: 'running' }, // 第一眼:刚起来
    { code: 1, stdout: 'not running' }, // 等几秒再看:已经崩了
  ]
  const ctx = okCtx({
    '/etc/init.d/jbox status': () => statuses.shift() || { code: 1, stdout: 'not running' },
    'logread -e sing-box': {
      code: 0,
      stdout: [
        ...
        'Wed Sep  2 20:29:29 2026 daemon.info procd: Instance jbox::jbox s in a crash loop',
      ].join('\n'),
    },
  })
  const { baseUrl, store, close } = await startApp(ctx)
