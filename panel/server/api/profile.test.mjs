...
import { createStore, DEFAULT_PROFILE } from '../store/jbox-store.mjs'
...
test('validateProfilePatch updates.jbox.days 必须是 1-30 的整数', () => {
  assert.ok(validateProfilePatch({ updates: { jbox: { days: 0 } } }))
  assert.ok(validateProfilePatch({ updates: { jbox: { days: 31 } } }))
  assert.equal(validateProfilePatch({ updates: { jbox: { days: 7 } } }), null)
})
...
test('validateProfilePatch rulesetDir 合法绝对路径通过', () => {
  assert.equal(validateProfilePatch({ rulesetDir: '/opt/j-box/data/rulesets' }), null)
})
...
test('GET /api/jbox/profile 返回默认 profile(地区种子已翻译成站点集)', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await fetch(`${baseUrl}/api/jbox/profile`)
...
test('PUT /api/jbox/profile 深合并后返回并持久化,未提及字段保留', async () => {
  const { baseUrl, store, close } = await startApp()
  try {
    const res = await putJson(baseUrl, '/api/jbox/profile', { ipv6: false, dns: { mode: 'dnsmasq' } })
    assert.equal(res.status, 200)
    const body = await res.json()
...
test('PUT 只碰 ipv6 的部分 patch 不因缺 dns 报错,且不影响 dns', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await putJson(baseUrl, '/api/jbox/profile', { ipv6: false })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.profile.ipv6, false)
...
test('PUT 非法 dns.mode → 400 且不写入', async () => {
  ...
  try {
    ...
    const res = await putJson(baseUrl, '/api/jbox/profile', { dns: { mode: 'bogus' } })
...
test('PUT 非法 ipv6 → 400 且不写入', async () => {
  ...
  try {
    const before = store.getProfile()
    const res = await putJson(baseUrl, '/api/jbox/profile', { ipv6: 'yes' })
    assert.equal(res.status, 400)
    assert.deepEqual(store.getProfile(), before)
  } finally {
    await close()
...
test('PUT 非法 routing.categories(缺 target) → 400 且不写入', async () => {
  ...
  try {
    const before = store.getProfile()
    const res = await putJson(baseUrl, '/api/jbox/profile', {
...
test('PUT 合法的 directRulesets 与 rulesetDir 仍能通过并落库', async () => {
  ...
  try {
    const res = await putJson(baseUrl, '/api/jbox/profile', {
      ...
      rulesetDir: '/opt/j-box/data/rulesets2',
    })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.deepEqual(body.profile.routing.directRulesets, ['geosite-cn', 'geoip-cn'])
    assert.equal(body.profile.rulesetDir, '/opt/j-box/data/rulesets2')
...
test('GET /defaults?region=CN → 中国大陆那一档', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await fetch(`${baseUrl}/api/jbox/profile/defaults?region=CN`)
    assert.equal(res.status, 200)
    const body = await res.json()
...
test('GET /defaults?region=HKMO → 香港澳门那一档', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await fetch(`${baseUrl}/api/jbox/profile/defaults?region=HKMO`)
    const body = await res.json()
...
test('GET /defaults?region=不认识的 → 回落到中国大陆', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await fetch(`${baseUrl}/api/jbox/profile/defaults?region=US`)
    const body = await res.json()
    assert.equal(body.defaults.fallbackDefault, 'proxy')
  } finally {
    await close()
...
test('GET /defaults 缺 region → 按 CN 兜底', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await fetch(`${baseUrl}/api/jbox/profile/defaults`)
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.defaults.fallbackDefault, 'proxy')
  } finally {
...
test('PUT 校验:策略必须有名字', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await putJson(baseUrl, '/api/jbox/profile', { routing: { policies: [{ rulesets: ['geosite-google'] }] } })
    assert.equal(res.status, 400)
  } finally {
    await close()
  }
...
test('PUT 校验:兜底只收字符串;「其他」是兜底占着的名字,站点集不能重名', async () => {
  const { baseUrl, close } = await startApp()
  try {
    assert.equal((await putJson(baseUrl, '/api/jbox/profile', { routing: { fallbackDefault: 'direct' } })).status, 200)
    assert.equal((await putJson(baseUrl, '/api/jbox/profile', { routing: { fallbackDefault: 3 } })).status, 400)
    assert.equal(
      (await putJson(baseUrl, '/api/jbox/profile', {
...
test('GET 时把地区翻译成站点集写回档案:界面看到的和内核跑的必须是同一份', async () => {
  ...
  try {
    const body = await (await fetch(`${baseUrl}/api/jbox/profile`)).json()
    ...
    // 已经落库,再读一次不会再长出一个
    const again = await (await fetch(`${baseUrl}/api/jbox/profile`)).json()
...
test('PUT 校验:测速地址必须是 http(s) URL', async () => {
  const { baseUrl, close } = await startApp()
  try {
    assert.equal((await putJson(baseUrl, '/api/jbox/profile', { testUrl: 'http://connect.rom.miui.com/generate_204' })).status, 200)
    assert.equal((await putJson(baseUrl, '/api/jbox/profile', { directTestUrl: 'https://www.msftconnecttest.com/connecttest.txt' })).status, 200)
    assert.equal((await putJson(baseUrl, '/api/jbox/profile', { testUrl: 'gstatic.com' })).status, 400)
    assert.equal((await putJson(baseUrl, '/api/jbox/profile', { directTestUrl: 'ftp://x' })).status, 400)
  } finally {
    await close()
  }
})
