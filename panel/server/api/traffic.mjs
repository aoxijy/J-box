// 每日流量查询:GET /api/jbox/traffic/month?month=YYYY-MM(月视图)、
// GET /api/jbox/traffic/day?day=YYYY-MM-DD(某天按节点、按域名/IP 的明细)。
...
export const registerTrafficRoutes = (app, { collector, ctx, paths, store, now = () => new Date() }) => {
  ...
  // GET /api/jbox/clients:终端分流选来源用。DHCP 租约里的设备 + 今天在流量里出现过的来源 IP
