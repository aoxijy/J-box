...
const importServerModule = async ({ dbPath, clashApiBase }) => {
  ...
  if (clashApiBase) {
    process.env.JBOX_CLASH_API_BASE = clashApiBase
  } else {
    delete process.env.JBOX_CLASH_API_BASE
  }
...
const makeTempDbPath = async (label) => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `jbox-controller-proxy-${label}-`))
