...
test('POST /route-test:内核解析 + 真实访问 + 在连接表里找到这条连接的链路', async () => {
  ...
  try {
    const res = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/route-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ target: 'www.baidu.com' }) })
...
test('POST /route-test:访问失败也报出站链路——请求挂着的时候就从连接表里认出探测连接', async () => {
  ...
  const run = async ({ withProbeConn }) => {
    ...
    await new Promise((r) => server.once('listening', r))
    try {
      const res = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/route-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ target: '8.8.8.8' }) })
      assert.equal(res.status, 200)
...
test('DNS 规则过期:内核里的选择和配置里定死的判断对不上就标出来,只换代理线路不标', async () => {
  ...
  const run = async (proxies, target) => {
    ...
    await new Promise((r) => server.once('listening', r))
    try {
      const res = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/route-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ target }) })
...
test('POST /route-test:每次查询都先清内核 DNS 缓存,而且清在解析之前', async () => {
  ...
  const run = async (body) => {
    ...
    await new Promise((r) => server.once('listening', r))
    try {
      const res = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/route-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) })
      assert.equal(res.status, 200)
      return calls
    } finally {
      await new Promise((r) => server.close(r))
...
test('S4:指定终端来源时,响应明确标出 DNS 判定是按终端预测的、解析和访问是面板自己发起的,没有该终端的来源(不冒充该终端实测)', async () => {
  ...
  await new Promise((r) => server.once('listening', r))
  try {
    const res = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/route-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ target: 'example.com', sourceIp: '192.168.3.9' }) })
    const body = await res.json()
    ...
    // 不给来源就没有这段
    const res2 = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/route-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ target: 'example.com' }) })
...
test('decideDnsServer:内核自己的 fakeip 规则先命中时记成 fakeIpRule,判定继续找真解析器;探测到占位地址标 fakeIpLocal 而不是"对端截下了查询"(第三轮 阶段 3)', async () => {
  ...
  await new Promise((r) => server.once('listening', r))
  try {
    const res = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/route-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ target: 'www.google.com' }) })
    const body = await res.json()
