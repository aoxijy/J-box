...
test('GET /servers/port-check', async () => {
  ...
  try {
    ...
    assert.deepEqual(await (await fetch(`${base}/api/jbox/servers/port-check?port=2026`)).json(), { ok: false, reason: 'reserved' })
    assert.deepEqual(await (await fetch(`${base}/api/jbox/servers/port-check?port=9000&id=a`)).json(), { ok: true })
  } finally {
