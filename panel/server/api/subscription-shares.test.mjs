...
test('subscription share management and public URL', async () => {
  ...
  try {
    const createdResponse = await fetch(`${base}/api/jbox/subscription-shares`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: '手机', host: 'router.local:2026', subscriptionIds: ['one'] }) })
    ...
    const regenerated = await fetch(`${base}/api/jbox/subscription-shares/${share.id}/regenerate`, { method: 'POST' })
    assert.equal(regenerated.status, 200)
    const next = (await regenerated.json()).share
    assert.notEqual(next.token, share.token)
    const disabled = await fetch(`${base}/api/jbox/subscription-shares/${share.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ enabled: false }) })
...
test('public URL decodes base64 subscription content before serving', async () => {
  ...
  try {
    ...
    const createdResponse = await fetch(`${base}/api/jbox/subscription-shares`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'base64', host: 'router.local:2026', subscriptionIds: ['one'] }) })
    const { share } = await createdResponse.json()
