...
export const validateProfilePatch = (patch, { reservedNames = [] } = {}) => {
  ...
  // 自动更新计划:jbox {auto, hour, days, channel} / geo {auto, hour, days, channel}
  if ('updates' in patch) {
    ...
    if ('jbox' in u) {
      const o = u.jbox
      if (!isPlainObject(o)) return 'updates.jbox must be an object'
      if ('auto' in o && !isBoolean(o.auto)) return 'updates.jbox.auto must be a boolean'
      if ('hour' in o && !isHour(o.hour)) return 'updates.jbox.hour must be an integer 0-23'
      if ('channel' in o && !['auto', 'direct', 'mirror'].includes(o.channel)) return 'updates.jbox.channel must be auto, direct or mirror'
      if ('checkChannel' in o && !['auto', 'direct', 'mirror'].includes(o.checkChannel)) return 'updates.jbox.checkChannel must be auto, direct or mirror'
      if ('days' in o && !(Number.isInteger(o.days) && o.days >= 1 && o.days <= 30)) return 'updates.jbox.days must be an integer 1-30'
    }
    if ('geo' in u) {
      const g = u.geo
...
// 提供(见 store/jbox-store.mjs),这里不替用户改写规则。
...
export const registerProfileRoutes = (app, { store } = {}) => {
  ...
  app.use('/api/jbox/profile', router)
}
