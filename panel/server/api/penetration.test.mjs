...
const post = async (baseUrl, target, extra = {}) => {
  const res = await fetch(`${baseUrl}/api/jbox/penetration`, {
    method: 'POST',
...
// ---- POST /api/jbox/penetration ----

test('POST /api/jbox/penetration 缺 target → 400,不 exec', async () => {
  const ctx = createMockContext({})
...
test('POST /api/jbox/penetration target 以 "-" 开头("--help") → 400,不 exec', async () => {
  const ctx = createMockContext({})
  const { baseUrl, close } = await startApp({ ctx })
  try {
...
test('POST /api/jbox/penetration target 以 "-" 开头("-x") → 400,不 exec', async () => {
  const ctx = createMockContext({})
  const { baseUrl, close } = await startApp({ ctx })
  try {
...
test('POST /api/jbox/penetration 合法域名/IPv4/IPv6 target 仍然通过(不被参数校验误拦)', async () => {
  // 走默认 profile(directRulesets: ['geosite-cn','geoip-cn']),所以要把这两个 .srs 的
  // 存在性也搭好,否则会在 could-not-check 那条路径上被拦下来,而不是这个用例真正想测的
  // 参数校验路径。
  const ctx = createMockContext({
    files: withSingbox(
      '/opt/j-box/panel/server/resources/geodata/geosite-cn.srs',
      '/opt/j-box/panel/server/resources/geodata/geoip-cn.srs',
    ),
...
test('按序首个命中生效:前一条 rule_set 命中时,后一条同样会命中的规则不会被求值(shadow)', async () => {
  ...
  const keyA = `${paths.singbox} rule-set match -f binary /opt/j-box/panel/server/resources/geodata/geosite-a.srs ${target}`
  const keyB = `${paths.singbox} rule-set match -f binary /opt/j-box/panel/server/resources/geodata/geosite-b.srs ${target}`
  const ctx = createMockContext({
    // geosite-b 从未被求值(短路),所以只需要 geosite-a 的 .srs 存在即可让 matchRuleSet
    // 走到 exec 那一步。
    files: withSingbox('/opt/j-box/panel/server/resources/geodata/geosite-a.srs'),
    execResults: {
...
test('策略组下钻:outbound 为策略组时经 clash_api 沿 now 字段逐层下钻到叶子节点', async () => {
  ...
  const keyHk = `${paths.singbox} rule-set match -f binary /opt/j-box/panel/server/resources/geodata/geosite-hk.srs ${target}`
  const ctx = createMockContext({
    files: withSingbox('/opt/j-box/panel/server/resources/geodata/geosite-hk.srs'),
...
test('ad-block reject 规则命中:matched.action=reject,无 outbound,不下钻', async () => {
  ...
  const keyAds = `${paths.singbox} rule-set match -f binary /opt/j-box/panel/server/resources/geodata/geosite-ads.srs ${target}`
  const ctx = createMockContext({
    files: withSingbox('/opt/j-box/panel/server/resources/geodata/geosite-ads.srs'),
...
// ---- POST /api/jbox/penetration:matchError(P4b 终审 Important 1)----
...
test('POST /penetration:sing-box 异常退出且无输出 → 200 + matchError,不是"确认不命中"', async () => {
  ...
  const key = `${paths.singbox} rule-set match -f binary /opt/j-box/panel/server/resources/geodata/geosite-cn.srs ${target}`
  const ctx = createMockContext({
    files: withSingbox(
      '/opt/j-box/panel/server/resources/geodata/geosite-cn.srs',
      '/opt/j-box/panel/server/resources/geodata/geoip-cn.srs',
...
test('订阅和节点站点直连(默认开):目标是某个节点的服务器域名 → 直连,排在站点集之前', async () => {
  ...
  const ctx = createMockContext({ files: withSingbox('/opt/j-box/panel/server/resources/geodata/geosite-hk.srs') })
...
test('分流改过但没重启:回传 routingStale', async () => {
  ...
  const metaPath = '/opt/j-box/etc/config.meta.json'
