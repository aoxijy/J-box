...
const LEASE = { ip: '10.0.0.160', mask: 24, gateway: '10.0.0.1', dns: ['10.0.0.1'], mac: '02:4f:42:00:00:01', hostname: 'jbox-probe', lanDevice: 'br-lan', dhcpMs: 1200, reused: false }
...
test('旁路目标:conntrack 里没改写、路由从 eth0 转出、回包发往 WAN 地址、内核连接表没有 → bypass,并带全部证据', async () => {
  ...
  assert.deepEqual(r.source, { kind: 'virtual', name: 'jbox-probe', ip: '10.0.0.160', mac: '02:4f:42:00:00:01', via: 'dhcp', dns: ['10.0.0.1'], gateway: '10.0.0.1', lanDevice: 'br-lan', reused: false, dhcpMs: 1200 })
...
test('HTTP 路由:非法目标 400;capability 接口回能力;POST 返回测试结果', async () => {
  ...
  try {
    const cap = await (await fetch(`${base}/api/jbox/terminal-test/capability`)).json()
    assert.deepEqual(cap, { ok: false, missing: ['root'], lan: null })
    const bad = await fetch(`${base}/api/jbox/terminal-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ target: '-x' }) })
    assert.equal(bad.status, 400)
    const ok = await fetch(`${base}/api/jbox/terminal-test`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ target: 'Example.COM' }) })
