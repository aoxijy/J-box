...
const put = (baseUrl, groups) => fetch(`${baseUrl}/api/jbox/groups`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ groups }) })
...
test('PUT /groups(failover):合法定义保存后 lanes 原样落库、mode 固定 static、参数补默认,内部子组不在公开候选里', async () => {
  ...
  const { baseUrl, close } = await startApp(store)
  try {
    const list = await (await fetch(`${baseUrl}/api/jbox/groups`)).json()
