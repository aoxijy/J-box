...
test('buildBackup:带档案和节点组,订阅 / 节点可选;不带密码、clash 密钥这类和机器绑定的东西', () => {
  ...
  const full = buildBackup(store, { now: () => new Date(Date.UTC(2026, 8, 5, 12, 0, 0)), jboxVersion: 'v0.1.108' })
  assert.equal(full.format, BACKUP_FORMAT)
  assert.equal(full.version, 1)
  assert.equal(full.exportedAt, '2026-09-05T12:00:00.000Z')
  assert.equal(full.jboxVersion, 'v0.1.108')
...
test('applyBackup:导进一个空库,档案 / 组 / 订阅 / 节点都在;rulesetDir 不导入;挂在不存在订阅上的节点丢掉', () => {
  ...
  assert.equal(dst.getProfile().rulesetDir, '/opt/j-box/data/rulesets', '本机路径不跟着文件走')
...
test('applyBackup:不带订阅的文件不动现有订阅;格式不对、版本不认识、档案不合法都拒绝且不落库', () => {
  ...
  assert.match(applyBackup(store, { hello: 1 }).error, /不是 J-Box/)
...
test('HTTP:GET /backup 按 subscriptions 参数决定带不带订阅;POST /backup/import 落库,坏文件 400', async () => {
  ...
  try {
    const full = await (await fetch(`${base}/api/jbox/backup`)).json()
    assert.equal(full.jboxVersion, 'v0.1.108')
    assert.equal(full.nodes.length, 2)
    const slim = await (await fetch(`${base}/api/jbox/backup?subscriptions=0&servers=0`)).json()
    assert.equal(slim.nodes, undefined)
    ...
    try {
      const ok = await fetch(`${base2}/api/jbox/backup/import`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(full) })
      ...
      const app_ = await fetch(`${base2}/api/jbox/backup/import?subscriptions=append`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(full) })
      ...
      const badMode = await fetch(`${base2}/api/jbox/backup/import?subscriptions=merge`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(full) })
      assert.equal(badMode.status, 400)
      const bad = await fetch(`${base2}/api/jbox/backup/import`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ format: 'x' }) })
...
test('面板设置和背景图:导出只带 config/ 且不是密码的键;导入整份换掉面板设置、背景图给空串就清掉', () => {
  ...
  const panelStorage = {
    readEntries: () => Object.fromEntries([...kv].filter(([k]) => !k.startsWith('jbox/') && !k.startsWith('config/access-'))),
  ...
  assert.deepEqual(file.panelSettings, { 'config/language': 'zh', 'config/global-radius': '15' }, '密码和 jbox/* 不导')
  ...
  incoming.panelSettings = { 'config/language': 'en', 'config/theme-mode': 'dark', 'config/access-password-hash': 'EVIL', 'jbox/profile': 'x', 'config/bad': 123 }
  ...
  assert.equal(kv.get('jbox/profile'), '{}')
