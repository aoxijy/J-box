...
const okCtx = (over = {}) => createMockContext({
  ...
  execResults: {
    ...
    '/etc/init.d/jbox-panel status': { code: 1, stdout: 'inactive' },
    ...over,
  },
})
...
test('GET /api/jbox/service/status → {core:{running,raw}, panel:{running,raw}, conflicts:[]}', async () => {
  const ctx = okCtx()
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/status`)
...
test('POST /api/jbox/service/core/start → {ok,code,stderr}', async () => {
  const ctx = okCtx()
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/core/start`, { method: 'POST' })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.ok, true)
    assert.ok(typeof body.code === 'number')
    assert.ok(typeof body.stderr === 'string')
    assert.ok(cmds(ctx).includes('/etc/init.d/jbox restart'))
  } finally {
    await close()
  }
})
...
test('启动内核 = 用当前设置重新生成配置并落盘', async () => {
  ...
  try {
    await fetch(`${baseUrl}/api/jbox/service/core/start`, { method: 'POST' })
...
test('POST /api/jbox/service/core/stop → {ok,code,stderr}', async () => {
  const ctx = okCtx({ '/etc/init.d/jbox status': { code: 1, stdout: 'inactive' } })
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/core/stop`, { method: 'POST' })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.ok, true)
    assert.ok(cmds(ctx).includes('/etc/init.d/jbox stop'))
    // 停止必须同时关掉开机自启,否则坏配置把网搞断时「停止」扛不过一次重启。
    assert.ok(cmds(ctx).includes('/etc/init.d/jbox disable'))
  } finally {
    await close()
  }
})
...
test('停止失败时不应关闭自启(内核还在跑,关自启只会让状态更乱)', async () => {
  const ctx = okCtx({ '/etc/init.d/jbox stop': { code: 1, stdout: '', stderr: 'boom' } })
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/core/stop`, { method: 'POST' })
    const body = await res.json()
...
test('停止成功但关自启失败时,如实把原因带回来', async () => {
  const ctx = okCtx({ '/etc/init.d/jbox disable': { code: 1, stdout: '', stderr: 'no rc.d' }, '/etc/init.d/jbox status': { code: 1, stdout: 'inactive' } })
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/core/stop`, { method: 'POST' })
    const body = await res.json()
...
test('重启不得关闭自启(init 的 restart 内部就是 stop+start,不能顺手把自启关了)', async () => {
  ...
  const { baseUrl, close } = await startApp(ctx)
  try {
    await fetch(`${baseUrl}/api/jbox/service/core/restart`, { method: 'POST' })
    assert.ok(!cmds(ctx).includes('/etc/init.d/jbox disable'))
  } finally {
    await close()
  }
...
test('POST /api/jbox/service/core/restart → {ok,code,stderr}', async () => {
  const ctx = okCtx()
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/core/restart`, { method: 'POST' })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.ok, true)
    assert.ok(cmds(ctx).includes('/etc/init.d/jbox restart'))
...
test('POST /api/jbox/service/core/enable → {ok,code,stderr}', async () => {
  const ctx = okCtx()
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/core/enable`, { method: 'POST' })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.ok, true)
    assert.ok(cmds(ctx).includes('/etc/init.d/jbox enable'))
  } finally {
    await close()
  }
})

test('POST /api/jbox/service/core/disable → {ok,code,stderr}', async () => {
  const ctx = okCtx()
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/core/disable`, { method: 'POST' })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.ok, true)
    assert.ok(cmds(ctx).includes('/etc/init.d/jbox disable'))
...
test('POST /api/jbox/service/core/invalid → 400 WITHOUT executing anything', async () => {
  const ctx = okCtx()
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/service/core/invalid`, { method: 'POST' })
...
test('GET /api/jbox/kernel/version → exec paths.singbox version, parse first line', async () => {
  const ctx = okCtx({
    [`${paths.singbox} version`]: { code: 0, stdout: 'sing-box 1.9.4\nBuild version: abc123', stderr: '' },
  })
  const { baseUrl, close } = await startApp(ctx)
  try {
    const res = await fetch(`${baseUrl}/api/jbox/kernel/version`)
    assert.equal(res.status, 200)
    const body = await res.json()
...
test('GET /api/jbox/kernel/version handles missing singbox gracefully', async () => {
  const ctx = okCtx({
...
test('GET /service/status:init 脚本 enabled 退出码非 0 → core.autostart=false', async () => {
  const ctx = okCtx({ '/etc/init.d/jbox enabled': { code: 1 } })
  const { baseUrl, close } = await startApp(ctx)
  try {
    const body = await (await fetch(`${baseUrl}/api/jbox/service/status`)).json()
...
test('停止后内核迟迟不退出 → ok:false 并说明,不再谎报已停止', async () => {
  ...
  try {
    ...
    assert.ok(!cmds(ctx).includes('/etc/init.d/jbox disable'))
    assert.ok(cmds(ctx).filter((c) => c === '/etc/init.d/jbox status').length >= 2, '应该轮询过 status')
  } finally {
    await close()
  }
})
...
test('部署途中点停止:部署被标成取消(不 enable),停止随后执行并 disable,最终状态跟最后一个动作走', async () => {
  ...
  let stopped = false
  const ctx = okCtx({
    '/etc/init.d/jbox status': () => (stopped ? { code: 1, stdout: 'inactive' } : { code: 0, stdout: 'running' }),
    '/etc/init.d/jbox stop': () => { stopped = true; return { code: 0 } },
  })
  // deployConfig 验证阶段会 sleep 几秒再看第二眼:借这个 sleep 把部署挂住
  ctx.sleep = () => verifyGate
  const { baseUrl, close } = await startApp(ctx)
  try {
    const starting = fetch(`${baseUrl}/api/jbox/service/core/start`, { method: 'POST' })
    // 等部署走到重启内核之后(挂在验证的 sleep 里)
    for (let i = 0; i < 200 && !cmds(ctx).includes('/etc/init.d/jbox restart'); i++) await new Promise((r) => setTimeout(r, 5))
    assert.ok(cmds(ctx).includes('/etc/init.d/jbox restart'))
    const stopping = fetch(`${baseUrl}/api/jbox/service/core/stop`, { method: 'POST' })
    ...
    const c = cmds(ctx)
    assert.ok(c.indexOf('/etc/init.d/jbox restart') < c.indexOf('/etc/init.d/jbox stop'), '停止必须排在部署之后执行')
    assert.ok(!c.includes('/etc/init.d/jbox enable'), '被取消的部署不能把自启打开')
    assert.ok(c.includes('/etc/init.d/jbox disable'))
  } finally {
    await close()
  }
})
