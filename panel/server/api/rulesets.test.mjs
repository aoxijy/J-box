...
test('列出一个分类里的条目,按类型+值摊平', async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/jbox/rulesets/entries?tag=geosite-google`)
...
test('搜索与分页只作用在匹配结果上', async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/jbox/rulesets/entries?tag=geosite-google&q=GOOGLE&offset=1&limit=1`)
    const body = await res.json()
    assert.equal(body.total, 5)
...
test('安装包缺少分类时清楚报错，不在线下载也不修改旧数据', async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/jbox/rulesets/entries?tag=geosite-openai`)
...
test('名字不合法直接 400——tag 会被拼进下载地址和文件名', async () => {
  ...
  try {
    for (const tag of ['../../etc/passwd', 'geosite-../x', 'whatever-cn', '']) {
      const res = await fetch(`${baseUrl}/api/jbox/rulesets/entries?tag=${encodeURIComponent(tag)}`)
...
test('GET /policies/entries:站点集的规则集展开 + 手写条件,分档计数、搜索、分页;兜底返回空', async () => {
  ...
  try {
    const all = await (await fetch(`${base}/api/jbox/policies/entries?name=Google`)).json()
    ...
    const ip = await (await fetch(`${base}/api/jbox/policies/entries?name=Google&tab=ip`)).json()
    assert.equal(ip.matched, 2)
    const q = await (await fetch(`${base}/api/jbox/policies/entries?name=Google&q=gstatic`)).json()
    assert.equal(q.matched, 1)
    const page = await (await fetch(`${base}/api/jbox/policies/entries?name=Google&limit=3`)).json()
    assert.equal(page.entries.length, 3)
    assert.equal(page.hasMore, true)
    const fb = await (await fetch(`${base}/api/jbox/policies/entries?name=${encodeURIComponent('其他')}`)).json()
    assert.equal(fb.fallback, true)
    const nf = await fetch(`${base}/api/jbox/policies/entries?name=Nope`)
...
test('GET /rulesets/preview:按网址拉回来解析,形状和 /rulesets/entries 一致;非 http(s) 直接 400;拉不动 503', async () => {
  ...
  const base = `http://127.0.0.1:${server.address().port}/api/jbox/rulesets/preview`
  try {
