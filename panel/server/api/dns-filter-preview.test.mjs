...
const fixture = (body, previewFetch = async () => { throw new Error('must not download') }) => {
  ...
  let reads = 0
  const deps = {
    store: { getProfile: () => profile, getRaw: (key) => { assert.equal(key, 'jbox/dns-filter-lists'); return JSON.stringify(state) }, setRaw: () => assert.fail('preview wrote state'), setProfile: () => assert.fail('preview saved settings') },
...
const api = async (t, deps) => {
  ...
  return async (params = {}) => {
    const response = await fetch(`http://127.0.0.1:${server.address().port}/api/jbox/dns-filter/preview?${new URLSearchParams({ url, ...params })}`)
