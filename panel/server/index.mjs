...
// J-Box 只管理本机唯一的 sing-box,clash_api 固定监听 127.0.0.1:9095;
// 环境变量覆盖仅用于测试(指向假上游),生产环境不应设置。
const DEFAULT_CLASH_API_BASE = process.env.JBOX_CLASH_API_BASE || 'http://127.0.0.1:9095'
...
// jbox-store 复用同一张 app_storage KV 表;controller 代理靠它拿本机 clash_api 的 secret。
...
// 键在 jbox/ 前缀下(isProtectedStorageKey 保护):不回显给浏览器,也不被设置同步清掉。
// 密钥只用来给会话记录里的 passwordTag 做 HMAC(把会话绑到签发时的密码上,改密即失效)。
const SESSION_SECRET_KEY = 'jbox/session-secret'
...
// 会话表也落库(同样在 jbox/ 前缀下):每次登录签发一个随机 id,记下签发时间、到期时间和
...
const SESSIONS_KEY = 'jbox/sessions'
...
// J-Box 系统层依赖:paths 描述 OpenWrt 上的固定安装布局,ctx 是真实的 exec/fs 抽象
...
export const isProtectedStorageKey = (key) => {
  return (
    typeof key === 'string' &&
    (key.startsWith('jbox/') || key.startsWith('config/access-') || key === backgroundImageStorageKey)
  )
}
...
const getProxyTarget = (req) => {
  ...
  // 缺省即本机:J-Box 只管理唯一的本地 sing-box,secret 从 store 里取。
...
const syncSelectionsAfterProxySwitch = () => {
  ...
  selectionSyncTimer = setTimeout(async () => {
    ...
    try {
      ...
      await regenerateIfPlanChanged({ store, ctx: jbCtx, paths: jbPaths, selections, log: (m) => console.log(m) })
    } catch (error) {
...
// Express 默认路由大小写不敏感:GET /API/jbox/profile 会命中 /api/jbox/profile 的路由,
...
// J-Box 业务路由:全部挂在守卫中间件之后、静态资源/SPA fallback 之前,
...
registerDeployRoutes(app, { store, ctx: jbCtx, paths: jbPaths })
registerServiceRoutes(app, { store, ctx: jbCtx, paths: jbPaths })
registerRulesetRoutes(app, { store, ctx: jbCtx, paths: jbPaths, fetchImpl: globalThis.fetch })
registerPenetrationRoutes(app, { store, ctx: jbCtx, paths: jbPaths, fetchImpl: globalThis.fetch })
registerNodeLatencyRoutes(app, { ctx: jbCtx, paths: jbPaths, store, fetchImpl: globalThis.fetch })
registerGroupRoutes(app, { store })
registerUpdateRoutes(app, { store, ctx: jbCtx, paths: jbPaths, fetchImpl: globalThis.fetch })
registerRouteTestRoutes(app, { store, ctx: jbCtx, paths: jbPaths, fetchImpl: globalThis.fetch })
registerTerminalTestRoutes(app, { store, ctx: jbCtx, paths: jbPaths, fetchImpl: globalThis.fetch })
...
registerTrafficRoutes(app, { collector: trafficCollector, ctx: jbCtx, paths: jbPaths, store })
...
const readDeployedConfig = async () => {
  let version = ''
  try { version = String(JSON.parse(await jbCtx.readFile(`${jbPaths.etc}/config.meta.json`)).generatedAt || '') } catch { version = '' }
  if (deployedDnsConfig.config && deployedDnsConfig.version === version) return deployedDnsConfig.config
  const config = JSON.parse(await jbCtx.readFile(jbPaths.configPath))
...
const dnsRewriteSourceViaProxy = async (name) => {
  try {
    ...
    const d = await decideDnsServer(jbCtx, jbPaths, config, name, { ignoreServers: [DNS_REWRITE_TAG] })
    if (!d || d.error) return null
    if (d.rejected) return false
    return Boolean(d.viaProxy)
  } catch { return null }
}
const dnsRewriteServer = createDnsRewriteServer({ store, fallbackServers: () => readSystemDns(jbCtx).catch(() => []), sourceViaProxy: dnsRewriteSourceViaProxy, log: (m) => console.log(m) })
...
const dnsFilterObserver = createDnsFilterObserver({
  ...
  enabled: async () => {
    try { return JSON.parse(await jbCtx.readFile(`${jbPaths.etc}/config.meta.json`)).dnsFilter?.enabled === true }
    catch { return false }
  },
})
const dnsFilterUpdater = registerDnsFilterRoutes(app, { store, ctx: jbCtx, paths: jbPaths, data: dnsFilterData, observer: dnsFilterObserver })
let dnsFilterTimer
registerServerRoutes(app, { store, ctx: jbCtx })
// 导出诊断包(后端设置那张卡片):版本、固件、内核状态、脱敏配置、最近日志,给 issue 用
registerDiagnosticsRoutes(app, { store, ctx: jbCtx, paths: jbPaths })
// 导出 / 导入(后端设置那张卡片):档案 + 节点组,可选订阅和节点
registerBackupRoutes(app, {
  store,
  readVersion: async () => (await readMeta(jbCtx, jbPaths)).version || '',
...
startScheduler({ store, ctx: jbCtx, paths: jbPaths, fetchImpl: globalThis.fetch, subscriptionFetchImpl: subscriptionFetch, runDeploy, log: (m) => console.log(m) })
...
const startServer = async () => {
  ...
  teardownProbeNetns(jbCtx).catch(() => {})
  if (server.listening) {
    return server
  }
