...
test('生成的配置通过 sing-box check(全协议 + wireguard + DNS 分流 + 广告)', { skip: skipIfNoBin }, () => {
  requireBin()
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'jbox-check-'))
  try {
    // 组织多协议订阅样本(分享链接)
    const sub = [
...
test('坏节点(缺 method)导致 check 失败', { skip: skipIfNoBin }, () => {
  requireBin()
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'jbox-checkbad-'))
  try {
    const config = {
      log: { level: 'warn' },
...
test('生成的配置通过 sing-box check(sing-box JSON 订阅 → wireguard endpoint)', { skip: skipIfNoBin }, () => {
  requireBin()
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'jbox-checkwg-'))
  try {
...
test('生成的配置通过 sing-box check(dns.mode=dnsmasq;仅 dns-in 入站被劫持,防 hijack 回环回归)', { skip: skipIfNoBin }, () => {
  requireBin()
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'jbox-checkdnsmasq-'))
  try {
    const sub = [
      'ss://YWVzLTI1Ni1nY206c2VjcmV0cHc=@us.example.com:8388#US-01',
      'trojan://pw@jp.example.com:443?sni=jp.example.com#JP-01',
...
test('一个节点都没命中的用户组也能过 sing-box check(挂 direct 占位)', { skip: skipIfNoBin }, () => {
  requireBin()
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'jbox-check-empty-'))
  try {
...
for (const fallbackDefault of ['direct', 'proxy']) {
  ...
  requireBin()
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), `jbox-check-${fallbackDefault}-`))
    try {
...
for (const mode of ['hijack', 'off']) {
  ...
  requireBin()
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), `jbox-check-${mode}-`))
    try {
      const sub = [
        'ss://YWVzLTI1Ni1nY206c2VjcmV0cHc=@us.example.com:8388#US-01',
        'trojan://pw@jp.example.com:443?sni=jp.example.com#JP-01',
